...
import { buildTextContent, normalizeMarkdownLineBreaks, } from '../../platforms/feishu/card-builders/content-builder.js';
...
export class FeishuAdapter {
    ...
    convert(message) {
        ...
        switch (content.type) {
            ...
            case 'file':
                ...
                throw new Error('File content cannot be converted synchronously. ' +
                    'Use send() directly for file messages, which handles upload via Feishu API.');
            case 'done':
                return {
                    msg_type: 'text',
                    content: buildTextContent(content.success
    ...
    async send(message) {
        try {
            ...
            const feishuMessage = this.convert(message);
     ...
     */
    async sendFileMessage(client, message) {
