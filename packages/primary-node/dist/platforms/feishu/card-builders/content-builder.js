 ...
 */
export function normalizeMarkdownLineBreaks(text) {
    return text
        .replace(/(?<!\\)\\r\\n/g, '\n')
        .replace(/(?<!\\)\\n/g, '\n')
        .replace(/(?<!\\)\\r/g, '\r');
 ...
 */
export function buildTextContent(text) {
...
export function normalizeCardMarkdown(value) {
    if (Array.isArray(value)) {
        return value.map((item) => normalizeCardMarkdown(item));
    }
    if (!value || typeof value !== 'object') {
        return value;
    }
    const record = value;
    const normalized = Object.fromEntries(Object.entries(record).map(([key, item]) => [key, normalizeCardMarkdown(item)]));
 ...
 */
export function buildPostContent(elements, title) {
    const postContent = {
        zh_cn: {
            ...
            content: elements.map((row) => row.map((element) => element.tag === 'text'
 ...
 */
export function buildSimplePostContent(text, title) {
    const element = {
        tag: 'text',
        text,
    };
