...
export class MessageHandler {
    ...
    async handleCardAction(data) {
        ...
        if (this.callbacks.routeCardAction) {
            logger.debug({ messageId: message_id, chatId: chat_id, actionValue: action.value }, 'Attempting to route card action');
        ...
        try {
            logger.debug({ messageId: cardActionMessageId, cardMessageId: message_id, chatId: chat_id, actionValue: action.value, routed: false }, 'Emitting card action as local message to agent');
            await this.callbacks.emitMessage({
