...
export class MessageHandler {
  ...
  async handleMessageReceive(data: FeishuEventData): Promise<void> {
    ...
    // Issue #3592: Re-validate small group status when group may have grown
    ...
    if (this.isGroupChat(chat_type) && !botMentioned && !isTriggerCommand) {
      ...
      // Issue #3592: Also re-check for already-marked small groups (with throttle)
      if (this.triggerModeManager.getMode(chat_id) === 'auto'
        && this.triggerModeManager.shouldRecheckSmallGroup(chat_id)) {
        await this.checkAndAutoDisableSmallGroup(chat_id);
      }
      // Skip message if trigger mode is not enabled
   ...
   * Issue #3592: Unmark small groups when they grow beyond 2 members.
   *
  ...
  private async checkAndAutoDisableSmallGroup(chatId: string): Promise<void> {
    ...
    try {
      ...
      } else if (this.triggerModeManager.isSmallGroup(chatId)) {
        // Issue #3592: Group grew beyond 2 members — unmark as small group
