...
export class MessageHandler {
  ...
  private async handleQuotedFileMessage(
    ...
    const downloadCmd = `npx @larksuite/cli im +messages-resources-download --message-id ${messageId} --file-key ${fileKey} --type ${resourceType} --as bot --output ./downloaded_file.${resourceType === 'image' ? 'jpg' : 'bin'}`;
    if (!localPath) {
      return {
  ...
  async handleMessageReceive(data: FeishuEventData): Promise<void> {
    ...
    if (message_type === 'image' || message_type === 'file' || message_type === 'media' || message_type === 'audio') {
      ...
      const downloadCmd = `npx @larksuite/cli im +messages-resources-download --message-id ${message_id} --file-key ${fileKey} --type ${resourceType} --as bot --output ./downloaded_file.${resourceType === 'image' ? 'jpg' : 'bin'}`;
