  ...
  private async handleQuotedFileMessage(
    ...
    if (this.client) {
      try {
        ...
        // Issue #1637: Ensure file has correct extension based on headers or magic bytes
        const correctedPath = await ensureFileExtensionFromPath(localPath, response.headers as Record<string, unknown>);
