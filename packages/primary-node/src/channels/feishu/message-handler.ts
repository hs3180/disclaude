...
export class MessageHandler {
  ...
  async handleMessageReceive(data: FeishuEventData): Promise<void> {
    ...
    const isAlwaysMode = this.triggerModeManager.getTriggerMode(chat_id) === 'always';
    if (this.isGroupChat(chat_type) && !botMentioned && !isAlwaysMode && !isTriggerCommand) {
      logger.debug({ messageId: message_id, chatId: chat_id, chat_type }, 'Skipped group chat message without @mention (trigger mode)');
    ...
    // Get chat history context for mention-triggered messages in group chats
