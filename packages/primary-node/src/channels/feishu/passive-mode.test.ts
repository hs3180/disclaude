...
describe('TriggerModeManager', () => {
  ...
  describe('small group auto-detection (Issue #2052)', () => {
    ...
    it('should be a no-op when unmarking a non-small-group chat', () => {
      const manager = new TriggerModeManager();
      manager.unmarkSmallGroup('oc_never_was_small');
      expect(manager.isSmallGroup('oc_never_was_small')).toBe(false);
    });

    it('should remove unmarked group from getTriggerEnabledChats', () => {
      const manager = new TriggerModeManager();
      manager.markAsSmallGroup('oc_small');
