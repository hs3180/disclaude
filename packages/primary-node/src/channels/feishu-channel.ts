...
import { InteractionManager, WelcomeService, createFeishuClient, dissolveChat, addMembers as addMembersToChat, removeMembers as removeMembersFromChat, getMembers, getBotChats, GroupService, type BotChatInfo } from '../platforms/feishu/index.js';
import {
  PassiveModeManager,
  MentionDetector,
  WelcomeHandler,
...
export class FeishuChannel extends BaseChannel<FeishuChannelConfig> {
  ...
  async addMembers(chatId: string, memberIds: string[]): Promise<{ success: boolean }> {
    ...
    await addMembersToChat(this.client, chatId, memberIds);
    return { success: true };
  }
  ...
  async removeMembers(chatId: string, memberIds: string[]): Promise<{ success: boolean }> {
    ...
    await removeMembersFromChat(this.client, chatId, memberIds);
    return { success: true };
  }
   ...
   */
  async listMembers(chatId: string): Promise<{ success: boolean; members: string[] }> {
    if (!this.client) {
      throw new Error('Feishu client not initialized');
    }
   ...
   */
  async listChats(): Promise<{ success: boolean; chats: BotChatInfo[] }> {
    if (!this.client) {
      throw new Error('Feishu client not initialized');
    }
    const chats = await getBotChats(this.client);
