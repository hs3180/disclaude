...
export class FeishuChannel extends BaseChannel<FeishuChannelConfig> {
  ...
  // Delegate trigger mode methods to TriggerModeManager (Issue #2193: renamed from PassiveModeManager)
  getTriggerMode(chatId: string): 'mention' | 'always' {
    return this.triggerModeManager.getTriggerMode(chatId);
  }

  setTriggerMode(chatId: string, mode: 'mention' | 'always'): void {
    this.triggerModeManager.setTriggerMode(chatId, mode);
  }

  getAlwaysModeChats(): string[] {
    return this.triggerModeManager.getAlwaysModeChats();
  }

  /**
   * Get the TriggerModeManager instance.
   * Issue #2069/2193: Allows external initialization from persisted records.
   */
