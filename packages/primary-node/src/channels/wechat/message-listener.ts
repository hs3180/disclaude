...
/** Maximum entries in dedup set before eviction. */
const DEDUP_MAX_SIZE = 10_000;

/** Number of entries to evict when dedup set is full. */
 ...
 * - Message deduplication via seen message IDs with FIFO eviction
 * - Exponential backoff on consecutive errors with max retry threshold
...
export class WeChatMessageListener {
    ...
    while (this.abortController && !this.abortController.signal.aborted) {
      try {
        ...
        for (const update of updates) {
          // eslint-disable-next-line no-await-in-loop -- sequential processing required for dedup
        ...
        // eslint-disable-next-line no-await-in-loop -- backoff delay
  ...
  private async processUpdate(update: WeChatUpdate): Promise<void> {
    ...
    // FIFO eviction when set grows too large
    if (this.seenMessageIds.size > DEDUP_MAX_SIZE) {
      ...
      logger.debug(
        { remaining: this.seenMessageIds.size },
  ...
  private convertToIncomingMessage(update: WeChatUpdate): IncomingMessage | null {
    ...
    switch (firstItem.type) {
      case 1: {
        // Text message
        const textItem = firstItem as WeChatTextItem;
        messageType = 'text';
      ...
      case 2: {
        // Image message
        const imageItem = firstItem as WeChatImageItem;
        messageType = 'image';
