 ...
 * - Exponential backoff on consecutive errors (max 5 before extended backoff)
...
export class WeChatMessageListener {
  ...
  private async processUpdate(update: WeChatUpdate): Promise<void> {
    ...
    if (this.seenMessageIds.size > MAX_DEDUP_CACHE_SIZE) {
      ...
      while (evicted < DEDUP_EVICT_COUNT) {
        const { value, done } = iter.next();
        if (done) break;
        this.seenMessageIds.delete(value);
        evicted++;
      }
      logger.debug(
  ...
  private convertToIncomingMessage(update: WeChatUpdate): IncomingMessage | null {
    ...
    switch (firstItem.type) {
      ...
      default: {
        ...
        messageType = 'text';
        const fallbackType = (firstItem as { type: number }).type;
        ...
        content = textParts.join('\n') || `[Unsupported message type: ${fallbackType}]`;
        break;
      }
    }
