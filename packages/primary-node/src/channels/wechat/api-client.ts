...
import type { WeChatGetUpdatesResponse, WeChatTypingResponse, WeChatCdnUploadResponse } from './types.js';
...
export class WeChatApiClient {
  ...
  // ---------------------------------------------------------------------------
  // Media upload (Issue #1556 Phase 3.2 — Media Handling)
  // ---------------------------------------------------------------------------
  ...
  async uploadMedia(params: {
    ...
    delete headers['Content-Length']; // FormData sets its own Content-Length
    ...
    try {
      ...
      if (ret !== undefined && ret !== 0) {
        const errMsg = (data as unknown as Record<string, unknown>).err_msg as string || `Error code ${ret}`;
