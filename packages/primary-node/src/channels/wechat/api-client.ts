...
export class WeChatApiClient {
  ...
  async uploadMedia(params: {
    ...
    const uploadUrl = `${this.baseUrl}/ilink/bot/upload`;
    ...
    logger.trace({ fileName, size: fileData.length }, 'Uploading file to CDN');
    ...
    try {
      ...
      if (ret !== undefined && ret !== 0) {
        ...
        logger.error({ ret, errMsg }, 'CDN upload returned error');
   ...
   *
   * POST /ilink/bot/sendmessage with type 2 (image_item)
   *
   ...
   *
   * POST /ilink/bot/sendmessage with type 3 (file_item)
   *
