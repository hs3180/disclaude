...
export class WeChatApiClient {
   ...
   *
   * Uses multipart/form-data to upload binary file content.
   * Returns CDN URL and file key for use in sendImage/sendFile.
   *
  ...
  async uploadMedia(params: {
    ...
    // Remove Content-Length — set automatically by FormData boundary
    ...
    try {
      ...
      if (data.ret !== undefined && data.ret !== 0) {
        ...
        throw new Error(`WeChat upload error [${data.ret}]: ${errMsg}`);
      }
