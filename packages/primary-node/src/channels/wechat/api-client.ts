...
import { MAX_MEDIA_FILE_SIZE, IMAGE_EXTENSIONS, type WeChatGetUpdatesResponse, type WeChatMediaUploadResult } from './types.js';
...
export class WeChatApiClient {
   ...
   * @returns CDN upload result with URL
   */
  async uploadMedia(filePath: string): Promise<WeChatMediaUploadResult> {
    const fileStat = await stat(filePath);
    if (fileStat.size > MAX_MEDIA_FILE_SIZE) {
      throw new Error(
        `File too large: ${fileStat.size} bytes exceeds ${MAX_MEDIA_FILE_SIZE} bytes (20 MB limit)`,
      );
    }
    ...
    const mimeType = isImage ? `image/${extension === '.jpg' ? 'jpeg' : extension.slice(1)}` : 'application/octet-stream';
    ...
    // Override Content-Type for multipart/form-data (browser sets boundary)
    delete headers['Content-Type'];

    logger.debug({ filePath, fileName, size: fileStat.size, isImage }, 'Uploading media');

    const data = await this.fetchJson<WeChatMediaUploadResult>(
      url,
      {
        method: 'POST',
        headers,
    ...
    if (!data.url) {
      throw new Error('Media upload succeeded but no URL returned');
    }
   ...
   *
   * Uses the same sendmessage endpoint with an image item.
   *
  ...
  async sendImage(params: {
    ...
    await this.postJson('ilink/bot/sendmessage', body);
    logger.debug({ to }, 'Image message sent');
  }

  /**
   * Send a file message.
   *
   * Uses the same sendmessage endpoint with a file item.
   *
  ...
  async sendFile(params: {
    ...
    const { to, fileUrl, fileName, fileSize, contextToken } = params;
    const clientId = this.generateClientId();
    ...
    await this.postJson('ilink/bot/sendmessage', body);
    logger.debug({ to, fileName }, 'File message sent');
  }
