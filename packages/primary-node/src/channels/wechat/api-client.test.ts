...
describe('WeChatApiClient', () => {
  ...
  describe('sendImage (Issue #1556 Phase 3.3)', () => {
    it('should send image message via POST with image_item', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        text: () => Promise.resolve(JSON.stringify({ ret: 0 })),
      });
  ...
  describe('sendFile (Issue #1556 Phase 3.3)', () => {
    it('should send file message via POST with file_item', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        text: () => Promise.resolve(JSON.stringify({ ret: 0 })),
      });
  ...
  describe('uploadMedia (Issue #1556 Phase 3.3)', () => {
    ...
    it('should throw on API error ret code', async () => {
      ...
      await expect(client.uploadMedia({
        fileData: Buffer.from('data'),
        fileName: 'test.txt',
      })).rejects.toThrow('WeChat upload error [2001]: Upload rejected');
    });
