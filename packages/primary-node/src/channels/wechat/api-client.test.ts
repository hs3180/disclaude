...
describe('WeChatApiClient', () => {
  ...
  describe('sendImage (Issue #1556 Phase 3.3)', () => {
    it('should send image message via POST with CDN URL', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        text: () => Promise.resolve(JSON.stringify({ ret: 0 })),
      });
    ...
    it('should include contextToken when provided', async () => {
      ...
      const callBody = JSON.parse(mockFetch.mock.calls[0][1].body);
      expect(callBody.msg.context_token).toBe('ctx-abc');
    });
  });

  describe('sendFile (Issue #1556 Phase 3.3)', () => {
    it('should send file message via POST with CDN URL', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        text: () => Promise.resolve(JSON.stringify({ ret: 0 })),
      });
  ...
  describe('uploadMedia (Issue #1556 Phase 3.3)', () => {
    it('should upload file and return CDN URL and file key', async () => {
      ...
      // Verify FormData upload was sent
      expect(mockFetch).toHaveBeenCalledWith(
      ...
      const callOpts = calls[0][1] as Record<string, unknown>;
    ...
    it('should throw when response is missing url or file_key', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        text: () => Promise.resolve(JSON.stringify({ ret: 0 })),
      });
    ...
    it('should use application/octet-stream when mimeType is not provided', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        text: () => Promise.resolve(JSON.stringify({
          ret: 0,
      ...
      expect(result.url).toBe('https://cdn.example.com/file.bin');
    });
  });
});
