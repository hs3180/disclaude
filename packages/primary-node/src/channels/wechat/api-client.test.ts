...
describe('WeChatApiClient', () => {
  ...
  describe('sendTyping (Issue #1556 Phase 3.2)', () => {
    ...
    it('should use short timeout (5s)', async () => {
      ...
      // Verify the request was made (timeout is internal via AbortController)
      expect(mockFetch).toHaveBeenCalledTimes(1);
    });
    ...
    it('should include auth headers', async () => {
      ...
      client.setToken('bot-token-abc');
      await client.sendTyping({ to: 'user-456' });

      const callHeaders = mockFetch.mock.calls[0][1].headers;
      expect(callHeaders).toHaveProperty('Authorization', 'Bearer bot-token-abc');
