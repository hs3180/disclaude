...
describe('WeChatMessageListener', () => {
  ...
  describe('edge cases', () => {
    ...
    it('should handle empty text_item text', async () => {
      let capturedMessage: IncomingMessage | undefined;
      const processor = vi.fn().mockImplementation(async (msg: IncomingMessage) => {
        capturedMessage = msg;
      });
    ...
    it('should use Date.now() as fallback timestamp', async () => {
      let capturedMessage: IncomingMessage | undefined;
      const processor = vi.fn().mockImplementation(async (msg: IncomingMessage) => {
        capturedMessage = msg;
      });
  ...
  describe('error handling', () => {
    it('should catch processor errors without crashing', async () => {
    ...
    it('should apply exponential backoff on consecutive errors', async () => {
      ...
      mockClient.getUpdates = vi.fn().mockImplementation(async () => {
        callCount++;
        // First call throws error, second call aborts (simulating shutdown during backoff)
        if (callCount === 1) {
      ...
      listener.start();
      // Wait enough for first error + backoff start, then stop
      await new Promise((r) => setTimeout(r, 100));
      await listener.stop();

      // Should have made at least 1 call and logged the error
