 ...
 * - Media handling via CDN upload (Issue #1556 Phase 3.3)
 *
...
/** Image file extensions that should be sent as image messages (type 2). */
...
export class WeChatChannel extends BaseChannel<WeChatChannelConfig> {
   ...
   *
   * Supports 'text', 'card' (downgraded to JSON text), and 'file' (via CDN upload) types.
  ...
  protected async doSendMessage(message: OutgoingMessage): Promise<string | void> {
    ...
    // File/image sending via CDN upload (Issue #1556 Phase 3.3)
    ...
    // Unsupported message types (including 'done')
    logger.warn(
   ...
   *
   * Issue #1556 Phase 3.3: supportsFile now true (via CDN upload).
   */
