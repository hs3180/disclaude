 ...
 * - Typing indicator support (Issue #1556)
 * - Media/image/file sending via CDN upload (Issue #1556)
 * - Thread support via context_token (Issue #1556)
 *
 ...
 * WeChat Channel.
 *
 * Provides WeChat (Tencent ilink) bot integration with full-duplex messaging:
 * - QR code authentication on start
 * - Text, image, and file message sending
 * - Long-poll message listening
 * - Typing indicator
 * - Thread support via context_token
 *
...
export class WeChatChannel extends BaseChannel<WeChatChannelConfig> {
   ...
   * - supportsThread: true (via context_token)
