...
// Fix #2: Use 'RestChannel.Session' as logger name so existing log filters
// that match on 'RestChannel' still capture cleanup logs.
...
export class RestSessionManager {
   ...
   */
  start(): void {
    // Fix #3: Guard against double-interval leak on repeated calls.
   ...
   * Fix #1: Use has() instead of get() for existence check — consistent
   * with the addMessage call-site pattern and avoids an unnecessary lookup.
   */
