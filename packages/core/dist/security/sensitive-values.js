...
const MASK = '[REDACTED]';
export function protectSensitiveValues(values) {
    ...
    for (const value of unique) {
        declarations.set(value, (declarations.get(value) ?? 0) + 1);
    }
    let disposed = false;
    return () => {
        if (disposed) {
        ...
        for (const value of unique) {
            ...
            if (remaining) {
                declarations.set(value, remaining);
            }
            else {
                declarations.delete(value);
            }
        }
        unique.length = 0;
    };
}
function replaceDeclared(text, values) {
    const output = [];
...
export function redactDeclaredSensitive(value, ancestors = new WeakSet()) {
    if (typeof value === 'string') {
        return replaceDeclared(value, [...declarations.keys()]);
    }
    ...
    try {
        if (Array.isArray(value)) {
            return value.map(item => redactDeclaredSensitive(item, ancestors));
        }
        const output = Object.create(null);
        for (const [key, descriptor] of Object.entries(Object.getOwnPropertyDescriptors(value))) {
            if (!descriptor.enumerable && !(value instanceof Error)) {
                continue;
            }
...
export class SensitiveOutputFilter {
    ...
    write(chunk) { this.pending += chunk; this.drain(false); }
    ...
    drain(final) {
        while (this.pending) {
            ...
            for (const value of this.values) {
                ...
                if (index >= 0 && index < safeEnd && (offset < 0 || index < offset)) {
                    offset = index;
                    match = value;
                }
            }
