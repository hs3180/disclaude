...
export class SessionTimeoutManager {
    ...
    runningPromise = null;
    disposed = false;
    constructor(config, callbacks, logger) {
        this.config = {
            enabled: true,
    ...
    start() {
        ...
        this.logger.info({ idleMinutes: this.config.idleMinutes, maxSessions: this.config.maxSessions, checkIntervalMinutes: this.config.checkIntervalMinutes }, 'Session timeout manager started');
    }
    /**
    ...
    executeCheck() {
        ...
        const remainingSessions = allSessions.filter((id) => !closedInPhase1.has(id) && !result.processingSkipped.includes(id));
        ...
        if (result.idleClosed.length > 0 || result.evicted.length > 0) {
            this.logger.info({ idleClosed: result.idleClosed.length, evicted: result.evicted.length, processingSkipped: result.processingSkipped.length }, 'Timeout check completed with closures');
        }
        return Promise.resolve(result);
    }
    /**
     ...
     */
    closeWithLog(chatId, reason) {
        const ctx = { chatId, reason };
