...
import { DEFAULT_TIMEZONE } from './scheduled-task.js';
...
export const defaultCommandRunner = (command, options) => {
    ...
    if (options.signal.aborted) {
        return Promise.reject(new CommandCancelledError());
    }
    return new Promise((resolve, reject) => {
        ...
        const awaitKilledGroup = (deadline) => {
...
export class Scheduler {
     ...
     */
    runningBlockingTaskChatIds = new Set();
    /**
     ...
     */
    consecutiveTaskFailures = new Map();
     ...
     */
    static GRACEFUL_SHUTDOWN_TIMEOUT_MS = 5000;
    /**
     * Stop the scheduler.
    ...
    async stop(timeoutMs) {
        ...
        if (this._drainPromise && waitTimeout > 0) {
            logger.info({ taskIds: Array.from(this.runningTasks), timeoutMs: waitTimeout }, 'Waiting for running tasks to complete...');
            const timeoutPromise = new Promise((resolve) => {
                setTimeout(() => {
                    if (this.runningTasks.size > 0) {
                        logger.warn({ taskIds: Array.from(this.runningTasks) }, 'Graceful shutdown timed out, abandoning running tasks');
                    }
                    resolve();
                }, waitTimeout);
            });
    ...
    addTask(task) {
        ...
        try {
            ...
            logger.error({ err: error, taskId: task.id, cron: task.cron, timezone: task.timezone || DEFAULT_TIMEZONE }, isCronError ? 'Invalid cron expression' : 'Failed to schedule task (check cron expression and timezone)');
        }
    }
    /**
...
${task.prompt ?? ''}`;
     ...
     */
    async clearFailureStreak(taskId) {
    ...
    async executeTask(task) {
        ...
        if (task.cooldownPeriod && this.cooldownManager) {
            ...
            if (isInCooldown) {
                ...
                logger.info({ taskId: task.id, name: task.name, remainingMinutes }, 'Task skipped - in cooldown period');
                // Send cooldown notification
                await this.callbacks.sendMessage(task.chatId, `⏰ 定时任务「${task.name}」冷静期中，跳过执行\n` +
        ...
        if (task.blocking && (this.runningTasks.has(task.id) || this.isolatedBlockingTurns.has(task.id))) {
            logger.info({ taskId: task.id, name: task.name }, 'Task skipped - previous execution still running');
            return;
        }
        ...
        if (task.blocking && task.chatId && (this.runningBlockingTaskChatIds.has(task.chatId) || [...this.isolatedBlockingTurns.values()].includes(task.chatId))) {
            logger.info({ taskId: task.id, name: task.name, chatId: task.chatId }, 'Task skipped - another blocking scheduled task is running for this chatId');
            return;
        }
        ...
        if (task.chatId && this.callbacks.isChatBusy?.(task.chatId)) {
            logger.info({ taskId: task.id, name: task.name, chatId: task.chatId }, 'Task skipped - chat is busy (agent is processing a message)');
            return;
        }
        ...
        try {
            ...
            if (!currentTask) {
                logger.info({ taskId: task.id, name: task.name }, 'Task file no longer exists, removing stale cron job');
            ...
            logger.error({ err: error, taskId: task.id }, 'Failed to verify schedule file existence, skipping execution');
            this.cleanupTaskTracking(task);
            this.removeTask(task.id);
            this.resolveDrainIfNeeded();
            return;
        ...
        try {
            ...
            if (task.command) {
                ...
                const controllers = this.activeCommandControllers.get(task.id) ?? new Set();
                ...
                try {
                    ...
                    const result = await this.commandRunner(task.command, {
            ...
            if (!this.inputMessageRouter || !task.chatId || !task.prompt) {
                logger.warn({ taskId: task.id, hasRouter: !!this.inputMessageRouter, hasChatId: !!task.chatId }, 'Cannot execute scheduled task: InputMessageRouter not configured or task has no chatId');
                await this.callbacks.sendMessage(task.chatId, `⚠️ 定时任务「${task.name}」无法执行: InputMessageRouter 未配置或任务缺少 chatId`);
                return;
            }
            ...
            await this.callbacks.sendMessage(task.chatId, `⏰ 定时任务「${task.name}」开始执行...`);
            {
                const systemMessage = {
                ...
                try {
                    ...
                    if (freshSession && task.blocking) {
                        ...
                        void turn.then(() => { this.isolatedBlockingTurns.delete(task.id); }, () => { this.isolatedBlockingTurns.delete(task.id); });
                    }
                    await Promise.race([
                        turn,
                        timeoutPromise,
            ...
            if (error instanceof TurnSupersededError) {
                ...
                logger.info(outcomeContext, 'Scheduled task turn superseded by a newer message (neutral — not counted as failure)');
                return;
            }
            ...
            if (error instanceof TaskTimeoutError) {
                ...
                logger.warn({ ...outcomeContext, timeoutMs: error.timeoutMs }, 'Scheduled task timed out waiting for the agent turn (turn not cancelled, outcome unknown — not counted as failure)');
                ...
                try {
                    await this.callbacks.sendMessage(task.chatId, `⏱️ 定时任务「${task.name}」执行超时 (${formatTimeout(error.timeoutMs)})，已停止等待` +
                        '（agent 轮次可能仍在后台继续）。若该任务确需更长运行时间，请在 SCHEDULE.md 设置 timeoutMs。');
                }
                catch (notifyErr) {
                    logger.warn({ err: notifyErr, taskId: task.id, chatId: task.chatId }, 'Failed to send timeout notification for scheduled task');
                }
                return;
            }
            ...
            logger.error({ err: error, ...failureContext }, 'Scheduled task failed (agent turn ended with an error)');
            if (consecutiveFailures >= CONSECUTIVE_FAILURE_ALERT_THRESHOLD) {
                ...
                logger.error({ ...failureContext, scheduleConsecutiveFailures: consecutiveFailures }, `Scheduled task has failed ${consecutiveFailures} consecutive runs — ` +
            ...
            // best-effort.
            try {
                await this.callbacks.sendMessage(task.chatId, `❌ 定时任务「${task.name}」执行失败: ${errorMessage}`);
            }
            catch (notifyErr) {
                logger.warn({ err: notifyErr, taskId: task.id, chatId: task.chatId }, 'Failed to send failure notification for scheduled task');
            }
        }
        finally {
     ...
     */
    async getCooldownStatus(taskId, cooldownPeriod) {
