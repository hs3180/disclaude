...
import { listChannels, resolveChannelsDir, getChannel, } from './channel-directory.js';
...
export class ChannelLoader {
    ...
    load() {
        ...
        for (const entry of result.channels) {
            ...
            if (!entry.valid) {
                logger.warn({ channelId: entry.manifest.id, error: entry.error }, 'Channel has invalid config');
                entries.push(entry);
                continue;
            }
            if (this.skipDisabled && !entry.manifest.enabled) {
                logger.debug({ channelId: entry.manifest.id }, 'Skipping disabled channel');
                continue;
            }
            entries.push(entry);
            logger.info({ channelId: entry.manifest.id, module: entry.manifest.module }, 'Dynamic channel discovered');
        }
        logger.info({ total: result.total, loaded: entries.length, enabled: result.enabled, disabled: result.disabled }, 'Channel loading complete');
        return entries;
    }
    /**
