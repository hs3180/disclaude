...
export class ChannelRegistry {
    ...
    register(descriptor) {
        if (this.descriptors.has(descriptor.type)) {
            throw new ChannelRegistryError(`Channel type "${descriptor.type}" is already registered. Use has() to check before registering.`);
        }
    ...
    create(type, config = {}) {
        ...
        if (!descriptor) {
            ...
            throw new ChannelRegistryError(`Unknown channel type "${type}". Available types: [${available}]`);
        }
