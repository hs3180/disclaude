...
export class BaseChannel extends EventEmitter {
     ...
     */
    constructor(config, defaultId, name) {
        super();
        this.config = config;
    ...
    async sendMessage(message) {
        ...
        try {
            ...
            logger.debug({ id: this.id, chatId: message.chatId, type: message.type, messageId }, 'Message sent');
            return messageId;
        }
        catch (error) {
            logger.error({ err: error, id: this.id, chatId: message.chatId }, 'Failed to send message');
            throw error;
        }
    }
    /**
    ...
    async emitMessage(message) {
        if (this.messageHandler) {
            ...
            logger.warn({ id: this.id, messageId: message.messageId }, 'No message handler registered');
        }
    }
    /**
