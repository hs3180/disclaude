...
import { SAFE_CHANNEL_ID_PATTERN, RESERVED_CHANNEL_IDS, } from '../types/channel-plugin.js';
 ...
 */
export function resolveChannelsDir(baseDir) {
 ...
 */
export function resolveChannelDir(channelId, baseDir) {
 ...
 */
export function resolveChannelConfigPath(channelId, baseDir) {
 ...
 */
export function parseChannelConfig(configPath) {
 ...
 * @returns YAML string
 */
export function serializeChannelConfig(manifest) {
    const data = {
        id: manifest.id,
        name: manifest.name,
 ...
 * Uses atomic mkdir (without recursive) to prevent TOCTOU race conditions.
 *
 ...
 */
export function addChannel(channelId, module, options, baseDir) {
    ...
    // Create channel directory atomically (no recursive — throws EEXIST if already exists)
    try {
    ...
    // Build manifest — name defaults to channelId, NOT to description
    const manifest = {
        id: channelId,
 ...
 */
export function removeChannel(channelId, baseDir) {
    validateChannelId(channelId);
    const channelDir = resolveChannelDir(channelId, baseDir);
 ...
 * @param baseDir - Base directory (defaults to process.cwd())
 */
export function setChannelEnabled(channelId, enabled, baseDir) {
    validateChannelId(channelId);
    const configPath = resolveChannelConfigPath(channelId, baseDir);
    if (!fs.existsSync(configPath)) {
 ...
 */
export function getChannel(channelId, baseDir) {
    validateChannelId(channelId);
    const channelDir = resolveChannelDir(channelId, baseDir);
    const configPath = resolveChannelConfigPath(channelId, baseDir);
    if (!fs.existsSync(configPath)) {
    ...
    try {
        const manifest = parseChannelConfig(configPath);
        // Validate YAML id matches directory name
        if (manifest.id !== channelId) {
            return {
                manifest,
                directoryPath: channelDir,
                configFilePath: configPath,
                valid: false,
                error: `Channel ID "${manifest.id}" in channel.yaml does not match directory name "${channelId}"`,
            };
        }
        return {
            manifest,
 ...
 */
export function listChannels(baseDir) {
    const channelsDir = resolveChannelsDir(baseDir);
    ...
    for (const item of items) {
        ...
        // Validate directory name as a channel ID
        try {
            validateChannelId(item.name);
        }
        catch {
            continue; // Skip directories with invalid channel IDs
        }
