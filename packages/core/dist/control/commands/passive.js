 ...
 */
function getMessageForMode(mode, messages) {
    switch (mode) {
 ...
 */
function handleModeToggle(command, context, commandName, messages) {
    // Issue #2291: Use triggerMode (enum-based interface)
    const modeManager = context.triggerMode;
    if (!modeManager) {
        context.logger?.warn({ chatId: command.chatId }, `/${commandName} command received but triggerMode is not configured`);
        return {
            success: false,
    ...
    const modeOrder = ['auto', 'mention', 'always'];
 ...
 */
export const handleTrigger = (command, context) => handleModeToggle(command, context, 'trigger', TRIGGER_MESSAGES);
