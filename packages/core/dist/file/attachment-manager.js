 ...
 *
 * Migrated to @disclaude/core (Issue #1041)
 */
/**
 * Attachment manager for in-memory storage of pending files.
 *
 * Implements the IAttachmentManager interface from types/adapter.ts
...
export class AttachmentManager {
     ...
     */
    formatAttachmentsForPrompt(chatId) {
     ...
     */
    cleanupOldAttachments(maxAgeMs = 60 * 60 * 1000) {
        const now = Date.now();
        const chatsToClean = [];
