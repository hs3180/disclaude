 ...
 */
function resolveMirror(configMirror) {
 ...
 */
function resolveRotate(configRotate) {
 ...
 */
async function buildFileDestination(logDir, rotate) {
    try {
        ...
        if (rotate) {
            ...
            try {
                ...
                console.warn('Log rotation symlink failed, retrying without it (filebeat still globs the numbered files):', redactSensitive(symlinkError));
 ...
 */
function setupSyncFilePassthrough() {
 ...
 */
function createRedactionSerializer(fields = SENSITIVE_FIELDS) {
...
export async function initLogger(config = {}) {
    ...
    let options = isDev ? getDevelopmentConfig() : getProductionConfig();
    ...
    let passthrough = logPassthrough;
    if (!passthrough) {
...
export function flushLogger() {
    ...
    return new Promise((resolve) => {
        ...
        const dests = new Set([activeFileDest, currentLogDest].filter(Boolean));
...
export async function closeLogger() {
    ...
    const fileTargets = passthroughTargets.filter((target) => target !== process.stdout && target !== process.stderr);
