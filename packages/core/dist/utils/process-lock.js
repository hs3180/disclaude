...
export class ProcessLock {
    ...
    acquire() {
        if (fs.existsSync(this.lockfilePath)) {
            ...
            if (existingPid !== null && this.isProcessRunning(existingPid)) {
                this.logger.error({ pid: existingPid, lockfile: this.lockfilePath }, `Another instance is already running (PID ${existingPid}). Exiting to prevent duplicate.`);
                return false;
            }
            // Stale PID file — old process is dead, safe to remove
            this.logger.warn({ stalePid: existingPid, lockfile: this.lockfilePath }, 'Stale PID file found (process no longer running). Removing stale lock.');
            try {
                fs.unlinkSync(this.lockfilePath);
            }
            catch {
                this.logger.warn({ lockfile: this.lockfilePath }, 'Failed to remove stale PID file, will attempt to overwrite');
            }
        }
        // Write our PID
        ...
        this.acquired = true;
        this.logger.info({ pid: process.pid, lockfile: this.lockfilePath }, 'Process lock acquired');
        return true;
    }
    /**
        ...
        try {
            if (fs.existsSync(this.lockfilePath)) {
                ...
                if (filePid === process.pid) {
                    fs.unlinkSync(this.lockfilePath);
                    this.logger.info({ lockfile: this.lockfilePath }, 'Process lock released');
                }
                else {
                    this.logger.warn({ filePid, ourPid: process.pid, lockfile: this.lockfilePath }, 'PID file belongs to another process, not removing');
                }
            }
        }
        catch (err) {
            this.logger.warn({ err, lockfile: this.lockfilePath }, 'Failed to release process lock during shutdown');
        }
        finally {
            this.acquired = false;
        }
    ...
    isDisclaudeProcess(pid) {
        try {
            ...
            this.logger.warn({ pid, cmdline: cmdStr.substring(0, 200) }, 'PID file references a non-Node.js process (PID recycled). Treating as stale.');
            return false;
        }
        catch {
