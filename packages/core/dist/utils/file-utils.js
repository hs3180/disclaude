 ...
 * ```
 */
export function ensureFileExtension(filePath, buffer) {
 ...
 * ```
 */
export async function ensureFileExtensionFromPath(filePath, headers) {
    // If file already has a known extension, return as-is
    const currentExt = path.extname(filePath).toLowerCase();
    if (currentExt && KNOWN_EXTENSIONS.has(currentExt)) {
        return filePath;
 ...
 */
async function renameWithExtension(filePath, ext) {
