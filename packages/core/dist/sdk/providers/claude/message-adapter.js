 ...
 */
export function adaptSDKMessage(message, taskRegistry) {
    const metadata = {};
    // 提取 session_id
    ...
    switch (message.type) {
        case 'assistant': {
            ...
            const toolBlocks = content.filter((block) => block.type === 'tool_use' && 'name' in block && 'input' in block);
            // 提取文本块 — block.type === 'text' 时 TypeScript 自动收窄为 BetaTextBlock
            const textBlocks = content.filter((block) => block.type === 'text' && 'text' in block);
            // 构建内容
            const contentParts = [];
            // 处理工具使用
            if (toolBlocks.length > 0) {
                ...
                contentParts.push(formatToolInput(block.name, block.input, taskRegistry));
            }
            // 处理文本
            ...
            if (message.subtype === 'success') {
                ...
                if ('usage' in message && message.usage) {
                    ...
                    const costUsd = message.total_cost_usd
                        ?? ('total_cost' in usage ? usage.total_cost : undefined);
 ...
 */
export function adaptUserInput(input) {
 ...
 */
function adaptMaxTerminationResult(subtype) {
    switch (subtype) {
 ...
 * 格式化工具输入用于显示
 */
function formatToolInput(toolName, input, taskRegistry) {
    if (!input) {
        return `🔧 ${toolName}`;
    }
    switch (toolName) {
