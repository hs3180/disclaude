...
const EMPTY_STREAM_TERMINATE_NOTICE = '❌ 上游返回了空响应（200 但零内容事件），本次会话未产生任何输出。请稍后重试。';
// ============================================================================
 ...
 */
export function stderrIndicatesUpstreamApiError(stderr) {
 ...
 */
export const SDK_PROCESS_EVENTS = ['exit', 'SIGINT', 'SIGTERM'];
 ...
 */
export function cleanupNewProcessListeners(snapshot) {
    let cleaned = 0;
    for (const event of SDK_PROCESS_EVENTS) {
 ...
 */
export function attachStderrToError(error, stderr) {
    if (error instanceof Error) {
        error[STDERR_SYMBOL] = stderr;
    }
}
/**
 ...
 */
export function getErrorStderr(error) {
    if (error instanceof Error) {
        return error[STDERR_SYMBOL];
    }
    return undefined;
}
/**
 ...
 */
export function isStartupFailure(messageCount, elapsedMs) {
...
export class ClaudeSDKProvider {
    ...
    queryStream(input, options) {
        ...
        if (options.autoCompactWindow === 'auto') {
            return withDiscoveredCompaction(input, options, (nextInput, nextOptions) => this.queryStream(nextInput, nextOptions));
        }
        ...
        sdkOptions.stderr = (data) => stderrFilter.write(data);
        // 创建输入适配器生成器
        ...
        async function* adaptInputStream() {
        ...
        async function* adaptIterator() {
            ...
            let blindWindowTimer = null;
            const clearBlindWindow = () => {
                if (blindWindowTimer) {
                    clearTimeout(blindWindowTimer);
            ...
            const clearContentWatchdog = () => {
                if (contentWatchdog) {
                    clearTimeout(contentWatchdog);
            ...
            const fireWatchdog = () => {
                ...
                stalled = true;
                logger.error({ messageCount, model: options.model, stallTimeoutMs: STALL_TIMEOUT_MS, apiBaseUrl: options.env?.ANTHROPIC_BASE_URL }, `GLM stall: no content_block_delta for ${STALL_TIMEOUT_MS}ms during in-flight request; interrupting (Issue #3706)`);
                queryResult.interrupt().catch((e) => {
                ...
                forceCloseTimer = armTimer(() => {
                    forceCloseTimer = null;
                    const maybeClose = queryResult.close;
                    if (typeof maybeClose === 'function') {
                        try {
                            maybeClose.call(queryResult);
                            logger.warn({ graceMs: STALL_FORCE_CLOSE_GRACE_MS }, 'stall watchdog: stream did not end within grace after interrupt(); force-closed query');
                        }
                        catch (e) {
            ...
            for (let queryAttempt = 0;; queryAttempt++) {
                ...
                try {
                    ...
                    const envFloodThreshold = Number.parseInt(process.env.DISCLAUDE_SYSTEM_FLOOD_THRESHOLD ?? '', 10);
                    ...
                    for await (const message of queryResult) {
                        ...
                        if (!firstMessageMs) {
                            firstMessageMs = now;
                            logger.info({ messageCount, messageType: message.type, ttftMs: now - queryStartMs }, 'SDK first message received (TTFT)');
                        }
                        else if (message.type === 'assistant') {
                            // Log timing for significant messages (not every system message)
                            logger.info({ messageCount, messageType: message.type, elapsedMs: now - queryStartMs }, 'SDK message received');
                        }
                        else {
                    ...
                    if (!partialsObserved && messageCount > 0) {
                        logger.error({ messageCount, model: options.model, apiBaseUrl: options.env?.ANTHROPIC_BASE_URL }, 'Issue #3706: stream_event partials never observed this turn — no-content-progress '
                    ...
                    logger.info({ totalMs, messageCount, ttftMs: firstMessageMs ? firstMessageMs - queryStartMs : undefined }, 'SDK iterator completed');
                    ...
                    if (errorTransient && messageCount === 0 && queryAttempt < MAX_QUERY_RETRIES) {
                        ...
                        logger.warn({ queryAttempt: queryAttempt + 1, err: error, errorCategory }, 'Transient error before first SDK message — retrying query (Issue #4192 L1)');
                        await delayMs(computeBackoffDelay(queryAttempt, QUERY_RETRY_TIMING));
                        continue;
                    }
                    logger.error({
            ...
            handle: {
                ...
                cancel: () => {
                    ...
                    if (typeof queryResult.interrupt === 'function') {
                        void queryResult.interrupt().catch((err) => {
    ...
    createInlineTool(definition) {
        return tool(definition.name, definition.description, definition.parameters, 
    ...
    createMcpServer(config) {
        if (config.type === 'inline') {
            const tools = (config.tools?.map(t => this.createInlineTool(t)) ?? []);
