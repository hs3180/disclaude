...
function decideDeny(name, deny) {
    if (deny.has(name)) {
        return {
            block: true,
            reason: `Tool "${name}" is disallowed by disclaude's permission policy ` +
 ...
 */
export function createPiToolPermissionGate(options) {
    const deny = new Set(options.disallowedTools ?? []);
    if (deny.size === 0) {
        return null;
    }
    return (context, signal) => Promise.resolve().then(() => {
        if (signal?.aborted) {
