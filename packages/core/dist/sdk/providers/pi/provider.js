...
export class PiAgentProvider {
    ...
    queryStream(input, options) {
        ...
        const adaptIterator = async function* () {
            ...
            agent = new Agent({
                ...
                ...(toolPermissionGate
                    ? { beforeToolCall: toolPermissionGate }
                    : {}),
            });
            const piAgent = agent;
            if (cancelRequested) {
    ...
    createMcpServer(config) {
        ...
        throw new Error('stdio MCP servers are not supported by PiAgentProvider.createMcpServer');
    }
    // --------------------------------------------------------------------------
    // Lifecycle
    // --------------------------------------------------------------------------
 ...
 */
function collectInlineTools(options) {
    const tools = [];
    for (const server of Object.values(options.mcpServers ?? {})) {
        ...
        else if (isAdaptedToolHandle(server.tools)) {
 ...
 */
function isAdaptedToolHandle(tools) {
