...
export class CodexAgentProvider {
    ...
    queryStream(input, options) {
        ...
        const thisBuiltinContext = options.cwd
            ? formatCodexBuiltinContext(mergeBuiltinResources(discoverBuiltinResources(resolve(options.cwd)), 
            ...
            discoverBuiltinResources(join(resolve(options.cwd), '.claude')), discoverBuiltinResources(this.builtinRoot)))
            : '';
        ...
        const timeoutLabel = this.execTimeoutMs && this.execTimeoutMs > 0
            ...
            const runInput = async (prompt, correlation) => {
                ...
                try {
                    ...
                    if (authFailed) {
                        ...
                        pushSynthetic({
                            type: 'error',
                            content: `codex exec timed out after ${timeoutLabel} and was killed — ` +
    ...
    queryAppServer(input, options, sandbox, binary) {
        let lifecycle;
            ...
            try {
                ...
                while (!stopped) {
                    ...
                    try {
                        ...
                        lifecycle = this.createAppServerLifecycle(binary, sessionKey, { sensitiveValues: options.sensitiveValues, correlation: next.value.correlation });
    ...
    createAppServerLifecycle(binary, sessionKey, diagnostics) {
