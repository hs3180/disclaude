...
import { adaptCodexEvent, classifyCodexEvent, isCodexAuthFailure, isCodexResumeTargetMissing, isCodexUsageLimit, userInputText, } from './exec-adapter.js';
 ...
 */
const REAUTH_NOTICE = '🔴 Codex 登录已失效（401 / 令牌过期）——请在运行 disclaude 的机器上执行 `codex login` 重新完成 Sign in with ChatGPT 授权，然后重发消息即可（当前消息未被处理）。';
/**
 ...
 */
const RESUME_TARGET_GONE_NOTICE = '⚠️ Codex 会话记录已不存在（可能被清理），已自动切换新会话——请重发你的问题，将从全新上下文开始（无此前对话内容）。';
/**
 ...
 */
const USAGE_LIMIT_NOTICE = '📊 Codex 用量已达上限（ChatGPT 订阅的 5 小时/周滚动窗口限额）。无需重启——窗口重置后直接重发消息即可自动恢复，当前对话上下文会保留。';
/** Actionable binary-missing error (thrown synchronously by queryStream). */
const BINARY_MISSING = (pathValue) => `CodexAgentProvider: codex CLI binary not found on PATH "${pathValue}" — install it first: ` +
...
function codexModelForChatGpt(model) {
...
export class CodexAgentProvider {
    name = 'codex';
    version = '0.6.0-forget-session';
    env;
    sandboxOverride;
    fullAccess;
    networkAccess;
    maxResumeInputTokens;
    builtinRoot;
    transportMode;
    appServerRoutes = new Map();
    appServerLifecycles = new Map();
    appServerStops = new Map();
    appServerThreadIds = new Map();
     ...
     */
    threadStash = new Map();
    skillsRegistries = new Map();
    reportedSkillRevisions = new WeakMap();
    /** Anonymous-stream key counter (queryStream without options.sessionKey). */
    anonSessionCounter = 0;
    disposed = false;
    constructor(options = {}) {
    ...
    skillsManifestFor(projectRoot, executionRoot = projectRoot) {
        if (!projectRoot || !executionRoot) {
            return '';
        }
    ...
    getInfo() {
        ...
        if (!this.findCodexBinary()) {
            problems.push('codex CLI binary not found on PATH — install it first: `npm install -g @openai/codex` (https://developers.openai.com/codex/cli)');
        }
        if (!this.hasAuth()) {
            problems.push('Codex auth missing (OAuth not completed) — run `codex login` (Sign in with ChatGPT); set CODEX_HOME if it is installed elsewhere');
        }
        const info = {
            name: this.name,
            version: this.version,
    ...
    forgetSession(sessionKey) {
        ...
        if (hadRegistration || hadStash) {
            logger.info({ sessionKey, hadRegistration, hadStash }, 'codex session forgotten on reset — governor registration + evicted-thread stash cleared (Issue #4644)');
        }
    }
    // --------------------------------------------------------------------------
    ...
    queryStream(input, options) {
        ...
        // binary check above.
        const sandboxDecision = resolveCodexSandboxPolicy(options, this.sandboxOverride, this.fullAccess);
        logger.info({
            sandbox: sandboxDecision.sandbox,
            permissionMode: sandboxDecision.sandbox === 'danger-full-access' ? 'full-access' : 'sandboxed',
        ...
        if (options.model && codexModel === undefined) {
            logger.warn({ configuredModel: options.model }, 'ignoring legacy gpt-5.1-codex model for ChatGPT-backed Codex; using the CLI default');
        }
        ...
        if (registration.evictedKey) {
            logger.info({ evictedKey: registration.evictedKey, sessionKey }, 'codex session cap reached — evicted the idlest session (LRU, #4634)');
        }
        ...
        const adaptIterator = async function* () {
            ...
            const fireStallWatchdog = () => {
                ...
                stalled = true;
                logger.error({ stallTimeoutMs: STALL_TIMEOUT_MS }, `codex stall: no exec events for ${STALL_TIMEOUT_MS}ms during an active run; ` +
            ...
            const enqueue = (event) => {
                ...
                if (event.type === 'turn.failed') {
                    ...
                    runFailureText += `\n${turnFailure}`;
                    logger.warn({ threadId: latestSessionId, content: turnFailure }, 'codex turn.failed event');
                }
                touchStallWatchdog();
            ...
            const runInput = async (prompt, correlation) => {
                ...
                if (pre.runningRuns >= pre.maxConcurrentRuns || pre.queuedRuns > 0) {
                    pushSynthetic({
                        type: 'status',
                        content: `⏳ Codex 并发已满（${pre.runningRuns}/${pre.maxConcurrentRuns} 运行中）` +
                ...
                try {
                    ...
                    if (stalled || result.aborted) {
                        logger.info({ sessionKey, abortExitLatencyMs: result.abortExitLatencyMs }, 'codex run aborted; late events and retry paths suppressed');
                    ...
                    if (authFailed) {
                        ...
                        const resetHint = /try again (?:at|in) [^.\n]+/i.exec(runFailureText)?.[0] ??
                        ...
                        pushSynthetic({
                            type: 'error',
                            content: `codex exec failed to spawn (${result.spawnError.message}). ` +
                        ...
                        logger.warn({ threadId: resumeTarget }, 'codex resume target missing (no rollout found); cleared — next turn starts a fresh session (Issue #4628)');
                        pushSynthetic({
                            type: 'error',
                        ...
                        pushSynthetic({
                            type: 'error',
                            content: `codex exec exited with code ${result.exitCode}${result.stderrTail ? `: ${result.stderrTail.trim().slice(-500)}` : ''}`,
                            role: 'assistant',
                        });
                    }
                        ...
                        pushSynthetic({
                            type: 'error',
                            content: 'codex exec exited 0 without completing a turn (no turn.completed event) — ' +
    ...
    queryAppServer(input, options, sandbox, binary, skillsManifest) {
        let lifecycle;
        ...
        let registration;
        let stopInput;
        const stopSignal = new Promise((resolveStop) => { stopInput = resolveStop; });
        let activeTurnId;
        let turnDone;
        let stallTimer;
        ...
        let interruptFlight;
        const isToolItem = (type) => type === 'commandExecution' || type === 'mcpToolCall';
        const fireStall = () => {
            stallTimer = undefined;
            if (stopped || !activeTurnId) {
                return;
            }
            ...
            void lifecycle?.interrupt(sessionKey).catch((error) => {
        ...
        const armStall = () => {
            ...
            if (pendingInputs.size > 0 || !activeTurnId) {
                return;
            }
            stallTimer = setTimeout(fireStall, stallTimeoutMs);
            stallTimer.unref?.();
        ...
        const onNotification = (method, params) => {
            if (method === 'transport/exited') {
                turnDone?.(params.error ?? new Error('codex app-server exited'));
                turnDone = undefined;
                return;
            }
            if (stopped) {
            ...
            if (method === 'item/started' && event.item?.type === 'commandExecution') {
                ...
                push({
                    ...
                    role: 'assistant',
                    ...(failed ? { metadata: { terminatedReason: 'turn_failed' } }
                        : interrupted ? { metadata: { terminatedReason: 'interrupted' } } : {}),
                });
                turnDone?.();
                if (stallTimer) {
                    clearTimeout(stallTimer);
        ...
        const stopHandle = (reason) => {
            ...
            stopInput();
            void input.return?.(undefined);
            interruptFlight = lifecycle?.interrupt(sessionKey).catch((error) => {
                push({ type: 'error', content: error instanceof Error ? error.message : String(error), role: 'system' });
            });
            turnDone?.(new Error(reason));
        };
        const stopSession = () => stopHandle('codex app-server session closed');
        ...
        void (async () => {
            try {
                ...
                while (!stopped) {
                    ...
                    let bindTurn;
                    const turnBinding = new Promise(resolve => { bindTurn = resolve; });
                    try {
                        if (stopped) {
                            break;
                        }
                        lifecycle = this.createAppServerLifecycle(binary, sessionKey, next.value.correlation, options.onUserInput ? async (request) => {
                            ...
                            const finish = () => {
                                ...
                                if (!stopped && activeTurnId === boundTurn) {
                                    armStall();
                                }
                            };
                        ...
                        activeTurnId = await lifecycle.startTurn(sessionKey, skillsManifest ? `${skillsManifest}\n\nUser request:\n${userInput}` : userInput, {
                            sandbox,
                            networkAccess: this.networkAccess,
                            cwd: options.cwd,
                        });
                ...
                if (this.appServerStops.get(sessionKey) === stopSession) {
        ...
        const iterator = (async function* () {
            while (!done || queue.length > 0) {
                if (queue.length === 0) {
                    await new Promise((resolveWake) => wakeups.push(resolveWake));
                    continue;
                }
                yield queue.shift();
            }
            ...
            handle: {
                ...
                steer: async (text) => {
                    if (stopped || !lifecycle) {
                        throw new Error('No active app-server turn');
                    }
    ...
    createAppServerLifecycle(binary, sessionKey, correlation, onUserInput) {
        const lifecycle = new CodexAppServerLifecycle({
            ...
            onNotification: (method, params) => {
                ...
                if (threadId) {
                    this.appServerRoutes.get(threadId)?.(method, params);
                }
            },
            onExit: (exit) => {
                if (this.appServerLifecycles.get(sessionKey) !== lifecycle) {
                ...
                if (threadId) {
                    this.appServerRoutes.get(threadId)?.('transport/exited', { error });
                }
            },
        });
    ...
    createInlineTool(_definition) {
        ...
        throw new Error('CodexAgentProvider: tools/MCP mapping is not supported yet — tracked as an open question on #4627.');
    }
    ...
    dispose() {
        this.disposed = true;
        for (const stop of this.appServerStops.values()) {
            stop();
        }
        this.appServerStops.clear();
        for (const lifecycle of this.appServerLifecycles.values()) {
