...
export class CodexAgentProvider {
    ...
    queryAppServer(input, options, sandbox) {
        const lifecycle = this.appServerLifecycle;
    ...
    ensureAppServerLifecycle(binary) {
        if (this.appServerLifecycle) {
            return;
        }
        this.appServerLifecycle = new CodexAppServerLifecycle({
            ...
            onExit: (exit) => {
                ...
                for (const route of this.appServerRoutes.values()) {
                    route('transport/exited', { error });
                }
            },
        });
    }
