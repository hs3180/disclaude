    ...
    run(options, onEvent) {
        ...
        const promise = new Promise((resolve) => {
            ...
            if (currentChild.stdout) {
                ...
                readline.on('line', (line) => {
                    ...
                    try {
                        ...
                        logger.debug({ pid: currentChild.pid, line: trimmed.slice(0, 500), lineLength: trimmed.length }, 'codex exec stdout line');
                    }
                });
            }
            ...
            currentChild.stderr?.on('data', (chunk) => {
                ...
                options.stderr?.(text);
                logger.debug({ pid: currentChild.pid, chunk: text.slice(0, 1000), chunkLength: text.length }, 'codex exec stderr chunk');
            });
