 ...
 */
export function resolveCodexSandboxPolicy(options, configSandbox, fullAccess = false) {
    const reasons = [];
    ...
    if (fullAccess) {
        ...
        throw new Error(`CodexAgentProvider: unknown permissionMode "${String(options.permissionMode)}" — ` +
            'refusing to infer a sandbox level (fail closed, #4631).');
    }
    reasons.push(fullAccess
    ...
    const denylist = (options.disallowedTools ?? []).map((name) => name.split('(')[0].trim().toLowerCase());
    const mutationHits = denylist.filter((name) => MUTATION_TOOL_NAMES.has(name));
    if (mutationHits.length > 0) {
        sandbox = 'read-only';
        reasons.push(`disallowedTools has mutation tools (${[...new Set(mutationHits)].join(', ')}) → capped at read-only`);
    }
    ...
    if (webSearchHits.length > 0) {
        throw new Error('CodexAgentProvider: disallowedTools includes web-search tools ' +
