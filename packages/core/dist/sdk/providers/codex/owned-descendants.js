 ...
 */
export async function captureDescendantGroups(parent) {
    if (process.platform === 'win32' || !Number.isSafeInteger(parent) || parent <= 0) {
        return [];
    }
 ...
 */
export async function signalDescendantGroups(groups, signal) {
    if (!groups.length) {
        return;
    }
    ...
    for (const group of groups) {
        ...
        try {
            process.kill(-group.group, signal);
        }
        catch (error) {
            if (error.code !== 'ESRCH') {
                throw error;
