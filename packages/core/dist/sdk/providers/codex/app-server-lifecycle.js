...
export class CodexAppServerLifecycle {
    ...
    constructor(options = {}) {
        ...
        const onUserInput = options.onUserInput ? async (request) => {
        ...
        this.asyncInputs = new CodexAsyncUserInput(onUserInput, async (threadId, turnId, text) => {
            ...
            if (response.turnId !== turnId) {
                throw new Error('Async answer did not match its originating turn');
            }
        ...
        this.transport = new CodexAppServerTransport({
            ...
            onNotification: (method, params) => {
                ...
                options.onNotification?.(method, handled ? { ...params, agentInputHandled: true } : params);
            },
            onExit: (exit) => {
    ...
    async interruptTurn(sessionKey) {
        ...
        this.transport.cancelUserInputs(session.threadId, turnId);
    ...
    forgetSession(sessionKey) {
        const session = this.sessions.get(sessionKey);
        if (session?.threadId) {
            this.asyncInputs.cancel('closed', session.threadId);
        }
