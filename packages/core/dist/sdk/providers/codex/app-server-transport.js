...
import { parseAgentInputParams, validateAgentInputAnswers } from '../../user-input.js';
...
export class CodexAppServerTransport {
    ...
    close() {
        ...
        this.cleanup = (async () => {
            let descendants = [];
            try {
                descendants = await captureDescendantGroups(this.child.pid ?? 0);
            }
            catch {
                this.logger.warn('Could not inspect app-server descendant groups before shutdown');
            }
            const signalDescendants = async (signal) => {
                try {
                    await signalDescendantGroups(descendants, signal);
                }
                catch {
                    this.logger.warn({ signal }, 'Could not signal verified app-server descendant groups');
                }
            };
    ...
    cancelUserInputs(threadId, turnId) {
        for (const [id, input] of this.inputs) {
            if (input.params.threadId === threadId && input.params.turnId === turnId) {
                this.cancelInput(id, 'Turn interrupted');
            }
        }
    }
    write(message) {
    ...
    receive(line) {
        ...
        if (message.method) {
            ...
            if (message.method === 'serverRequest/resolved' && event?.requestId !== undefined) {
                if (this.inputs.get(event.requestId)?.params.threadId === event.threadId) {
                    this.cancelInput(event.requestId, undefined, 'resolved');
                }
            }
            if (message.method === 'turn/completed' && event?.threadId && event.turn?.id) {
                for (const [id, input] of this.inputs) {
                    if (input.params.threadId === event.threadId && input.params.turnId === event.turn.id) {
                        this.cancelInput(id, undefined, 'turn-ended');
                    }
                }
            }
            try {
    ...
    failAll(error) {
        ...
        for (const id of this.inputs.keys()) {
            this.cancelInput(id, undefined, 'closed');
        }
    ...
    cancelInput(id, replyError, reason = 'cancelled') {
    ...
    receiveUserInput(id, raw) {
        if (!this.acceptingRequests) {
            return;
        }
        // A duplicated server request must not create two actionable cards or replies.
        if (this.seenInputIds.has(id)) {
            return;
        }
        let params;
        try {
            if (!(typeof id === 'string' && id.length <= 200) && !Number.isSafeInteger(id)) {
        ...
        const request = { ...params, requestId: id, signal: abort.signal, respond: async (value) => {
                if (this.inputs.get(id) !== pending || pending.writing || !this.acceptingRequests) {
                    throw new Error('User-input request is no longer active');
                }
        ...
        void Promise.resolve().then(() => {
            if (!abort.signal.aborted) {
                return this.options.onUserInput?.(request);
            }
            return undefined;
