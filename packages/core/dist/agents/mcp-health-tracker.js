...
export class McpHealthTracker {
    ...
    constructor(options = {}) {
        ...
        this.degradationThreshold = Math.max(1, options.degradationThreshold ?? DEFAULT_DEGRADATION_THRESHOLD);
        this.isFastTripFailure = options.isFastTripFailure;
        this.now = options.now ?? (() => new Date());
    }
    /** Fetch-or-create the health record for a tool. */
    getOrCreate(toolName) {
    ...
    trip(toolName) {
        ...
        if (!rec.degraded) {
            rec.degraded = true;
            rec.degradedAt = this.now().toISOString();
            logger.warn({ toolName, reason: 'manual', totalFailures: rec.totalFailures }, 'MCP tool marked degraded (manual trip)');
        }
    }
