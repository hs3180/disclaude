...
function acceptsRuntimeKey(key) {
 ...
 */
export function loadRuntimeEnv(workspaceDir, strict = false) {
    ...
    try {
        ...
        for (const line of content.split('\n')) {
            ...
            if (eqIndex > 0) {
                ...
                if (acceptsRuntimeKey(key) && !value.includes('\0')) {
                    env[key] = value;
                }
            }
        }
 ...
 */
export function setRuntimeEnv(workspaceDir, key, value, options = {}) {
    if (!acceptsRuntimeKey(key) || /[\r\n\0]/.test(value)) {
        throw new Error('Unsafe runtime environment entry');
    }
    ...
    if (options.expiresAt !== undefined) {
        existing[`${key}${EXPIRY_SUFFIX}`] = new Date(options.expiresAt).toISOString();
    }
 ...
 */
export function deleteRuntimeEnv(workspaceDir, key) {
    const existing = loadRuntimeEnv(workspaceDir, true);
    if (!(key in existing)) {
        return;
    }
