 ...
 */
const CONFIG_FILE_NAMES = ['disclaude.config.yaml', 'disclaude.config.yml'];
/**
        ...
        for (const fileName of CONFIG_FILE_NAMES) {
            ...
            if (existsSync(filePath)) {
                if (searchPath !== SEARCH_PATHS[0]) {
                    logger.warn({ filePath, preferredDirectory: SEARCH_PATHS[0] }, 'Using legacy config location; move it to ~/.disclaude/disclaude.config.yaml');
                }
...
export function loadConfigFile(filePath) {
    ...
    try {
        ...
        logger.info({ path: fileInfo.path, keys: Object.keys(parsed) }, 'Configuration file loaded successfully');
        return {
            ...parsed,
 ...
 */
export function getConfigFromFile(fileConfig) {
 ...
 */
export function getPreloadedConfig() {
...
export function validateConfig(config) {
    ...
    if (config.agent?.autoCompactWindow !== undefined &&
    ...
    if (config.agent?.agentBackend === 'codex') {
        ...
        if (config.agent.model && !isCodexModel(config.agent.model)) {
            logger.error(`agent.model must be a Codex/ChatGPT model (expected gpt-5.x or newer, got "${config.agent.model}")`);
            return false;
        }
    }
    ...
    if (config.agent?.agentBackend !== undefined) {
        const allowed = ['claude', 'pi', 'codex', 'deepseek'];
        if (!allowed.includes(config.agent.agentBackend)) {
            logger.error(`agent.agentBackend must be one of: ${allowed.join(', ')} (got "${config.agent.agentBackend}"). ` +
    ...
    if (config.agent?.codexSandbox !== undefined) {
        const allowedSandbox = ['read-only', 'workspace-write', 'danger-full-access'];
        if (!allowedSandbox.includes(config.agent.codexSandbox)) {
            logger.error(`agent.codexSandbox must be one of: ${allowedSandbox.join(', ')} (got "${config.agent.codexSandbox}"). ` +
                'It is the codex exec sandbox level, only meaningful with agentBackend: codex.');
            return false;
        }
    }
    if (config.agent?.fullAccess !== undefined &&
        typeof config.agent.fullAccess !== 'boolean') {
        logger.error(`agent.fullAccess must be a boolean (got ${String(config.agent.fullAccess)}). ` +
            'Set it to true only when the unrestricted Codex sandbox is explicitly intended.');
        return false;
    }
    if (config.agent?.fullAccess === true &&
        config.agent.codexSandbox !== undefined &&
        config.agent.codexSandbox !== 'danger-full-access') {
        logger.error('agent.fullAccess: true conflicts with agent.codexSandbox: ' +
            `"${config.agent.codexSandbox}". Remove codexSandbox or set it to "danger-full-access".`);
        return false;
    }
    if (config.agent?.codexNetworkAccess !== undefined &&
        typeof config.agent.codexNetworkAccess !== 'boolean') {
        logger.error(`agent.codexNetworkAccess must be a boolean (got ${String(config.agent.codexNetworkAccess)}). ` +
    ...
    if (config.agent?.codex !== undefined) {
        ...
        if (config.agent.codex.transport !== undefined &&
            config.agent.codex.transport !== 'exec' &&
            config.agent.codex.transport !== 'app-server') {
            logger.error(`agent.codex.transport must be "exec" or "app-server" (got ${String(config.agent.codex.transport)})`);
            return false;
        }
        for (const [name, value] of [
            ...
            if (value !== undefined && (!Number.isFinite(value) || value <= 0)) {
                logger.error(`agent.codex.${name} must be a positive number (got ${value}). ` +
        ...
        if (execTimeoutMs !== undefined && (!Number.isFinite(execTimeoutMs) || execTimeoutMs < 0)) {
            logger.error(`agent.codex.execTimeoutMs must be a non-negative number (got ${execTimeoutMs}). ` +
...
export function validateRequiredConfig(config) {
    ...
    if (explicitlyUsesGlm && !config.glm?.apiBaseUrl) {
        errors.push({
            ...
            message: 'glm.apiBaseUrl is required for the selected GLM provider; configure a supported Anthropic-compatible proxy endpoint',
        });
    }
