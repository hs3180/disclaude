...
/**
 * Build the user taste (preference) context section.
 *
 * Issue #2335: Injects learned user preferences into the agent prompt
 * so the agent respects established preferences without repeated corrections.
 *
 * The taste context is a formatted string generated by TasteManager.formatTasteContext().
 * If no taste rules exist, returns an empty string (section is omitted entirely).
 *
 * @param tasteContext - Formatted taste context from TasteManager, or undefined to skip
 * @returns Formatted taste context section, or empty string if no context
 */
export function buildTasteContextSection(tasteContext?: string): string {
  if (!tasteContext) {
    return '';
  }
...
**Important**: These preferences were learned from your past interactions with this user. Follow them strictly to avoid the user having to repeat corrections. If the user explicitly overrides a preference in the current conversation, follow the override instead.
