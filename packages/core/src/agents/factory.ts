 ...
 * Issue #2717 Phase 1: Added channelMcpFactory injection support.
 *
...
import { type ChatAgent as ChatAgentInterface, type BaseAgentConfig, type AgentProvider, type SchedulerCallbacks, type MessageBuilderOptions, type ChannelCapabilities, Config } from '../index.js';
import { ChatAgent, type ChatAgentConfig } from './chat-agent/index.js';

/**
 * Callbacks for ChatAgent.
 * Re-exported for external use (matches worker-node/types.ts ChatAgentCallbacks).
 */
export interface ChatAgentExternalCallbacks {
  /** Send a text message */
  ...
  /** Get channel capabilities for a chat */
  getCapabilities?: (chatId: string) => ChannelCapabilities | undefined;
  /** Get chat history for first message context */
 ...
 * ```
 */
export function toChatAgentCallbacks(callbacks: SchedulerCallbacks): ChatAgentExternalCallbacks {
  return {
...
export interface AgentCreateOptions {
  ...
  /**
   * Factory function to create channel MCP server configuration.
   * Issue #2717 Phase 1: Injected to break core ↔ mcp-server dependency.
   * Should be provided by the caller (e.g., primary-node) that has access
   * to the @disclaude/mcp-server package.
   */
...
export class AgentFactory {
  ...
  static createAgent(
    chatId: string,
    callbacks: ChatAgentExternalCallbacks,
    ...
    const config: ChatAgentConfig = {
      ...
      channelMcpFactory: options.channelMcpFactory,
    };
