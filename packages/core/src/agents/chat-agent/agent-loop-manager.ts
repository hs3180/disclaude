 ...
 * Issue #2717 Phase 1: Replaced direct import of createChannelMcpServer
 * with injected channelMcpFactory dependency to break circular dependency
 * between core and mcp-server packages.
 *
...
import { Config, MessageChannel, RestartManager, ConversationOrchestrator, type StreamingUserMessage, type QueryHandle, type AgentQueryOptions, type QueryStreamResult } from '../../index.js';
...
export interface LoopContext {
   ...
   * When provided, used to create the 'channel-mcp' server entry.
   * When omitted, channel MCP tools are not included.
   */
...
export class AgentLoopManager {
  ...
  private buildMcpServers(chatId: string, callbacks: ChatAgentCallbacks): Record<string, unknown> {
    ...
    if (shouldIncludeContextMcp && this.ctx.channelMcpFactory) {
      mcpServers['channel-mcp'] = this.ctx.channelMcpFactory();
