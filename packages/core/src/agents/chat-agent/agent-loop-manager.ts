 ...
 * Migrated from @disclaude/worker-node to @disclaude/core (Issue #2717 Phase 1).
 *
 ...
 *
 * Issue #2717: createChannelMcpServer is now injected via LoopContext to avoid
 * circular dependency between @disclaude/core and @disclaude/mcp-server.
 */
...
import type { StreamingUserMessage, QueryHandle, AgentQueryOptions } from '../../sdk/types.js';
import type { QueryStreamResult } from '../base-agent-query.js';
...
export interface LoopContext {
   ...
   * Issue #2717 Phase 1: Previously imported from @disclaude/mcp-server.
   */
...
export class AgentLoopManager {
  ...
  private buildMcpServers(chatId: string, callbacks: ChatAgentCallbacks): Record<string, unknown> {
    ...
    if (shouldIncludeContextMcp && this.ctx.createChannelMcpServer) {
      mcpServers['channel-mcp'] = this.ctx.createChannelMcpServer();
