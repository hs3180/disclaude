/**
 * Agent Runtime Context - Dependency injection for agent initialization.
 *
 * Extracted from types.ts as part of #2345 Phase 4.
 *
...
export interface AgentRuntimeContext {
  ...
  getAgentConfig(): { apiKey: string; model: string; apiBaseUrl?: string; provider: import('./types.js').AgentProvider };
...
/** Set the runtime context for agents. Must be called by main package before using any agents. */
export function setRuntimeContext(ctx: AgentRuntimeContext): void {
  globalRuntimeContext = ctx;
}

/** Get the runtime context. Throws if context is not set. */
...
/** Check if runtime context is set. Useful for conditional behavior during migration. */
