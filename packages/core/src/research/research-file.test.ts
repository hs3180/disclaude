...
describe('isValidTopic', () => {
  ...
  it('should accept CJK topics', () => {
    expect(isValidTopic('人工智能安全')).toBe(true);
    expect(isValidTopic('ai安全研究')).toBe(true);
  });
...
describe('buildTemplate', () => {
  it('should build template with topic and description', () => {
    const result = buildTemplate('ai-safety', 'Research AI safety', '- [ ] Goal 1');
  ...
  it('should capitalize topic display name', () => {
    const result = buildTemplate('machine-learning', 'ML research', '- [ ] G');
    expect(result).toContain('# Machine Learning');
  });

  it('should use default goal when empty', () => {
    const result = buildTemplate('test', 'desc', '- [ ] Define research objectives');
    expect(result).toContain('Define research objectives');
  });
});

describe('formatFinding', () => {
  it('should format finding with source', () => {
    const finding: ResearchFinding = {
      title: 'RLHF Effectiveness',
      source: 'https://arxiv.org/abs/2309',
      content: 'Reduces harmful outputs by 50%',
    };
    const result = formatFinding(finding);
    expect(result).toBe('### RLHF Effectiveness\n- 来源：https://arxiv.org/abs/2309\n- 关键内容：Reduces harmful outputs by 50%');
  });

  it('should format finding without source', () => {
    const finding: ResearchFinding = {
      title: 'Observation',
      content: 'No source available',
    };
    const result = formatFinding(finding);
    expect(result).toBe('### Observation\n- 关键内容：No source available');
  });
});
...
describe('appendToSection', () => {
  ...
  it('should append to an empty section', () => {
    const result = appendToSection(baseContent, '已收集的信息', '### Finding 1\n- 关键内容：Data');
    expect(result).toContain('### Finding 1');
    expect(result).toContain('已收集的信息');
  });

  it('should append to a non-empty section', () => {
    const contentWithFindings = baseContent.replace(
      '## 已收集的信息\n\n',
      '## 已收集的信息\n### First\n- 关键内容：C1\n\n'
    );
    const result = appendToSection(contentWithFindings, '已收集的信息', '### Second\n- 关键内容：C2');
    expect(result).toContain('### First');
    expect(result).toContain('### Second');
  });

  it('should create section if it does not exist', () => {
    const result = appendToSection(baseContent, '新章节', 'New content');
    expect(result).toContain('## 新章节');
    expect(result).toContain('New content');
  });

  it('should not duplicate section heading', () => {
    const result = appendToSection(baseContent, '已收集的信息', '### Finding 1');
    const count = (result.match(/## 已收集的信息/g) || []).length;
    expect(count).toBe(1);
  });
});
...
describe('replaceSection', () => {
  ...
  it('should replace existing section content', () => {
    const result = replaceSection(baseContent, '研究结论', 'This is the conclusion.');
    expect(result).toContain('This is the conclusion.');
    expect(result).toContain('## 研究结论');
  });

  it('should create section if it does not exist', () => {
    const result = replaceSection('# Test', '研究结论', 'Conclusion text');
    expect(result).toContain('## 研究结论');
...
describe('insertSection', () => {
  it('should insert section in alphabetical order', () => {
    const content = '## C\n\n## E\n\n';
    const result = insertSection(content, 'D', 'D content');
    expect(result).toContain('## D');
    expect(result).toContain('D content');
  });

  it('should append at end if no section is after it alphabetically', () => {
    const content = '## A\n\n## B\n\n';
    const result = insertSection(content, 'Z', 'Z content');
    expect(result).toContain('## Z');
    const zIndex = result.indexOf('## Z');
    const bIndex = result.indexOf('## B');
    expect(zIndex).toBeGreaterThan(bIndex);
  });
});
...
describe('parseContent', () => {
  const fullContent = [
    ...
    '',
    '## 研究结论',
    'RLHF is effective but has trade-offs.',
    '',
    '## 相关资源',
  ...
  it('should parse title and description', () => {
    ...
    expect(parsed.title).toBe('AI Safety');
    expect(parsed.description).toBe('Research AI safety best practices');
  });

  it('should parse goals correctly', () => {
    const parsed = parseContent(fullContent);
    expect(parsed.goals).toEqual([
      'Review alignment research',
      'Analyze RLHF methods',
      'Test guardrails',
    ]);
    expect(parsed.completedGoals).toEqual(['Analyze RLHF methods']);
    expect(parsed.pendingGoals).toEqual(['Review alignment research', 'Test guardrails']);
  });

  it('should parse findings', () => {
    const parsed = parseContent(fullContent);
    expect(parsed.findings).toHaveLength(2);
    expect(parsed.findings[0].title).toBe('RLHF Effectiveness');
    expect(parsed.findings[0].source).toBe('https://arxiv.org/abs/2309');
    expect(parsed.findings[0].content).toBe('Reduces harmful outputs by 50%');
    expect(parsed.findings[1].title).toBe('Alignment Tax');
    expect(parsed.findings[1].content).toBe('~5% capability reduction');
  });
  ...
  it('should parse conclusion', () => {
    const parsed = parseContent(fullContent);
    expect(parsed.conclusion).toContain('RLHF is effective but has trade-offs');
  });
  ...
  it('should include raw content', () => {
    const parsed = parseContent(fullContent);
    expect(parsed.raw).toBe(fullContent);
  });
  ...
  it('should handle empty sections gracefully', () => {
    ...
    const parsed = parseContent(minimalContent);
...
// ==========================================================================
// ResearchFileManager integration tests
// ==========================================================================
...
describe('ResearchFileManager', () => {
  ...
  describe('listTopics', () => {
    ...
    it('should skip _archived directory', async () => {
      ...
      await fs.writeFile(
        path.join(tmpDir, 'research', '_archived', 'RESEARCH.md'),
        '# archived'
      );
  ...
  describe('setConclusion', () => {
    it('should set conclusion content', async () => {
      ...
      await manager.setConclusion(
        'test',
        'Research complete. Key finding: X is better than Y.'
      );
