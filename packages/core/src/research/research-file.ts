 ...
 *
 * 1. **Initialization (Phase 1)**: Create RESEARCH.md with structured template
 * 2. **Auto-update (Phase 2)**: Add findings, questions, resources during research
 * 3. **Archive (Phase 3)**: Move completed research to archive directory
 *
...
export interface ParsedResearch {
  ...
  description: string;
  /** All research goals (checklist items under ## 研究目标) */
  goals: string[];
...
/** Valid topic name pattern: alphanumeric, CJK, hyphens, underscores, dots */
const TOPIC_PATTERN = /^[a-zA-Z0-9\u4e00-\u9fff\u3400-\u4dbf][a-zA-Z0-9\u4e00-\u9fff\u3400-\u4dbf._-]*$/;
 ...
 * Supports CJK characters for Chinese topic names.
 *
...
export class ResearchFileManager {
  ...
  async initialize(options: ResearchInitOptions): Promise<void> {
    ...
    if (!isValidTopic(topic)) {
      throw new Error(
        `Invalid topic name: "${topic}". Must be alphanumeric/CJK with hyphens/underscores.`
      );
    }
    ...
    const content = buildTemplate(topic, description, goalsSection);
  ...
  async addFinding(topic: string, finding: ResearchFinding): Promise<void> {
    ...
    const findingEntry = formatFinding(finding);
    const updated = appendToSection(content, '已收集的信息', findingEntry);
  ...
  async addQuestion(topic: string, question: string): Promise<void> {
    ...
    const updated = appendToSection(content, '待调查的问题', entry);
    await this.write(topic, updated);
  ...
  async resolveQuestion(topic: string, question: string, answer: string): Promise<void> {
    ...
    const findingEntry = formatFinding(finding);
    updated = appendToSection(updated, '已收集的信息', findingEntry);
  ...
  async addResource(topic: string, resource: ResearchResource): Promise<void> {
    ...
    const updated = appendToSection(content, '相关资源', entry);
    await this.write(topic, updated);
  ...
  async setConclusion(topic: string, conclusion: string): Promise<void> {
    const content = await this.read(topic);
    const updated = replaceSection(content, '研究结论', conclusion);
    await this.write(topic, updated);
  ...
  async addGoal(topic: string, goal: string): Promise<void> {
    const content = await this.read(topic);
    const entry = `- [ ] ${goal}`;
    const updated = appendToSection(content, '研究目标', entry);
    await this.write(topic, updated);
...
// ============================================================================
// Module-level helper functions (pure, testable)
// ============================================================================
 ...
 * @param description - Research description
 * @param goalsSection - Pre-formatted goals checklist
 * @returns Complete RESEARCH.md content
 */
export function buildTemplate(topic: string, description: string, goalsSection: string): string {
 ...
 */
export function appendToSection(content: string, sectionName: string, entry: string): string {
  ...
  if (sectionIndex === -1) {
    ...
    return insertSection(content, sectionName, entry);
  }
...
export function replaceSection(content: string, sectionName: string, newContent: string): string {
  ...
  if (sectionIndex === -1) {
    ...
    return insertSection(content, sectionName, newContent);
  }
 ...
 * @returns Parsed research data
 */
export function parseContent(raw: string): ParsedResearch {
