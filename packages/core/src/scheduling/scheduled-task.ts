...
export interface ScheduledTask {
  ...
  model?: string;
  /**
   * Optional per-task SOUL.md personality file path.
   * When set, overrides the global SOUL.md for this specific task.
   * Supports tilde expansion and relative paths (resolved against workspace).
   * Defined in schedule markdown frontmatter (e.g., `soul: "~/.disclaude/souls/code-reviewer.md"`).
   *
   * Issue #1315: Per-task personality support.
   */
  soul?: string;
}
