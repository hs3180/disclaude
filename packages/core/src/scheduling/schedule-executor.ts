 ...
 * Issue #1315: Per-task SOUL.md personality injection for scheduled tasks.
 *
 ...
 *     -> SoulLoader.load(soulPath) if specified
 ...
 * 1. Loads per-task soul if a soul path is provided (Issue #1315)
...
export function createScheduleExecutor(options: ScheduleExecutorOptions): TaskExecutor {
  ...
  return async (chatId: string, prompt: string, userId?: string, model?: string, soulPath?: string): Promise<void> => {
    // Issue #1315: Load per-task soul if a soul path is provided
