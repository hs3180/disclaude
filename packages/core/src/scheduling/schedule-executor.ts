 ...
 *     -> loadSoulFile(soulPath) (optional, for per-task personality)
 ...
 * @param systemPromptAppend - Optional system prompt append for per-task soul (Issue #1315)
 ...
 * Issue #1315: Per-task SOUL.md personality injection via systemPromptAppend.
 *
...
export function createScheduleExecutor(options: ScheduleExecutorOptions): TaskExecutor {
  ...
  return async (chatId: string, prompt: string, userId?: string, model?: string, soulPath?: string): Promise<void> => {
    // Issue #1315: Load per-task soul if specified
    let systemPromptAppend: string | undefined;
    if (soulPath) {
      const { loadSoulFile } = await import('../soul/loader.js');
      const soulResult = await loadSoulFile(soulPath);
      if (soulResult) {
    ...
    // Issue #1315: Pass systemPromptAppend for per-task soul
