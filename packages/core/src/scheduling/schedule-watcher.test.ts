...
const { mockMkdir, mockWriteFile, mockReadFile, mockReaddir, mockStat, mockUnlink, mockRm, mockAccess, mockFsWatch } = vi.hoisted(() => {
...
describe('ScheduleFileScanner', () => {
  ...
  describe('parseFile', () => {
    it('should parse a valid schedule file', async () => {
      ...
      const task = await scanner.parseFile(`${MOCK_DIR}/daily-report/SCHEDULE.md`, 'daily-report');
    ...
    it('should return null when required fields are missing (no name)', async () => {
      ...
      const task = await scanner.parseFile(`${MOCK_DIR}/invalid/SCHEDULE.md`, 'invalid');
    ...
    it('should return null when file read fails', async () => {
      ...
      const task = await scanner.parseFile(`${MOCK_DIR}/missing/SCHEDULE.md`, 'missing');
      expect(task).toBeNull();
    });
    ...
    it('should handle file without frontmatter gracefully', async () => {
      ...
      const task = await scanner.parseFile(`${MOCK_DIR}/no-frontmatter/SCHEDULE.md`, 'no-frontmatter');
      expect(task).toBeNull();
    });
    ...
    it('should parse optional fields', async () => {
      ...
      const task = await scanner.parseFile(`${MOCK_DIR}/custom-task/SCHEDULE.md`, 'custom-task');
      expect(task).not.toBeNull();
      expect(task!.id).toBe('schedule-custom-task');
    ...
    it('should parse model field when specified (Issue #1338)', async () => {
      ...
      const task = await scanner.parseFile(`${MOCK_DIR}/coding-task/SCHEDULE.md`, 'coding-task');
      expect(task).not.toBeNull();
    ...
    it('should default model to undefined when not specified', async () => {
      ...
      const task = await scanner.parseFile(`${MOCK_DIR}/no-model/SCHEDULE.md`, 'no-model');
      expect(task).not.toBeNull();
    ...
    it('should parse quoted model value (stripping quotes)', async () => {
      ...
      const task = await scanner.parseFile(`${MOCK_DIR}/fast-task/SCHEDULE.md`, 'fast-task');
      expect(task).not.toBeNull();
    ...
    it('should parse unquoted model value (Issue #1338)', async () => {
      ...
      const task = await scanner.parseFile(`${MOCK_DIR}/unquoted-model/SCHEDULE.md`, 'unquoted-model');
      expect(task).not.toBeNull();
      expect(task!.model).toBe('glm-4.7');
    });
    ...
    it('should not strip mismatched nested quotes from model value (Issue #1338)', async () => {
      ...
      const task = await scanner.parseFile(`${MOCK_DIR}/nested-quote/SCHEDULE.md`, 'nested-quote');
      expect(task).not.toBeNull();
    ...
    it('should parse unquoted string values', async () => {
      ...
      const task = await scanner.parseFile(`${MOCK_DIR}/unquoted/SCHEDULE.md`, 'unquoted');
      expect(task).not.toBeNull();
    ...
    it('should parse quoted string values (stripping quotes)', async () => {
      ...
      const task = await scanner.parseFile(`${MOCK_DIR}/quoted/SCHEDULE.md`, 'quoted');
      expect(task).not.toBeNull();
    ...
    it('should default enabled to true when not specified', async () => {
      ...
      const task = await scanner.parseFile(`${MOCK_DIR}/default-enabled/SCHEDULE.md`, 'default-enabled');
      expect(task).not.toBeNull();
      expect(task!.enabled).toBe(true);
    });
    ...
    it('should include sourceFile and fileMtime', async () => {
      ...
      const task = await scanner.parseFile(`${MOCK_DIR}/test/SCHEDULE.md`, 'test');
      expect(task).not.toBeNull();
      expect(task!.sourceFile).toBe(`${MOCK_DIR}/test/SCHEDULE.md`);
      expect(task!.fileMtime).toEqual(new Date('2026-03-20T12:00:00Z'));
    });

    it('should derive dirName from file path when not provided (Issue #2526)', async () => {
      mockReadFile.mockResolvedValue(makeScheduleContent());

      const task = await scanner.parseFile(`${MOCK_DIR}/inferred-name/SCHEDULE.md`);
      expect(task).not.toBeNull();
      expect(task!.id).toBe('schedule-inferred-name');
    });
  });

  describe('scanAll', () => {
    it('should scan all subdirectories for SCHEDULE.md files', async () => {
      // Issue #2526: readdir with withFileTypes returns directory entries
      ...
      mockReaddir.mockResolvedValue([
        dirEntry('daily-report', true),
        dirEntry('weekly-summary', true),
    ...
    it('should skip subdirectories that fail to parse', async () => {
      ...
      mockReaddir.mockResolvedValue([dirEntry('valid'), dirEntry('invalid')]);
      mockReadFile
  ...
  describe('writeTask', () => {
    it('should write a task with schedule- prefix stripped into subdirectory', async () => {
    ...
    it('should create subdirectory before writing', async () => {
      const task: ScheduledTask = {
        id: 'schedule-daily-report',
        name: 'Daily Report',
        cron: '0 9 * * *',
  ...
  describe('deleteTask', () => {
    it('should delete a task directory and return true', async () => {
      const result = await scanner.deleteTask('schedule-daily-report');
      expect(result).toBe(true);
      expect(mockRm).toHaveBeenCalledWith(`${MOCK_DIR}/daily-report`, { recursive: true, force: true });
    });
    ...
    it('should return false when directory does not exist (ENOENT)', async () => {
      mockRm.mockRejectedValue({ code: 'ENOENT' } as NodeJS.ErrnoException);
  ...
  describe('getFilePath', () => {
    it('should return SCHEDULE.md path inside subdirectory', () => {
  ...
  describe('parseFile - empty model warning (Issue #1338)', () => {
    it('should warn when model is empty string', async () => {
      ...
      const task = await scanner.parseFile(`${MOCK_DIR}/empty-model/SCHEDULE.md`, 'empty-model');
      expect(task).not.toBeNull();
...
describe('ScheduleFileWatcher', () => {
  ...
  describe('file event handling', () => {
    ...
    it('should ignore plain .md files that are not SCHEDULE.md', () => {
    ...
    it('should debounce rapid file events', () => {
      eventCallback('change', 'test/SCHEDULE.md');
      eventCallback('change', 'test/SCHEDULE.md');
      eventCallback('change', 'test/SCHEDULE.md');
