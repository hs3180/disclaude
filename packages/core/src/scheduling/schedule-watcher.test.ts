 ...
 *
 * Issue #2526: Updated to test subdirectory-based schedule structure
 * (`schedules/<name>/SCHEDULE.md` instead of `schedules/<name>.md`).
 *
...
/** Create a mock fs.Dirent for a directory entry. */
function mockDir(name: string): { name: string; isDirectory: () => boolean; isFile: () => boolean } {
  return { name, isDirectory: () => true, isFile: () => false };
}

/** Create a mock fs.Dirent for a file entry. */
function mockFile(name: string): { name: string; isDirectory: () => boolean; isFile: () => boolean } {
...
describe('ScheduleFileScanner', () => {
  ...
  describe('scanAll', () => {
    it('should scan all subdirectories with SCHEDULE.md files', async () => {
    ...
    it('should skip subdirectories that fail to parse', async () => {
      mockReaddir.mockResolvedValue([mockDir('valid'), mockDir('invalid')]);
      mockReadFile
  ...
  describe('writeTask', () => {
    it('should write a task to subdirectory with SCHEDULE.md', async () => {
    ...
    it('should call ensureDir before writing and create task subdirectory', async () => {
      const task: ScheduledTask = {
        id: 'schedule-test',
        name: 'Test',
        cron: '0 0 * * *',
  ...
  describe('deleteTask', () => {
    it('should delete a SCHEDULE.md inside subdirectory and return true', async () => {
...
describe('ScheduleFileWatcher', () => {
  ...
  describe('file event handling', () => {
    ...
    it('should ignore regular .md files that are not SCHEDULE.md', () => {
      eventCallback('change', 'daily-report/readme.md');
      vi.advanceTimersByTime(20);
    ...
    it('should not call onFileChanged when changed file fails to parse', async () => {
      ...
      eventCallback('change', 'bad-dir/SCHEDULE.md');
      vi.advanceTimersByTime(20);
      await vi.runAllTimersAsync();
