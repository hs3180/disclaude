...
/**
 * Helper: create a mock Dirent that simulates a directory entry.
 * Issue #2526: scanAll now uses readdir with { withFileTypes: true }.
 */
...
describe('ScheduleFileScanner', () => {
  ...
  describe('scanAll', () => {
    it('should scan all subdirectories with SCHEDULE.md', async () => {
      mockReaddir.mockResolvedValue([
        mockDirent('daily-report', true),
        mockDirent('weekly-summary', true),
        mockDirent('not-a-dir.txt', false),
      ]);
      mockReadFile.mockResolvedValue(makeScheduleContent());
  ...
  describe('writeTask', () => {
    it('should write a task with schedule- prefix stripped to subdirectory/SCHEDULE.md', async () => {
  ...
  describe('getFilePath', () => {
    it('should return subdirectory/SCHEDULE.md path from task ID', () => {
...
describe('ScheduleFileWatcher', () => {
  ...
  describe('file event handling', () => {
    ...
    it('should handle errors during file event processing gracefully', async () => {
      ...
      eventCallback('rename', 'error-schedule/SCHEDULE.md');
      vi.advanceTimersByTime(20);
      await vi.runAllTimersAsync();
      ...
      expect(onFileRemoved).toHaveBeenCalledWith(
        ...
        `${MOCK_DIR}/error-schedule/SCHEDULE.md`
      );
    });
    ...
    it('should handle change event errors gracefully', async () => {
      ...
      eventCallback('change', 'error-schedule/SCHEDULE.md');
      vi.advanceTimersByTime(20);
      await vi.runAllTimersAsync();
