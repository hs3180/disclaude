...
describe('ChatStore', () => {
  ...
  describe('initialization', () => {
    ...
    it('should load legacy passiveMode from disk (Issue #2193 migration)', async () => {
      const record = {
        chatId: 'oc_legacy',
        createdAt: new Date().toISOString(),
        expiresAt: new Date(Date.now() + 60_000).toISOString(),
      ...
      vi.mocked(fsPromises.readdir).mockResolvedValue(['oc_legacy.json'] as any);
      vi.mocked(fsPromises.readFile).mockResolvedValue(JSON.stringify(record));

      const freshStore = new ChatStore({ storeDir });
      const chat = await freshStore.getTempChat('oc_legacy');
      expect(chat).not.toBeNull();
