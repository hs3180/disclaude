 ...
 *
 * Schedules follow a subdirectory structure similar to skills:
 *
 * ```
 * schedules/
 * ├── daily-report/
 ...
 * Issue #2526: Reorganized to subdirectory + SCHEDULE.md structure.
 *
 ...
 * Issue #2526: Schedules now use subdirectory structure like skills.
 */
const SCHEDULE_FILENAME = 'SCHEDULE.md';

/**
 * Generate task ID from a directory name or file name.
 * When given a directory name like "daily-report", produces "schedule-daily-report".
 * When given a file name like "SCHEDULE.md", produces "schedule-SCHEDULE" (legacy fallback).
 * The preferred usage is to pass the parent directory name.
 */
function generateTaskId(dirOrFileName: string): string {
  const baseName = path.basename(dirOrFileName, '.md');
...
export class ScheduleFileScanner {
  ...
  /**
   * Scan all schedule subdirectories for SCHEDULE.md files and return parsed tasks.
   * Issue #2526: Schedules now use subdirectory structure: schedules/<name>/SCHEDULE.md
   */
   ...
   * Issue #2526: Task ID is derived from the parent directory name.
   */
  async parseFile(filePath: string): Promise<ScheduleFileTask | null> {
    try {
      ...
      // Derive task ID from parent directory name (e.g., schedules/daily-report/SCHEDULE.md → "schedule-daily-report")
      const dirName = path.basename(path.dirname(filePath));

      const task: ScheduleFileTask = {
        id: generateTaskId(dirName),
   ...
   * Issue #2526: Schedules use subdirectory structure: schedules/<name>/SCHEDULE.md
   */
  async writeTask(task: ScheduledTask): Promise<string> {
    ...
    const filePath = path.join(scheduleDir, SCHEDULE_FILENAME);
   ...
   * Issue #2526: Deletes the SCHEDULE.md inside the subdirectory.
   */
  async deleteTask(taskId: string): Promise<boolean> {
    ...
    const filePath = path.join(this.schedulesDir, slug, SCHEDULE_FILENAME);
   ...
   * Issue #2526: Returns path to schedules/<slug>/SCHEDULE.md.
   */
...
export class ScheduleFileWatcher {
   ...
   * Issue #2526: Supports recursive watching — events may come from subdirectories
   * with paths like "daily-report/SCHEDULE.md".
   */
  private handleFileEvent(eventType: string, filename: string | null): void {
    ...
    // For recursive watching, filename includes subdirectory path (e.g., "daily-report/SCHEDULE.md")
   ...
   * Issue #2526: Derives task ID from the subdirectory name in the path.
   */
  private async processFileEvent(eventType: string, filePath: string, filename: string): Promise<void> {
    // Derive task ID from subdirectory name
    // filename may be "daily-report/SCHEDULE.md" (recursive watch) or "SCHEDULE.md" (flat)
    const [firstPart] = filename.split(path.sep);
    const taskId = generateTaskId(firstPart);
