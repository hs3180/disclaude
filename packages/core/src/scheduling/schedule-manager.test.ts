 ...
 * Issue #2526: Creates subdirectory structure `<name>/SCHEDULE.md`.
 * The scanner generates task ID as `schedule-${subdirectoryName}`.
 */
...
Schedule: \`${cron}\`
`;
  // Issue #2526: Write to subdirectory/SCHEDULE.md
  const scheduleDir = path.join(tempDir, 'schedules', scheduleName);
  await fs.mkdir(scheduleDir, { recursive: true });
  await fs.writeFile(path.join(scheduleDir, 'SCHEDULE.md'), content, 'utf-8');
}

/** Derive the task ID from the schedule name (matches ScheduleFileScanner behavior) */
function taskId(scheduleName: string): string {
  // Accept both 'daily-report' and 'daily-report.md' for backward compat in test calls
  return `schedule-${scheduleName.replace(/\.md$/, '')}`;
}
...
describe('ScheduleManager', () => {
  ...
  describe('file system edge cases', () => {
    ...
    it('should handle directory with non-subdirectory files gracefully', async () => {
      await fs.writeFile(
...
# Bad Schedule
`;
      // Issue #2526: Write malformed SCHEDULE.md in a subdirectory
      const badDir = path.join(tempDir, 'schedules', 'bad-schedule');
