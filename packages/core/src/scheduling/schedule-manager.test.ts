 ...
 * Uses subdirectory layout: schedules/<dirName>/SCHEDULE.md
 */
...
# Bad Schedule
`;
      const badSubDir = path.join(tempDir, 'schedules', 'bad');
      await fs.mkdir(badSubDir, { recursive: true });
      await fs.writeFile(
        path.join(badSubDir, 'SCHEDULE.md'),
        malformedContent,
        'utf-8'
      );
