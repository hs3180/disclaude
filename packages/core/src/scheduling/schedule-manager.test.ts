 ...
 *
 * Issue #2526: Now writes to subdirectory structure:
...
Schedule: \`${cron}\`
`;
  const taskDir = path.join(tempDir, 'schedules', dirName);
  await fs.mkdir(taskDir, { recursive: true });
  await fs.writeFile(path.join(taskDir, 'SCHEDULE.md'), content, 'utf-8');
}
...
describe('ScheduleManager', () => {
  ...
  describe('file system edge cases', () => {
    ...
    it('should handle directory with non-schedule files gracefully', async () => {
      // A file (not directory) at the top level should be skipped
      await fs.writeFile(
