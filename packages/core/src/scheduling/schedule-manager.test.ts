 ...
 *
 * Issue #2526: Uses subdirectory layout `schedules/<slug>/SCHEDULE.md`.
 * The scanner generates task ID as `schedule-<slug>`.
 */
...
Schedule: \`${cron}\`
`;
  const dir = path.join(tempDir, 'schedules', slug);
  await fs.mkdir(dir, { recursive: true });
  await fs.writeFile(path.join(dir, 'SCHEDULE.md'), content, 'utf-8');
}

/** Derive the task ID from the slug (matches ScheduleFileScanner behavior). */
...
describe('ScheduleManager', () => {
  ...
  describe('file system edge cases', () => {
    ...
    it('should handle directory with non-schedule subdirectories gracefully', async () => {
      ...
      await fs.mkdir(path.join(tempDir, 'schedules', 'random-dir'), { recursive: true });
      await fs.writeFile(
        path.join(tempDir, 'schedules', 'random-dir', 'readme.txt'),
...
# Bad Schedule
`;
      const dir = path.join(tempDir, 'schedules', 'bad');
      await fs.mkdir(dir, { recursive: true });
      await fs.writeFile(
