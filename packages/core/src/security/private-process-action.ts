...
import type { PrivateAction } from './action-bound-input.js';

export interface PrivateProcessActionOptions {
  id: string;
  title: string;
  description: string;
  /** Executable and arguments are installed by the harness, never by a callback. */
  command: string;
 ...
 */
export function createPrivateProcessAction(options: PrivateProcessActionOptions): PrivateAction {
  ...
  return {
    ...
    consume: (value, context) => new Promise(resolve => {
      ...
      child.stdin?.on('error', () => { /* Consumer can reject input without reading. */ });
