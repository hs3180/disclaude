 ...
 */
export async function readProcessGroupResources(groupId: number): Promise<ProcessGroupResources> {
  ...
  try {
    ...
    const toolKinds: Record<string, number> = Object.create(null) as Record<string, number>;
    for (const line of stdout.split('\n')) {
      const row = /^\s*(\d+)\s+(\d+)\s+(\d+)\s+([\d:-]+)\s+(.+)$/.exec(line);
      if (!row || Number(row[2]) !== groupId) {continue;}
      processCount++;
      rssKiB += Number(row[3]);
      const age = row[4].split('-');
      const parts = (age.pop() as string).split(':').map(Number);
  ...
  } catch {return { groupId, available: false };}
}
