  ...
  queryStream(input: AsyncGenerator<UserInput>, options: AgentQueryOptions): StreamQueryResult {
    ...
    const adaptIterator = async function* (this: void): AsyncGenerator<AgentMessage> {
      ...
      const runInput = async (prompt: string): Promise<void> => {
        ...
        // Mark before acquiring the run slot: a queued turn is still active
        // user work and must not be selected as an LRU victim.
        governorSink.setSessionBusy(sessionKey, true);
        ...
        if (aborted || terminated) {
          ...
          lease.release();
          governorSink.setSessionBusy(sessionKey, false);
          return;
        }
        runActive = true;
