...
// ============================================================================
// Issue #2920: Stderr Capture and Startup Error Extraction
// ============================================================================
...
/**
 * Error with attached stderr output.
 *
 * Used to propagate captured stderr from the provider layer to upstream
 * consumers (e.g., ChatAgent.processIterator).
 */
export interface ErrorWithStderr extends Error {
  stderr?: string;
}

/**
 * Tag an existing Error with captured stderr.
 *
 * Mutates the error object to add a `stderr` property. Returns the same
 * error reference so callers can re-throw it directly.
 */
function attachStderr(error: Error, stderr: string): ErrorWithStderr {
  (error as ErrorWithStderr).stderr = stderr;
  return error as ErrorWithStderr;
}
 ...
 * "API 认证失败 (401)".
 *
 * The patterns are deliberately broad to avoid missing real errors.
 * When no pattern matches, the last few lines of stderr are returned
 * as a fallback, which is still better than the raw exit code.
 *
...
export function extractStartupDetail(stderr: string, fallbackMessage: string): string {
  ...
  // Pattern: MCP server configuration/initialization errors
  // Examples: "MCP server "amap-maps" failed to initialize"
  //           "Error initializing MCP server 'xxx'"
  //           "MCP server "xxx" configuration error"
  const mcpMatch = tailLines.match(
    /MCP server\s+["']([^"']+)["']\s+(?:failed|error|configuration)/i
  ) ?? tailLines.match(
    /(?:failed|error).*(?:initializing|configure?)\s+MCP\s+server\s+["']([^"']+)["']/i
  );
  if (mcpMatch) {
  ...
  // Examples: "authentication failed: 401 token expired or invalid"
  //           "401 Unauthorized: invalid API key"
  const authMatch = tailLines.match(
    /(?:authentication|auth|api\s+key).*(?:401|expired|invalid|unauthorized)/i
  ) ?? tailLines.match(
    /401\s+(?:unauthorized|token|authentication)/i
  );
  if (authMatch) {
  ...
  // Examples: "MCP server "playwright" timed out after 30s"
  const timeoutMatch = tailLines.match(
    /MCP server\s+["']([^"']+)["']\s+(?:timeout|timed?\s*out)/i
  );
  if (timeoutMatch) {
    return `MCP server "${timeoutMatch[1]}" 启动超时`;
  }

  // Pattern: command not found / empty command / ENOENT
  // Examples: "spawn ENOENT" "command undefined not found"
  const cmdMatch = tailLines.match(
    /(?:spawn|command).*(?:ENOENT|undefined|empty|not\s+found)/i
  ) ?? tailLines.match(/ENOENT/);
  if (cmdMatch) {
    return 'MCP 服务器 command 为空或不存在';
  }

  // Pattern: any explicit "Error:" line — take the first from the end
  ...
  async *queryOnce(
    ...
    try {
      ...
      if (stderr && error instanceof Error) {
        ...
        throw attachStderr(error, stderr);
      }
      throw error;
    }
  }
  ...
  queryStream(
    ...
    async function* adaptIterator(): AsyncGenerator<AgentMessage> {
      try {
        ...
        if (stderr && error instanceof Error) {
          logger.error(
            { err: error, messageCount, stderr },
            'adaptIterator error (stderr captured)'
          );
          throw attachStderr(error, stderr);
        }
