...
const cache = new Map<string, { expires: number; window?: number }>();

/** Never confuse output max_tokens with an input/context limit. */
export function readContextLimit(model: Record<string, unknown>): number | undefined {
...
export async function discoverCompactionWindow(
  ...
  try {
    ...
    logger.warn(
      { model },
      'Invalid model API URL; set agent.autoCompactWindow explicitly. No threshold was inferred.'
    );
    return undefined;
  }
  const key =
  ...
  try {
    ...
    for (const path of [`${modelsPath}/${encodeURIComponent(model)}`, modelsPath]) {
      ...
      const body = (await response.json()) as { id?: string; data?: Record<string, unknown>[] };
      const entry =
        body.id === model
          ? body
      ...
      if (limit) {
        ...
        logger.info(
          { model, contextLimit: limit, autoCompactWindow: window },
          'Resolved model context from provider API'
        );
        break;
      }
    }
  ...
  if (window === undefined) {
    logger.warn(
      { model },
      'Model API did not provide a context limit; set agent.autoCompactWindow explicitly. No guessed compaction threshold will be injected; SDK behavior is unchanged.'
    );
  }
  return window;
}
...
export function withDiscoveredCompaction(
  input: AsyncGenerator<UserInput>,
  options: AgentQueryOptions,
  start: (input: AsyncGenerator<UserInput>, options: AgentQueryOptions) => StreamQueryResult
): StreamQueryResult {
  const abort = new AbortController();
  let active: StreamQueryResult | undefined;
  return {
    handle: {
      close() {
        abort.abort();
