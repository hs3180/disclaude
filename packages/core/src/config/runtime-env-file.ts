...
export function readRuntimeFile(filePath: string): string {
...
export function writeRuntimeFile(filePath: string, content: string): void {
  if (Buffer.byteLength(content) > MAX_BYTES) {throw new Error('Runtime environment file is too large');}
