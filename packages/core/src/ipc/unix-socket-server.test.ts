...
describe('createInteractiveMessageHandler', () => {
  ...
  describe('sendMessage request', () => {
    ...
    it('should pass mentions to handler (Issue #1742)', async () => {
      const request = createRequest('sendMessage', 'req-5d', {
        chatId: 'chat-1',
        text: 'Hello',
        mentions: [{ userId: 'ou_bot1', name: 'Bot A' }],
      });
      const response = await handler(request);

      expect(container.handlers!.sendMessage).toHaveBeenCalledWith(
        'chat-1', 'Hello', undefined, [{ userId: 'ou_bot1', name: 'Bot A' }],
      );
      expect(response.success).toBe(true);
    });
  });
