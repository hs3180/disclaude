...
export class UnixSocketIpcServer {
  ...
  private async handleMessage(conn: IpcConnectionLike, data: string): Promise<void> {
    ...
    try {
      const chatId = (request.payload as Record<string, unknown>)?.chatId as string | undefined;
      const response = await withTiming(logger, 'ipc:handleRequest', chatId, () => this.handler(request));
