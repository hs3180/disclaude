...
export interface ChannelApiHandlers {
  ...
  insertDocxImage?: (documentId: string, imagePath: string, opts?: { index?: number; width?: number; height?: number; caption?: string }) => Promise<{ blockId: string; fileToken: string }>;
}
    ...
    try {
      switch (request.type) {
        ...
        case 'insertDocxImage': {
          ...
          const { documentId, imagePath, index, width, height, caption } =
            request.payload as IpcRequestPayloads['insertDocxImage'];
          try {
            const result = await handlers.insertDocxImage(documentId, imagePath, { index, width, height, caption });
            return { id: request.id, success: true, payload: { success: true, ...result } };
          } catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            return { id: request.id, success: false, error: errorMessage };
