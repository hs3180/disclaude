...
export interface ChannelApiHandlers {
  ...
  createGroup?: (params: { name: string; members: string[]; description?: string }) => Promise<{ chatId: string }>;
}
    ...
    try {
      switch (request.type) {
        ...
        case 'createGroup': {
          ...
          try {
            const result = await handlers.createGroup({ name, members, description });
