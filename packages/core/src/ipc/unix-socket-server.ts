...
export interface ChannelApiHandlers {
  ...
  /** Add members to a group chat (optional platform capability) */
  addMembers?: (chatId: string, memberIds: string[]) => Promise<{ success: boolean }>;
  /** Remove members from a group chat (optional platform capability) */
  removeMembers?: (chatId: string, memberIds: string[]) => Promise<{ success: boolean }>;
  /** Get members of a group chat (optional platform capability) */
  getMembers?: (chatId: string) => Promise<{ success: boolean; members: string[] }>;
  /** Get all chats the bot is in (optional platform capability) */
  getBotChats?: () => Promise<{ success: boolean; chats: Array<{ chatId: string; name: string }> }>;
}
    ...
    try {
      switch (request.type) {
        ...
        case 'getMembers': {
          ...
          if (!handlers.getMembers) {
            return {
              id: request.id,
              success: false,
              error: 'getMembers not supported by this channel',
            };
          }
          const { chatId } =
            request.payload as IpcRequestPayloads['getMembers'];
          try {
            const result = await handlers.getMembers(chatId);
        ...
        case 'getBotChats': {
          ...
          if (!handlers.getBotChats) {
            return {
              id: request.id,
              success: false,
              error: 'getBotChats not supported by this channel',
            };
          }
          try {
            const result = await handlers.getBotChats();
