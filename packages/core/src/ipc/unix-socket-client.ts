...
export class UnixSocketIpcClient {
   ...
   * Issue #2278: Inline image insertion for Feishu docs.
   *
   * @param params - Insert parameters (documentId, imagePath, index, width)
   */
  async insertDocxImage(
    ...
    try {
      return await this.request('insertDocxImage', params);
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error));
      logger.error({ err: error, params }, 'insertDocxImage failed');
