...
describe('MessageRouter', () => {
  describe('route() with UserMessage', () => {
    ...
    it('should pass chatType to handler (Issue #3641)', async () => {
      const handler = createMockHandler();
      const router = new MessageRouter({ handler });
      const msg = createUserMessage({ chatType: 'topic' });
  ...
  describe('route() with multiple messages', () => {
    ...
    it('should route messages to different chatIds', async () => {
      ...
      expect(handler.handleUserMessage).toHaveBeenNthCalledWith(
        1, 'oc_chatA', 'Hello!', 'feishu-msg-1', undefined, undefined, undefined, undefined
      );
      expect(handler.handleUserMessage).toHaveBeenNthCalledWith(
        2, 'oc_chatB', 'Hello!', 'feishu-msg-2', undefined, undefined, undefined, undefined
      );
    });
  });
