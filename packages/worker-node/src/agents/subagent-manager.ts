 ...
 * All subagents are ChatAgent instances — no type parameter needed.
 *
 * Features:
...
export interface SubagentOptions {
  ...
  /** Optional cron expression (metadata for scheduled subagents) */
  schedule?: string;
 ...
 *
 * Issue #2513: All subagents are ChatAgent instances created via
 * AgentFactory.createAgent(). There is no schedule/task distinction.
 *
 * @example
 * ```typescript
