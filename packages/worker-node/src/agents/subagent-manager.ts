 ...
 *
 * Issue #997: Provides a unified API for subagent creation.
 ...
 * Issue #2513: Removed schedule/task type distinction. There is only one
 * Agent type (ChatAgent). All subagents are created identically via
 ...
 *
 * Provides a unified interface for creating subagents via AgentFactory.createAgent().
 * Issue #2513: All subagents are ChatAgent instances created identically.
 * There is no type distinction between schedule and task subagents.
 *
 * @example
 * ```typescript
...
export class SubagentManager {
  ...
  async spawn(options: SubagentOptions): Promise<SubagentHandle> {
    const subagentId = `${options.name}-${randomUUID().slice(0, 8)}`;
  ...
  /**
   * Spawn a ChatAgent subagent in memory.
   *
   * All subagents are created identically via AgentFactory.createAgent()
   * regardless of their purpose (scheduled tasks, one-time tasks, etc.).
   */
