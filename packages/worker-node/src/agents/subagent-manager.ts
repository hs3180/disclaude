 ...
 *
 * Issue #997: Unifies subagent creation for all scenarios.
 ...
 * Issue #2513: Removed schedule/task type distinction — all subagents are
 * ChatAgent instances created identically via AgentFactory.createAgent().
 *
 * Features:
 ...
 * │        │                                                    │
 * │        ▼                                                    │
 * │   ┌─────────┐   All subagents are ChatAgent instances      │
 * │   │ Process │   created via AgentFactory.createAgent()     │
 * │   │ Manager │                                              │
 * │   └─────────┘                                              │
 * │                                                             │
 * │   list() ──► SubagentHandle[]                              │
 ...
 *
 * Issue #2513: Removed `type` field — all subagents are ChatAgent instances
 * created identically via AgentFactory.createAgent().
 *
 * @example
 * ```typescript
...
export interface SubagentOptions {
  ...
  /** Optional cron expression for context (informational only) */
  schedule?: string;
 ...
 *
 * All subagents are ChatAgent instances created identically via
 * AgentFactory.createAgent(). There is no distinction between
 * "schedule" and "task" subagents.
 *
...
export class SubagentManager {
   ...
   * Spawn a subagent.
   *
   * Issue #2513: No type parameter needed — all subagents are ChatAgent
   * instances created via AgentFactory.createAgent().
   *
  ...
  /**
   * Spawn a ChatAgent in memory for task execution.
   *
   * Issue #2513: Unified from the previous spawnScheduleAgent/spawnTaskAgent
   * methods which had identical implementations.
   */
