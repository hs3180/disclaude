 ...
 *   chatId: 'chat-123',
 *   callbacks: { sendMessage: async (chatId, text) => { ... } },
 * });
 * ```
 */
export class SubagentManager {
  ...
  /** Spawn and execute an agent in memory. Unified logic for both schedule and task types. */
  private async spawnInMemoryAgent(subagentId: string, options: SubagentOptions): Promise<void> {
    ...
    try {
      await agent.executeOnce(options.chatId, options.prompt, undefined, options.senderOpenId);
      handle.status = 'completed';
      handle.completedAt = new Date();
      logger.info({ subagentId, type: options.type }, 'Subagent completed');
    } catch (error) {
      handle.status = 'failed';
      handle.error = error instanceof Error ? error.message : String(error);
      handle.completedAt = new Date();
      logger.error({ err: error, subagentId, type: options.type }, 'Subagent failed');
    }
