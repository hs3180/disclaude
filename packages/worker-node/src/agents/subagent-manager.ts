 ...
 * Issue #1501: 'skill' type removed (handled via executeOnce or .md agents).
 *
 * @module agents/subagent-manager
 */

import { randomUUID } from 'crypto';
import type { ChatAgent } from '@disclaude/core';
import { AgentFactory } from './factory.js';
import {
  executeInMemoryAgent,
  logger,
  type SubagentHandle,
...
export class SubagentManager {
  ...
  async spawn(options: SubagentOptions): Promise<SubagentHandle> {
    ...
    try {
      const label = options.type === 'schedule' ? 'Schedule subagent' : 'Task subagent';
      const agent = AgentFactory.createAgent(options.chatId, options.callbacks);

      await executeInMemoryAgent(
        agent,
        handle,
        options,
        label,
        (h) => this.notifyStatusChange(h),
        (a) => this.inMemoryAgents.set(subagentId, a),
