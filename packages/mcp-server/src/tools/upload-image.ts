 ...
 *
 * Issue #1919 Phase 1: Upload an image to the platform and return the image_key.
 * The image_key can then be used in card JSON to embed images inside card messages.
 *
 ...
 * Issue #1919 Phase 1: Uses the dedicated uploadImage IPC message type.
 */
...
export async function upload_image(params: {
  ...
  try {
    ...
    if (!IMAGE_EXTENSIONS.includes(ext)) {
      return {
        success: false,
        error: `Not an image file: ${path.basename(resolvedPath)}`,
        message: `❌ Not an image file. Supported formats: ${IMAGE_EXTENSIONS.join(', ')}`,
      };
    }
    ...
    if (stats.size > MAX_IMAGE_SIZE) {
      ...
      return {
        ...
        message: `❌ Image file too large: ${sizeMB}MB (maximum 10MB)`,
      };
    }
    ...
    logger.info({ fileName, fileSize, chatId, imageKey }, 'Image uploaded successfully');
