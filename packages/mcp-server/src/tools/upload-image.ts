 ...
 *
 * Issue #1919: Uploads a local image file to Feishu and returns the image_key
 * that can be used in card img elements for inline image embedding.
 *
 * This enables Agents to generate images (e.g., charts, plots) and embed
 * them directly in card messages instead of sending them as separate files.
 *
 ...
 * Issue #1919: Routes through IPC to use Feishu im.image.create API.
 */
async function uploadImageViaIpc(
  ...
  if (!result.imageKey) {
    throw new Error('IPC upload succeeded but no image_key returned');
  }
  return {
