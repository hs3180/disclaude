...
describe('register_temp_chat', () => {
  ...
  describe('successful registration', () => {
    ...
    it('should pass triggerMode parameter to IPC (Issue #2069)', async () => {
      mockIpcClient.registerTempChat.mockResolvedValue({
        success: true, chatId: 'oc_test', expiresAt: '2026-04-05T12:00:00Z',
      });
      await register_temp_chat({
    ...
    it('should include trigger mode in success message (Issue #2069)', async () => {
      mockIpcClient.registerTempChat.mockResolvedValue({
        success: true, chatId: 'oc_test', expiresAt: '2026-04-05T12:00:00Z',
      });
    ...
    it('should not include trigger mode in message when undefined (Issue #2069)', async () => {
      mockIpcClient.registerTempChat.mockResolvedValue({
        success: true, chatId: 'oc_test', expiresAt: '2026-04-05T12:00:00Z',
      });
      const result = await register_temp_chat({
