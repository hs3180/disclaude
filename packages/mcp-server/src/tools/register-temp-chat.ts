 ...
 * @param params.triggerMode - Optional declarative trigger mode ('always' = respond to all, 'mention'/undefined = @mention only)
 */
export async function register_temp_chat(params: {
  ...
  try {
    ...
    const modeDesc = triggerMode === 'always' ? ', trigger mode: always' : triggerMode === 'mention' ? ', trigger mode: mention' : '';
    return {
      success: true,
