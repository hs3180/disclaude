...
- **triggerMode**: Set to \`'always'\` to respond to all messages. Default/\`'mention'\` = only respond to @mentions. (optional)
...
\`\`\``,
    parameters: z.object({
      ...
      triggerMode: z.enum(['mention', 'always']).optional().describe('Set to "always" to respond to all messages (default: "mention" = only @mentions)'),
    }),
