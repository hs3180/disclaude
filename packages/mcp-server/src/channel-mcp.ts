...
- **triggerMode**: Set to \`'always'\` to make bot respond to all messages. \`'mention'\` or undefined = bot only responds to @mentions. (optional)
...
\`\`\``,
    parameters: z.object({
      ...
      triggerMode: z.enum(['mention', 'always']).optional().describe("Set to 'always' to make bot respond to all messages, 'mention' for @mention only"),
    }),
