...
- **triggerMode**: Set to \`"always"\` to respond to all messages, \`"mention"\` to only respond to @mentions. Default = "mention". (optional)
...
\`\`\``,
    parameters: z.object({
      ...
      triggerMode: z.enum(['mention', 'always']).optional().describe('Trigger mode: "always" = respond to all, "mention" = @mention only'),
    }),
