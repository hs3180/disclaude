 ...
 *
 * Issue #1641: Validation failures and errors must use isError: true so the
 * agent knows the operation did NOT succeed. Previously, all failures used
 * toolSuccess() which misled the agent into thinking the operation completed.
 */
...
**Reference:** https://open.feishu.cn/document/common-capabilities/message-card/message-cards-content/using-markdown-tags`,
    ...
    handler: async ({ card, chatId, parentMessageId }: {
      ...
      // Issue #1641: Card validation failures must use toolError, not toolSuccess
...
\`\`\``,
    ...
    handler: async ({ question, options, chatId, title, context, actionPrompts, parentMessageId }: {
      ...
      // Issue #1355: Pre-validation to prevent message sending on invalid params
      // Issue #1641: Use toolError for all validation failures
