 ...
 *
 * @module channel-cli/utils/card-image-resolver
 */
import { promises as fsp } from 'fs';
import { resolve, extname } from 'path';
import { createLogger, uploadImage } from "../../../core/dist/index.js";
 ...
 */
async function uploadAndGetImageKey(filePath) {
    try {
 ...
 */
export async function resolveCardImages(card, 
...
onProgress) {
    ...
    function replaceImagePaths(obj) {
        ...
        if (record.tag === 'markdown' && typeof record.content === 'string') {
            record.content = record.content.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (match, alt, imagePath) => {
