 ...
 *
 * @module channel-cli/utils/card-validator
 */
/**
...
export function isValidFeishuCard(content) {
    if (!isRecord(content)) {
        return false;
    }
    ...
    if ('schema' in content && content.schema !== '1.0') {
        return false;
    }
 ...
 */
export function detectMarkdownTableWarnings(card) {
    const warnings = [];
    const { elements } = card;
    ...
    for (const element of elements) {
        if (typeof element === 'object' &&
            ...
            if (containsGfmTable(content)) {
                warnings.push('Card markdown element contains GFM table syntax (| col |), which is NOT supported by Feishu cards. ' +
 ...
 */
export function containsGfmTable(content) {
 ...
 */
export function getCardValidationError(content) {
    if (content === null) {
    ...
    const obj = content;
    if (obj.schema === '2.0') {
        if ('config' in obj && !isRecord(obj.config)) {
            return 'config must be an object';
        }
        if ('header' in obj && !isRecord(obj.header)) {
            return 'header must be an object with title when provided';
        }
        if (isRecord(obj.header) && !('title' in obj.header)) {
            return 'header.title is missing';
        }
        if (!('body' in obj)) {
            return 'missing required fields: body';
        }
        if (!isRecord(obj.body)) {
            return 'body must be an object with elements';
        }
        if (!Array.isArray(obj.body.elements)) {
            return 'body.elements must be an array';
        }
