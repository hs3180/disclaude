...
import { Config, MESSAGE_LOGGING, createLogger } from "../../../core/dist/index.js";
...
export class MessageLogger {
    ...
    async migrateLegacyFiles() {
        try {
            ...
            const legacyFlatFiles = entries.filter((entry) => entry.isFile() && entry.name.endsWith('.md'));
            // Find legacy chatId directories with date files inside
            const legacyChatDirs = entries.filter((entry) => entry.isDirectory() && !/^\d{4}-\d{2}-\d{2}$/.test(entry.name));
     ...
     */
    async logIncomingMessage(messageId, senderId, chatId, content, messageType, timestamp) {
        const entry = {
            messageId,
            senderId,
            chatId,
     ...
     */
    async logCardInteraction(messageId, senderId, chatId, content, timestamp) {
        const entry = {
            messageId,
            senderId,
            chatId,
     ...
     */
    async logOutgoingMessage(messageId, chatId, content, messageType = 'text', timestamp) {
        const entry = {
            messageId,
            senderId: 'bot',
            chatId,
     ...
     */
    async getChatHistory(chatId, maxLengthOverride) {
        try {
     ...
     */
    renderHistorySegments(segments, maxLength) {
     ...
     */
    async getChatLogFilePaths(chatId) {
        try {
            const entries = await fs.readdir(this.chatDir, { withFileTypes: true });
            const dateDirs = entries
                .filter((e) => e.isDirectory() && /^\d{4}-\d{2}-\d{2}$/.test(e.name))
    ...
    rememberProcessedId(id) {
        ...
        logger.debug({ evicted: count, remaining: this.processedMessageIds.size }, 'Trimmed processed-message dedup cache');
    }
