...
import { discoverConfigFile } from "../../core/dist/config/discovery.js";
export function workspaceSelection(value, home = homedir()) {
...
export async function validateWorkspaceSelection(path) {
    let parent = path;
    while (true) {
        try {
...
export async function setupWorkspaceOnFirstRun() {
    if (process.env.DISCLAUDE_WORKSPACE_DIR?.trim()) {
        return;
    }
    const found = discoverConfigFile();
    if (found.path && !found.exists) {
        throw new Error(`Configuration file does not exist: ${found.path}`);
    }
    ...
    if (workspace?.dir !== undefined) {
        return;
    } // Existing explicit settings remain authoritative, including invalid ones.
    const configuredEnv = config.env;
    if (typeof configuredEnv?.DISCLAUDE_WORKSPACE_DIR === 'string' && configuredEnv.DISCLAUDE_WORKSPACE_DIR.trim()) {
        return;
    }
    ...
    try {
        ...
        while (true) {
            ...
            try {
                ...
                console.log(`Cannot use this workspace: ${error.message}`);
                continue;
            }
            const confirm = await ui.question(`Use ${selected}? [y/N]: `, { signal: abort.signal });
            if (cancelled || !/^y(es)?$/i.test(confirm.trim())) {
                throw new Error('Workspace setup cancelled.');
            }
            ...
            if (latest !== original) {
                throw new Error('Configuration changed during setup. Please retry.');
            }
            ...
            try {
                ...
                } // exclusive publish for new installs
                else {
                    if ((await lstat(configPath)).isSymbolicLink()) {
                        throw new Error('Use the configuration file target directly, not a symlink, for setup.');
                    }
