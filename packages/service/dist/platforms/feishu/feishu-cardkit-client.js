...
export class FeishuCardKitClient {
     ...
     */
    updateElementContent(cardId, elementId, content, sequence, uuid) {
        return this.requestWithRefresh('PUT', `/cards/${encodeURIComponent(cardId)}/elements/${encodeURIComponent(elementId)}/content`, { content, sequence, uuid: uuid ?? randomUuid() });
    }
    /**
     ...
     * @param uuid - optional request id; generated if omitted
     */
    updateCard(cardId, card, sequence, uuid) {
     ...
     */
    finalizeStreaming(cardId, sequence, uuid, settings = { config: { streaming_mode: false } }) {
    ...
    async requestWithRefresh(method, path, body) {
        ...
        try {
            ...
            const freshToken = await (this.refreshPromise ??= this.onUnauthorized().then((token) => {
                this.refreshed = true;
                return token;
            }, (err) => {
    ...
    async request(method, path, body) {
        ...
        try {
            ...
            if (res.status === 401) {
                ...
                throw new CardKitClientError('Card Kit API rejected tenant_access_token (401 Unauthorized). Refresh LARKSUITE_CLI_TENANT_ACCESS_TOKEN.', 401, parsed);
            }
            if (!res.ok) {
                throw new CardKitClientError(`Card Kit ${method} ${path} failed: HTTP ${res.status}`, res.status, parsed);
            }
            ...
            if (typeof code === 'number' && code !== 0) {
                ...
                throw new CardKitClientError(`Card Kit ${method} ${path} failed: business code ${code} (${msg})`, res.status, parsed);
            }
            ...
            if (err instanceof Error && err.name === 'AbortError') {
                throw new CardKitClientError(`Card Kit ${method} ${path} aborted (timed out after ${this.timeoutMs}ms or cancelled)`, 0);
            }
            ...
            throw new CardKitClientError(`Card Kit ${method} ${path} request failed: ${err instanceof Error ? err.message : String(err)}`, 0);
        }
        finally {
 ...
 */
export function createCardKitClientFromEnv(options) {
    const tenantAccessToken = process.env.LARKSUITE_CLI_TENANT_ACCESS_TOKEN;
    if (!tenantAccessToken) {
        throw new Error('createCardKitClientFromEnv: LARKSUITE_CLI_TENANT_ACCESS_TOKEN is not set — Card Kit streaming cannot authenticate. Export it before constructing the client.');
    }
