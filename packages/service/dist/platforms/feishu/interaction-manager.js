...
export class InteractionManager {
     ...
     */
    async handleAction(event, defaultHandler) {
        ...
        if (!context) {
            logger.debug({ messageId: message_id, chatId: chat_id }, 'No registered interaction found for action');
        ...
        if (context.expectedActions.length > 0 && !context.expectedActions.includes(actionKey)) {
            logger.warn({ messageId: message_id, actionKey, expectedActions: context.expectedActions }, 'Unexpected action key');
            return false;
        }
