 ...
 */
export function decryptMessage(encryptedBase64, encodingAESKey) {
    try {
 ...
 */
export function encryptMessage(content, encodingAESKey, appId) {
    try {
        const { key, iv } = decodeAESKey(encodingAESKey);
        // Format: random(16) + msgLen(4) + msg + appId
 ...
 */
export function generateSignature(token, timestamp, nonce, encrypted) {
 ...
 */
export function verifySignature(signature, token, timestamp, nonce, encrypted) {
