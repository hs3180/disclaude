...
if (endpoint && process.env.DISCLAUDE_CHROMIUM_BINARY)
    throw new Error('Choose an existing endpoint or managed Chromium, not both');
mkdirSync(dirname(socketPath), { recursive: true, mode: 0o700 });
if ((statSync(dirname(socketPath)).mode & 0o077) !== 0)
    throw new Error('Socket parent directory must be private (0700)');
if (!statSync(cwd).isDirectory())
    throw new Error('Browser workspace must be a directory');
...
closeSync(lock);
const peers = new Set();
let managed, target, admin, coordinator, server, listening = false, stopping, browserStarting;
const startupAbort = new AbortController();
const delay = ms => new Promise(resolve => setTimeout(resolve, ms));
const event = record => {
    const line = JSON.stringify(record) + '\n';
    if (process.env.DISCLAUDE_BROWSER_EVENTS)
        appendFileSync(process.env.DISCLAUDE_BROWSER_EVENTS, line, { mode: 0o600 });
};
...
function reportFatal(type, reason) {
    ...
    try {
        event({ type, error: error.stack || error.message });
    }
    catch { /* Preserve the original fatal path if diagnostics cannot be written. */ }
    void shutdown().catch(shutdownError => {
        try {
            event({ type: 'service-shutdown-error', error: shutdownError.stack || shutdownError.message });
        }
...
try {
    if (!endpoint) {
        ...
        managed.child.once('exit', () => { if (!stopping)
    ...
    coordinator = new Coordinator({ url: info.webSocketDebuggerUrl, target, event, onTargetChange: value => { target = value; },
        ...
        cleanupWorker: options => { if (options?.runtime)
            rmSync(options.runtime, { recursive: true, force: true }); },
    ...
    server = createServer(peer => {
        ...
        const reply = (id, result, error) => { if (!peer.destroyed)
            peer.write(JSON.stringify({ id, ...(error ? { error } : { result }) }) + '\n'); };
        ...
        peer.on('close', () => {
            ...
            ticket?.cancel();
            if (lease)
                void coordinator.release(lease);
        });
        async function handle(message) {
            ...
            try {
                ...
                if (method === 'status')
                    reply(id, { state: coordinator.holder?.state || (coordinator.closed ? 'unavailable' : 'idle'), queued: coordinator.queue.length });
                else if (method === 'acquire') {
                    if (ready)
                        throw new Error('This connection already requested control');
                    ...
                    if (!ready)
                        throw new Error('Acquire first');
                    reply(id, await ready);
                }
                else if (method === 'heartbeat') {
                    if (!lease)
                        throw new Error('No held lease');
                    ...
                    if (!lease)
                        throw new Error('No held lease');
                    if (typeof message.script !== 'string' || message.script.length > 1024 * 1024)
                        throw new Error('Invalid script');
                    const executionCwd = message.cwd === undefined ? cwd : message.cwd;
                    if (typeof executionCwd !== 'string' || !executionCwd.startsWith('/') || !statSync(executionCwd).isDirectory())
                        throw new Error('Execution cwd must be an existing absolute directory');
                    reply(id, await coordinator.execute(lease, 'script', { code: message.script, cwd: executionCwd }));
                }
                else if (method === 'release') {
                    if (!lease) {
                        ticket?.cancel();
                        reply(id, { released: false });
                    }
                    else
                        reply(id, { released: await coordinator.release(lease) });
                }
                else
