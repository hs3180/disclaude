...
export async function diagnoseBrowser({ binary, headless = false, signal }) {
    if (!binary || !isAbsolute(binary))
        throw new Error('doctor requires --binary with an absolute executable path');
    ...
    let browser, client;
    const checkCancelled = () => { if (signal?.aborted)
        throw new Error('Browser diagnosis cancelled'); };
    const stop = async () => {
        try {
            if (client) {
    ...
    try {
        for (let cycle = 0; cycle < 2; cycle++) {
            ...
            for (let attempt = 0; attempt < 30; attempt++) {
                ...
                if (dom.result.value === marker)
                    break;
                await new Promise(resolve => setTimeout(resolve, 100));
            }
            if (dom.result.value !== marker)
                throw new Error('Browser navigation/input probe failed');
            const screenshot = await client.call('Page.captureScreenshot', { format: 'png' }, sessionId);
            if (!Buffer.from(screenshot.data, 'base64').subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10])))
                throw new Error('Browser screenshot probe failed');
            if (cycle === 0) {
                ...
                if (!cookie.success)
                    throw new Error('Browser could not set a temporary test cookie');
            }
            ...
            if (cycle === 0 && !retained)
                throw new Error('Browser could not read its running-session test cookie');
            if (cycle === 1)
                report.cookiePersistence = retained ? 'retained' : 'not-retained';
