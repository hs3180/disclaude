...
export async function connectBrowser(socketPath) {
    ...
    const rejectAll = error => { for (const item of pending.values()) {
        clearTimeout(item.timer);
        item.reject(error);
    } pending.clear(); };
    ...
    return {
        request(method, args = {}) {
            if (socket.destroyed)
                return Promise.reject(new Error('Browser IPC closed'));
...
export async function main() {
    ...
    if (process.argv.length > 2)
        throw new Error('Coordinated browser-use accepts stdin scripts only; daemon lifecycle is owned by the service');
    if (!process.env.DISCLAUDE_BROWSER_SOCKET)
        throw new Error('DISCLAUDE_BROWSER_SOCKET is required');
    let code = '';
    ...
    if (!code.trim())
        throw new Error('Pipe a Python browser-use script on stdin');
...
export async function withBrowserLease(client, execute, onQueued = () => { }) {
    ...
    try {
        ...
        heartbeat = setInterval(() => {
            client.request('heartbeat').catch(() => { if (!releasing)
                client.close(); });
        }, 1000);
        await execute();
