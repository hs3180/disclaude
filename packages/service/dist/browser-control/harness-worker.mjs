...
const send = message => { if (process.connected)
        ...
        if (message.kind === 'init') {
            ...
            for (let attempt = 0; attempt < 60; attempt++) {
                ...
                if (daemon.exitCode !== null)
                    throw new Error('Harness exited during startup');
                await new Promise(resolve => setTimeout(resolve, 100));
            }
            throw new Error('Harness IPC readiness timeout');
        }
        else if (message.kind === 'execute') {
            if (message.command !== 'script' || typeof message.value?.code !== 'string')
