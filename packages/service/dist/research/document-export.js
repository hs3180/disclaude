export function resultParagraphs(project, at) {
    ...
    for (const direction of project.directions) {
        ...
        for (const finding of direction.findings) {
            ...
            for (const source of finding.sources) {
                lines.push(source.title, source.location, source.excerpt);
            }
            if (finding.caveat) {
                lines.push(`限制或反证：${finding.caveat}`);
            }
        }
    }
    ...
    for (const feedback of project.feedback) {
        lines.push(`${states[feedback.status]}：${feedback.text}`, feedback.reason ?? '');
    }
    const chunks = lines.flatMap(line => line.split('\n')).filter(line => line.trim()).flatMap(line => {
        const chars = Array.from(line), chunks = [];
        for (let i = 0; i < chars.length; i += 2000) {
            chunks.push(chars.slice(i, i + 2000).join(''));
        }
        return chunks;
    });
    const paragraphs = [];
    for (const chunk of chunks) {
        ...
        if (last >= 0 && paragraphs[last].length + chunk.length + 1 <= 2000) {
            paragraphs[last] += `\n${chunk}`;
        }
        else {
    ...
    if (paragraphs.length > 50 || paragraphs.join('\n').length > 48_000) {
        throw new Error('成果超过单次文档追加限制；完整成果仍保留在项目中。');
    }
    return paragraphs;
}
export async function documentDeadline(operation) {
    let timer;
    try {
