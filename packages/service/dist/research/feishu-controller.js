...
const page = (value) => typeof value === 'number' && Number.isSafeInteger(value) && value >= 0 ? value : 0;
function callbackValue(action) {
    if (object(action.value).research === true) {
        return object(action.value);
    }
    const name = string(action.name);
    if (name.startsWith('research:') && name.length <= 500) {
        try {
            return { ...object(JSON.parse(name.slice(9))), research: true };
        }
        catch { /* Invalid form identity. */ }
    }
    return {};
}
...
export class FeishuResearchController {
    ...
    resolveWorkingDir;
    manager;
    constructor(directory, workspace, send, update, runner = createResearchRunner(workspace), readDocument, appendDocument, resolveWorkingDir) {
        this.send = send;
        this.resolveWorkingDir = resolveWorkingDir;
        if (!isAbsolute(directory)) {
            throw new Error('Research project storage must use an absolute directory');
        }
        this.manager = new ResearchManager(new ProjectStore(directory), runner, async (project) => {
            const card = projectCard(project);
            if (project.cardId) {
                await update(project.cardId, card);
                return project.cardId;
            }
            const id = await send({ chatId: project.chat, type: 'card', card, threadId: project.thread });
            if (!id) {
                throw new Error('Research project card delivery returned no message ID');
            }
            return id;
        }, readDocument, appendDocument);
    }
    /** Actor identity comes only from the receiving channel, never operation JSON. */
    async executeTask(context, operation) {
        const { owner, chat, source, thread } = context;
        if (!owner || !chat || !source) {
            throw new Error('Task actor context unavailable');
        }
        ...
        if (operation.action === 'create') {
            ...
            if (!this.resolveWorkingDir) {
                throw new Error('Project directory resolver unavailable');
            }
        ...
        if (operation.action === 'control') {
            ...
            if (current.revision !== operation.revision) {
                throw new Error('Task revision changed; read current state before retrying');
            }
    ...
    static isCallback(raw) {
        return callbackValue(object(raw.action)).research === true;
    }
    async open(owner, chat, thread) {
        if (!owner || !chat) {
            throw new Error('无法确认任务的用户和会话。');
        }
        await this.showIndex(owner, chat, thread);
    }
    async showIndex(owner, chat, thread, offset = 0, archived = false) {
        let workingDir, error;
        try {
            workingDir = await this.resolveWorkingDir?.(chat);
        }
        catch {
            error = '当前项目目录不可用，暂不能建立新任务。请用 /project info 检查，或用 /project use <目录> 切换。已有任务仍可打开。';
        }
        ...
        try {
            ...
            if (actionName === 'create') {
                ...
                if (!nonce || nonce.length > 100) {
                    throw new Error('创建表单已失效，请重新打开任务。');
                }
                ...
                if (existing) {
                    await this.manager.show(existing.id, owner, chat);
                    return;
                }
            ...
            if (actionName === 'preview-project-link' || actionName === 'confirm-project-link') {
                if (!this.resolveWorkingDir || typeof value.revision !== 'number') {
                    throw new Error('当前项目目录不可用，请检查目录绑定。');
                }
                ...
                if (actionName === 'preview-project-link') {
                    ...
                    await this.manager.confirmProjectLink(id, owner, chat, value.revision, string(value.token), directory);
                }
                return;
            }
            if (actionName === 'unlink-project' && typeof value.revision === 'number') {
                await this.manager.unlinkProject(id, owner, chat, value.revision);
                return;
            }
            if (['open', 'refresh'].includes(actionName)) {
                await this.manager.show(id, owner, chat, actionName === 'open' ? { thread: message } : undefined);
                return;
            }
            if (actionName === 'evidence') {
                await this.send({ chatId: chat, type: 'card', threadId: project.thread, card: evidenceCard(project, string(value.direction), page(value.index)) });
                return;
            }
            if (actionName === 'history') {
                await this.send({ chatId: chat, type: 'card', threadId: project.thread, card: historyCard(project, page(value.offset)) });
                return;
            }
            if (actionName === 'continue' || actionName === 'continue-finding') {
                if (!['completed', 'cancelled'].includes(project.status)) {
                    throw new Error('请先结束当前任务，再从成果建立后续任务。');
                }
            ...
            const actions = ['pause', 'resume', 'cancel', 'feedback', 'stop-direction', 'archive', 'unarchive', 'export'];
            if (!actions.includes(actionName) || typeof value.revision !== 'number') {
                throw new Error('任务操作无效，请刷新任务。');
            }
            await this.manager.act(id, owner, chat, value.revision, actionName, actionName === 'feedback' ? string(form.feedback) : string(value.direction));
        }
        catch (error) {
