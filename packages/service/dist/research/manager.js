...
import { changedDocumentFeedback, documentToken } from './document-source.js';
...
export class ResearchManager {
    ...
    disposed = false;
    exporting = new Set();
    constructor(store, runner, publish, readDocument, appendDocument) {
        this.store = store;
        this.runner = runner;
        this.publish = publish;
        this.readDocument = readDocument;
        this.appendDocument = appendDocument;
    }
    load() {
        if (this.disposed) {
            throw new Error('任务服务已停止。');
        }
        if (this.loaded) {
            return;
        }
    ...
    project(id) {
        ...
        if (!project) {
            throw new Error('任务不存在或不属于当前用户和会话。');
        }
        return project;
    }
    get(id, owner, chat) {
        ...
        if (!p || p.owner !== owner || p.chat !== chat) {
            throw new Error('任务不存在或不属于当前用户和会话。');
        }
        return structuredClone(p);
    }
    list(owner, chat, archived = false) {
        this.load();
    ...
    async create(input) {
        ...
        if (existing) {
            await this.show(existing.id, input.owner, input.chat);
        ...
        if (parent && !['completed', 'cancelled'].includes(parent.status)) {
            throw new Error('请先结束原任务，再从成果继续任务。');
        }
        ...
        if (workingDir !== undefined && !isAbsolute(workingDir)) {
            throw new Error('任务工作目录必须是绝对路径。');
        }
        const now = new Date().toISOString();
        const token = documentToken(input.documentUrl ?? '');
        if (token && !this.readDocument) {
            throw new Error('当前任务服务未配置文档读取能力。');
        }
        const p = { ...input, workingDir, projectLink: parent?.projectLink ? { directory: parent.projectLink.directory, token: randomUUID() } : undefined, id: randomUUID(), status: 'paused', revision: 0, createdAt: now, updatedAt: now,
    ...
    checkLegacyLink(p) {
        if (p.workingDir) {
            throw new Error('该任务已有固定项目目录，不支持通过历史关联迁移。');
        }
        if (this.running.has(p.id) || ['running', 'pausing', 'cancelling'].includes(p.status)) {
            throw new Error('请先暂停任务并等待当前回合结束，再调整项目关联。');
        }
    }
    async previewProjectLink(id, owner, chat, revision, directory) {
        this.get(id, owner, chat);
        const p = this.project(id);
        this.checkLegacyLink(p);
        if (p.revision !== revision) {
            throw new Error('任务已更新，请刷新后操作。');
        }
        if (p.projectLink) {
            throw new Error('该历史任务已有关联，请先撤销原关联。');
        }
        if (!isAbsolute(directory)) {
            throw new Error('项目关联目录必须是绝对路径。');
        }
    ...
    async confirmProjectLink(id, owner, chat, revision, token, directory) {
        this.get(id, owner, chat);
        const p = this.project(id);
        if (p.projectLink?.token === token) {
            await this.display(p);
            return;
        }
        this.checkLegacyLink(p);
    ...
    async unlinkProject(id, owner, chat, revision) {
        this.get(id, owner, chat);
        const p = this.project(id);
        this.checkLegacyLink(p);
        if (p.revision !== revision) {
        ...
        p.projectLink = undefined;
        p.linkPreview = undefined;
        this.record(p, '已撤销项目导航关联。任务身份、文件及成果保留。');
        await this.display(p);
    }
    async show(id, owner, chat, reopen) {
        this.get(id, owner, chat);
        const project = this.project(id);
        const oldCard = project.cardId, oldThread = project.thread;
        if (reopen) {
            project.cardId = undefined;
            project.thread = reopen.thread;
        }
        await this.display(project);
        if (reopen && project.deliveryError) {
            project.cardId = oldCard;
            project.thread = oldThread;
            this.store.save(project);
        }
    }
    async act(id, owner, chat, revision, action, value = '') {
        this.get(id, owner, chat);
        const p = this.project(id);
        if (revision !== p.revision) {
            throw new Error('项目已更新，请刷新后操作。');
        }
        if (action === 'export') {
            await this.exportResult(p);
            return;
        }
        if (action === 'archive' || action === 'unarchive') {
            if (!['completed', 'cancelled'].includes(p.status)) {
                throw new Error('请先结束任务，再归档任务。');
            }
        ...
        if (['completed', 'cancelled'].includes(p.status)) {
            throw new Error('任务已结束，可从成果创建后续任务。');
        }
        if (action === 'pause') {
            if (p.status !== 'running') {
                throw new Error('当前任务未在执行。');
            }
            ...
            if (!['paused', 'failed', 'interrupted', 'waiting-user'].includes(p.status) || this.running.has(id)) {
                throw new Error('请等待当前回合收尾后再恢复。');
            }
            if (p.status === 'waiting-user' && p.feedback.length <= (p.clarificationFeedbackCount ?? p.feedback.length)) {
                throw new Error('请先提交任务所需的补充信息，再继续。');
            }
            p.status = 'running';
            ...
            if (!value.trim() || value.length > 3000) {
                throw new Error('请填写 3000 字以内的范围调整或补充意见。');
            }
    ...
    async exportResult(p) {
        ...
        try {
            ...
            if (latest.token !== document.token) {
                throw new Error('Document binding mismatch');
            }
            if (!reconcile) {
                if (latest.fingerprint !== operation.baseFingerprint) {
                    operation.status = 'conflict';
                    operation.error = '文档已有新修改，本次未追加成果。原成果和文档均保留，请从成果继续任务以处理新意见。';
            ...
            if (index < 0 || raw.indexOf(operation.fragment, index + operation.fragment.length) >= 0) {
                ...
                operation.error = '尚无法确认上次写入；本次没有重复追加。请检查文档，稍后再次核对，项目成果始终保留。';
            }
            else {
                document.publishedFragments = [...fragments, operation.fragment];
                if (latest.fingerprint !== operation.baseFingerprint) {
                    operation.status = 'conflict';
                    operation.error = '成果快照已追加，但检测到并发修改。双方内容均保留，请从成果继续任务以处理新意见。';
                }
                else {
    ...
    display(p) {
        ...
        const next = previous.then(async () => {
            ...
            try {
                ...
                p.deliveryError = '项目卡片更新失败；可用 /project 重新打开，已有进度保留。';
            }
            if (!this.disposed) {
                this.store.save(p);
            }
        ...
        void next.finally(() => { if (this.publishing.get(p.id) === next) {
            this.publishing.delete(p.id);
        } }).catch(() => { });
        return next;
    }
    start(p) {
        if (this.running.has(p.id) || this.disposed || p.status !== 'running') {
            return;
        }
    ...
    async syncDocument(p) {
        ...
        try {
            ...
            const snapshot = await Promise.race([this.readDocument(document.token, document.publishedFragments), new Promise((_resolve, reject) => {
            ...
            if (changes.length) {
                ...
                document.generation++;
                p.feedback.push(...changes.map(change => ({ text: change.text, status: 'pending', at: snapshot.syncedAt,
    ...
    applyCheckpoint(p, snapshot, result, pending, feedbackCount) {
        ...
        if (result.feedback.some(f => !expected.includes(f.feedbackIndex))) {
        ...
        if (result.state === 'waiting-user') {
            pending.filter(f => f.status === 'pending' || f.status === 'needs-clarification').forEach(f => {
                f.status = 'needs-clarification';
                f.reason = result.clarification;
            });
    ...
    async execute(p, signal) {
        try {
            while (!this.disposed && p.status === 'running') {
                await this.syncDocument(p);
                if (this.disposed || p.status !== 'running') {
                    break;
                }
                if (p.stepCount >= 12) {
                    p.status = 'paused';
            ...
            if (!this.disposed) {
                p.status = p.status === 'cancelling' ? 'cancelled' : 'failed';
    ...
    async idle(id) { await this.running.get(id)?.done; }
    dispose() {
        if (this.disposed) {
            return;
        }
