 ...
 *
 * Part of the DisclaudeService architecture.
 *
 * @module @disclaude/service
 */
...
export class ChannelManager {
     ...
     */
    setupHandlers(channel, messageHandler, controlHandler) {
        channel.onMessage(async (message) => {
            ...
            try {
                ...
                logger.error({ channelId: channel.id, messageId: message.messageId, error }, 'Failed to handle channel message');
            }
        });
     ...
     * Issue #1594: Added for DisclaudeService integration.
     */
     ...
     */
    registerChatId(chatId, channel) {
        const existing = this.chatIdChannelMap.get(chatId);
        if (existing && existing.id !== channel.id) {
            logger.debug({ chatId, fromChannelId: existing.id, toChannelId: channel.id }, 'ChatId ownership reassigned to different channel');
        }
     ...
     */
    resolveChannelForChatId(chatId) {
        ...
        for (const channel of this.channels.values()) {
            if (channel.ownsChatId(chatId)) {
                logger.info({ chatId, channelId: channel.id, channelName: channel.name }, 'Resolved channel ownership via ownsChatId query (map miss)');
    ...
    async broadcast(message) {
        ...
        const results = await Promise.allSettled(Array.from(this.channels.values()).map(async (channel) => {
            try {
                ...
                logger.warn({ channelId: channel.id, chatId: message.chatId, error }, 'Channel failed to send message');
                throw error;
            }
        }));
        // Log any failures
        ...
        results.forEach((result, index) => {
            if (result.status === 'rejected') {
                logger.warn({ channelId: channelArray[index].id, chatId: message.chatId }, 'Message delivery failed');
            }
        });
    }
    /**
    ...
    async stopAll() {
        ...
        const failures = results.filter((r) => r.status === 'rejected');
        if (failures.length > 0) {
            logger.warn({ failureCount: failures.length, total: this.channels.size }, 'Some channels failed to stop during shutdown');
        }
    }
    /**
