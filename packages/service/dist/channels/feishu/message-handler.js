...
import { FeishuPrivateWorkflows } from './private-workflows.js';
...
export class MessageHandler {
    ...
    privateInput;
    agentInput;
    async requestAgentInput(request, context) {
        if (!this.agentInput) {
            throw new Error('Feishu input channel is unavailable');
        }
    ...
    research;
    projectTaskAppId;
    projectTaskGrantNamespace = crypto.randomUUID();
    taskContext(owner, chat, source, thread) {
        const controller = this.research;
        if (!controller || !owner) {
            return undefined;
        }
        try {
            return projectTaskGateway.issue(this.projectTaskGrantNamespace, operation => controller.executeTask({ owner, chat, source, thread }, operation));
        }
        catch {
    ...
    constructor(options) {
        ...
        if (options.privateInput) {
            this.privateInput = new FeishuPrivateInput(options.privateInput, options.callbacks.sendMessage);
        }
    ...
    initialize(client) {
        ...
        const researchDirectory = process.env.DISCLAUDE_RESEARCH_PROJECTS_DIR
            || path.join(Config.getWorkspaceDir(), '.disclaude', 'project-tasks', 'feishu', crypto.createHash('sha256').update(this.projectTaskAppId).digest('hex').slice(0, 24));
        if (!this.research) {
            this.research = new FeishuResearchController(researchDirectory, Config.getWorkspaceDir(), this.callbacks.sendMessage, async (messageId, card) => {
                ...
                if (result.code !== 0) {
                    throw new Error('任务卡片更新失败，已有进度保留。');
                }
                ...
                try {
                    if (!(await fs.stat(result.projectContext.workingDir)).isDirectory()) {
                        throw new Error('Not a directory');
                    }
                }
                catch {
                    throw new Error('当前项目工作目录不存在或不可访问，请恢复目录后再建立任务。');
                }
        ...
        try {
            ...
            if (message_type === 'image' || message_type === 'file' || message_type === 'media' || message_type === 'audio' || message_type === 'video') {
                ...
                if (sender?.sender_type === 'user') {
                    const context = this.taskContext(extractOpenId(sender), chat_id, message_id, fileMetadata.threadRootId);
                    if (context) {
                        fileMetadata.projectTaskContext = context;
                    }
                }
            ...
            if (/^\/private(?:\s|$)/.test(textWithoutMentions.trim())) {
                ...
                try {
                    if (!this.privateInput) {
                        throw new Error('No private input consumer');
                    }
            ...
            if (sender?.sender_type === 'user') {
                ...
                if (context) {
                    metadata.projectTaskContext = context;
                }
            }
    ...
    async handleCardAction(data) {
        ...
        if (FeishuPrivateInput.isPrivateCallback(rawData)) {
            ...
            void this.privateWorkflows.submit(rawData).then(handled => {
                if (!handled) {
                    return this.privateInput?.submit(rawData);
                }
                return undefined;
