...
export class FileStorageService {
    ...
    constructor(config) {
        this.config = config;
        if (!Number.isSafeInteger(config.maxFileSize) || config.maxFileSize < 1) {
    ...
    async initialize() {
        ...
        for (const entry of await readdir(this.root, { withFileTypes: true })) {
            ...
            if ((await stat(metadata)).size > 16_384) {
                throw new Error('Stored file metadata is oversized');
            }
    ...
    shutdown() { this.initialized = false; this.files.clear(); }
    async storeFromBase64(base64, fileName, mimeType, source = 'user') {
        if (!this.initialized) {
        ...
        if (!/^[A-Za-z0-9+/]*={0,2}$/u.test(normalized) || normalized.length % 4 === 1
            || normalized.length > Math.ceil(this.config.maxFileSize / 3) * 4) {
        ...
        const ref = { id, fileName, mimeType, source, size: content.length, createdAt: Date.now(), localPath: join(directory, 'content') };
        const record = { version: 1, ref, sha256: digest(content) };
    ...
    async getContent(id) {
        const record = this.files.get(id);
        if (!record || !uuid.test(id)) {
            throw new Error('File not found');
        }
        const content = await readFile(join(this.root, id, 'content'));
        if (content.length !== record.ref.size || digest(content) !== record.sha256) {
            throw new Error('Stored file content failed integrity verification');
        }
        return content.toString('base64');
    }
}
export const persistentFileStorageProvider = () => Promise.resolve({ FileStorageService });
