...
import { Config, WS_HEALTH, createLogger, BaseChannel, eventBus, DEFAULT_CHANNEL_CAPABILITIES, attachmentManager, } from "../../../core/dist/index.js";
import { InteractionManager, createFeishuClient, } from '../platforms/feishu/index.js';
import { TriggerModeManager, MentionDetector, WelcomeHandler, MessageHandler as FeishuMessageHandler, messageLogger, WsConnectionManager, } from './feishu/index.js';
import { VIDEO_EXTENSIONS, extractVideoCover } from '../utils/video-cover-extractor.js';
import { IMAGE_EXTENSIONS, EXT_TO_FEISHU_FILE_TYPE, MAX_IMAGE_SIZE, MAX_FILE_SIZE, uploadImage, uploadFile, } from '../utils/feishu-upload.js';
import { extractCardTextContent } from '../platforms/feishu/card-builders/card-text-extractor.js';
import { buildTextContent, normalizeCardMarkdown, normalizeMarkdownLineBreaks, } from '../platforms/feishu/card-builders/content-builder.js';
// Issue #4400 (#4208 P2-c): Card Kit streaming wiring.
import { createCardKitClientFromEnv, } from '../platforms/feishu/feishu-cardkit-client.js';
import { buildStreamingPlaceholderCard, STREAMING_REPLY_ELEMENT_ID, STREAMING_THINKING_ELEMENT_ID, } from '../platforms/feishu/card-builders/streaming-card-builder.js';
import { configuredFeishuMessageBytes, FEISHU_RETRY_MESSAGE_BYTES, truncateFeishuMessage, } from './feishu-message-chunker.js';
 ...
 */
export function extractChatIdFromEvent(data) {
    const raw = data;
    if (!raw) {
        return undefined;
    }
...
export function extractFeishuApiError(err) {
    ...
    const body = (response?.data ?? e.data ?? e);
...
export class FeishuChannel extends BaseChannel {
    ...
    streamingCardKitClient;
    streamingMessageIds = new Map();
    streamingSequences = new Map();
    deliveryHealth = {
        status: 'unknown',
        attempts: 0,
        successes: 0,
     ...
     */
    offlineQueue = [];
    requestPrivateWorkflow(request) {
        return this.feishuMessageHandler.requestPrivateWorkflow(request);
    }
    requestAgentInput(request, context) {
    ...
    async doStart() {
        ...
        const eventDispatcher = new lark.EventDispatcher({}).register({
            'im.message.receive_v1': async (data) => {
                try {
                    await this.feishuMessageHandler.handleMessageReceive(data);
                }
                catch (error) {
                    logger.error({ err: error }, 'Failed to handle message receive');
                    await this.notifyUserDirectly(extractChatIdFromEvent(data) ?? '', '⚠️ 处理消息时遇到内部错误，请稍后重试。');
                }
            },
            'card.action.trigger': async (data) => {
                try {
                    await this.feishuMessageHandler.handleCardAction(data);
                }
                catch (error) {
                    logger.error({ err: error }, 'Failed to handle card action');
                    await this.notifyUserDirectly(extractChatIdFromEvent(data) ?? '', '⚠️ 处理卡片操作时遇到错误，请稍后重试。');
                }
            },
            ...
            'im.chat.access_event.bot_p2p_chat_entered_v1': async (data) => {
                try {
                    await this.welcomeHandler.handleP2PChatEntered(data);
                }
                catch (error) {
            ...
            'im.chat.member.added_v1': async (data) => {
                try {
                    await this.welcomeHandler.handleChatMemberAdded(data);
                }
                catch (error) {
        ...
        // Create SDK logger
        const sdkLogger = {
            error: (...msg) => logger.error({ context: 'LarkSDK' }, String(msg)),
            warn: (...msg) => logger.warn({ context: 'LarkSDK' }, String(msg)),
            info: (...msg) => logger.info({ context: 'LarkSDK' }, String(msg)),
            debug: (...msg) => logger.debug({ context: 'LarkSDK' }, String(msg)),
            trace: (...msg) => logger.trace({ context: 'LarkSDK' }, String(msg)),
        };
    ...
    async doSendMessageInternal(message) {
        if (!this.client) {
         ...
         */
        const sendFeishuMessage = async (msgType, content) => {
            if (useThreadReply) {
            ...
            const createResp = await client.im.message.create({
                ...
                data: {
                    ...
                    ...(useThreadReply ? { root_id: message.threadId } : {}),
                },
            });
         ...
         */
        const logOutgoing = (msgId, contentSummary, msgType) => {
            if (!msgId) {
                return;
            }
            messageLogger
        ...
        switch (message.type) {
            ...
            case 'card': {
                const messageId = await sendFeishuMessage('interactive', JSON.stringify(normalizeCardMarkdown(message.card || {})));
                ...
                const rawCardDescription = message.description || (message.card ? extractCardTextContent(message.card) : '[card]');
            ...
            case 'file': {
                ...
                if (isImage) {
                    ...
                    if (!imageKey) {
                        ...
                        fileMessageId = await sendFeishuMessage('media', JSON.stringify({ file_key: fileKey, image_key: imageKey }));
                    }
                    logger.info({
                        chatId: message.chatId,
                        messageId: fileMessageId,
     ...
     */
    buildPostContentWithMentions(mentions, text) {
    ...
    getCardKitClient() {
        ...
        try {
            ...
            logger.warn({ err }, 'startStreaming: Card Kit client unavailable (tenant token missing?) — declining to sendMessage');
            return null;
        }
    }
     ...
     */
    async startStreaming(chatId, parentMessageId) {
        ...
        try {
            ...
            if (!cardId) {
                logger.warn({ chatId, created }, 'startStreaming: createCard returned no card_id — declining to sendMessage');
                return null;
            }
            ...
            if (sent.data?.message_id) {
                this.streamingMessageIds.set(cardId, sent.data.message_id);
            }
            // createCard carries no sequence; the first PUT/PATCH uses sequence 1.
            this.streamingSequences.set(cardId, 0);
            logger.info({ chatId, cardId, parentMessageId }, 'startStreaming: streaming card created and sent to chat');
            return cardId;
        }
        catch (err) {
    ...
    async finalizeStreaming(id) {
        ...
        try {
            if (!client) {
                throw new Error('Streaming client unavailable during finalization');
            }
     ...
     */
    async notifyUserDirectly(chatId, text) {
    ...
    queueOfflineMessage(message) {
        ...
        if (this.offlineQueue.length >= maxSize) {
            ...
            logger.warn({ chatId: dropped?.message.chatId, type: dropped?.message.type }, 'Offline queue full, dropping oldest message');
        }
        this.offlineQueue.push({ message, queuedAt: Date.now() });
        logger.info({ chatId: message.chatId, type: message.type, queueSize: this.offlineQueue.length }, 'Message queued (WebSocket reconnecting)');
    }
    /**
    ...
    async flushOfflineQueue() {
        ...
        const valid = queue.filter((entry) => {
            ...
            if (age > maxAge) {
                logger.debug({ chatId: entry.message.chatId, type: entry.message.type, ageMs: age, maxAgeMs: maxAge }, 'Discarding expired offline message');
                return false;
            }
            return true;
        });
        ...
        for (const entry of valid) {
            try {
                ...
                logger.error({ err: error, chatId: entry.message.chatId, type: entry.message.type }, 'Failed to send queued message');
            }
        }
