...
function takePrefixByBytes(text, maxBytes) {
    let result = '';
    let bytes = 0;
...
function takeSuffixByBytes(text, maxBytes) {
...
export function truncateFeishuMessage(text, maxBytes = DEFAULT_FEISHU_MESSAGE_BYTES) {
    if (maxBytes <= 0) {
...
export function configuredFeishuMessageBytes(env = process.env) {
