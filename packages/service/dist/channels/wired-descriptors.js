 ...
 */
import { createInboundAttachment, Config, ActionBoundInput, createPrivateProcessAction, createLogger, } from "../../../core/dist/index.js";
...
import { createChannelCallbacksFactory, createDefaultMessageHandler, createChannelApiHandlers, } from '../utils/channel-handlers.js';
import { buildInteractiveCard, buildActionPrompts, validateInteractiveParams, } from '../platforms/feishu/card-builders/index.js';
// ============================================================================
 ...
 */
export const REST_WIRED_DESCRIPTOR = {
    type: 'rest',
    name: 'REST API',
    ...
    createCallbacks: (channel, context) => createChannelCallbacksFactory(channel, context.logger, {
        sendDoneSignal: true,
        getChatHistory: (chatId) => messageLogger.getChatHistory(chatId),
        getChatLogFilePaths: (chatId) => messageLogger.getChatLogFilePaths(chatId),
    }),
    createMessageHandler: (channel, context) => createDefaultMessageHandler(channel, context, {
     ...
     */
    setup: (channel, _config, context) => {
        const restChannel = channel;
 ...
 */
export function createConfiguredPrivateInput() {
...
export const FEISHU_WIRED_DESCRIPTOR = {
    type: 'feishu',
    name: 'Feishu',
    ...
    createMessageHandler: (channel, context) => createDefaultMessageHandler(channel, context, {
        ...
        extractAttachments: (message) => {
            ...
            return attachments?.map((att) => createInboundAttachment(att.fileName, chatId, messageType, {
    ...
    setup: (channel, config, context) => {
        ...
        config.resolveActionPrompt = (messageId, chatId, actionValue, actionText) => contextStore.generatePrompt(messageId, chatId, actionValue, actionText);
        ...
        const triggerModeAdapter = {
            getMode: (chatId) => triggerModeManager.getMode(chatId),
            setMode: (chatId, mode) => triggerModeManager.setMode(chatId, mode),
        };
        ...
        const feishuHandlers = {
            ...
            pushToAgent: async (chatId, message, options) => {
        ...
        context.service.registerFeishuHandlers(feishuHandlers);
 ...
 */
export const WECHAT_WIRED_DESCRIPTOR = {
    type: 'wechat',
    name: 'WeChat',
    ...
    setup: (channel, _config, context) => {
        ...
        // Register with DisclaudeService for REST API routing
        context.service.registerChannelHandlers('wechat', wechatHandlers, wechatChannel);
 ...
 */
export const BUILTIN_WIRED_DESCRIPTORS = [
