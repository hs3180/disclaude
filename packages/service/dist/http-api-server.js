...
import { isPrivateWorkflowRequest } from './channels/feishu/private-workflows.js';
/**
...
export class HttpApiServer {
    ...
    sendMessageHandler;
    sendCardHandler;
    privateWorkflowHandler;
    setPrivateWorkflowHandler(handler) {
     ...
     */
    async handleProjectTask(req, res) {
        if (!this.config.apiToken) {
            this.sendJson(res, 503, { ok: false, message: 'Project tasks require API authentication' });
            return;
        }
        ...
        try {
            ...
            if (Object.keys(raw).some(key => !['context', 'operation'].includes(key))
                || typeof raw.context !== 'string' || !/^[a-f0-9-]{36}$/u.test(raw.context)) {
                throw new Error('Invalid context');
            }
            ({ context } = raw);
            operation = parseTaskOperation(raw.operation);
        }
        catch {
            this.sendJson(res, 400, { ok: false, message: 'Invalid project task request' });
            return;
        }
        try {
            this.sendJson(res, 200, { ok: true, result: await projectTaskGateway.execute(context, operation) });
        }
        catch (error) {
            // Do not echo the opaque context or leak task state on a rejected operation.
            this.sendJson(res, error instanceof TaskContextError ? 403 : 409, { ok: false, message: error instanceof TaskContextError ? 'Project task context expired or unavailable' : 'Task operation rejected; refresh task state or project binding' });
        }
    }
    async handlePrivateWorkflow(req, res) {
        // This endpoint selects executable code. Never expose it in unauthenticated API mode.
        if (!this.config.apiToken) {
            this.sendJson(res, 503, { ok: false, message: 'Private workflows require API authentication' });
            return;
        }
        if (!this.privateWorkflowHandler) {
            this.sendJson(res, 503, { ok: false, message: 'Private workflows unavailable' });
            return;
        }
        let body;
        try {
            ...
            this.sendJson(res, 400, { ok: false, message: 'Invalid private workflow request' });
            return;
        }
        if (!isPrivateWorkflowRequest(body)) {
            this.sendJson(res, 400, { ok: false, message: 'Invalid private workflow request' });
            return;
        }
        try {
            this.sendJson(res, 200, { ok: true, ...await this.privateWorkflowHandler(body) });
        }
        catch {
            this.sendJson(res, 500, { ok: false, message: 'Private workflow request failed' });
        }
    }
