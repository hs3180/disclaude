 ...
 */
import { buildSessionKey, chatIdOfSessionKey, createLogger, getProvider, Config, resolveAgentPreset } from "../../core/dist/index.js";
 ...
 */
function busyTurnGuardKey(sessionKey, turnStartedAtMs) {
...
export class ChatSessionPool {
    ...
    releaseChatAgent(chatId, sessionId, agent) {
        const key = this.sessionKeyOf(chatId, sessionId);
        if (this.agents.get(key) !== agent) {
            return;
        }
        try {
            agent.reset();
            ...
            this.reset(chatId, false, sessionId);
        }
    }
    /** Keyed by buildSessionKey(chatId, threadRootId) — see class doc (Issue #4587 part 2). */
    agents = new Map();
    callbacksBySession = new Map();
    selectedPresetBySession = new Map();
    presets;
    options;
    /**
    ...
    createAgent(chatId, callbacks, preset, skipHistory = false, sdkSessionKey = chatId, modelOverride, freezeDirectory = false) {
        ...
        if (freezeDirectory) {
            ...
            if (directory !== undefined) {
                ...
                cwdResolver = () => {
                    let available = false;
                    try {
                        available = statSync(directory).isDirectory();
                    }
                    catch { /* fail closed */ }
     ...
     */
    sessionKeyOf(chatId, threadRootId) {
     ...
     */
    isAgentBusy(chatId, threadRootId) {
     ...
     */
    getOrCreateChatAgent(chatId, callbacks, threadRootId, session) {
        ...
        if (!agent) {
            ...
            const selected = this.presets
                ...
                : undefined;
            agent = this.createAgent(chatId, callbacks, selected?.ok ? selected.preset : undefined, skipHistory, sessionKey, session?.model, session !== undefined);
     ...
     */
    reset(chatId, skipContext, threadRootId) {
        const sessionKey = this.sessionKeyOf(chatId, threadRootId);
    ...
    async steer(chatId, prompt, threadRootId) {
        const agent = this.agents.get(this.sessionKeyOf(chatId, threadRootId));
        if (!agent?.isBusy) {
     ...
     */
    enforceBusyTurnCap(sessionKey, agent, now) {
        const cap = this.options.busyTurnHardCapMs ?? DEFAULT_BUSY_TURN_HARD_CAP_MS;
        if (cap <= 0) {
            return;
        } // Cap disabled — no tracking, never stop.
        ...
        if (turnStarted > 0) {
            ...
            if (this.busyTurnStoppedFor.has(guardKey)) {
                return;
            }
        ...
        logger.warn({ chatId, sessionKey, busyMinutes: busyMin, capMs: cap }, 'Busy turn exceeded hard cap (Issue #4577) — stopping query');
        agent.stop(chatId);
