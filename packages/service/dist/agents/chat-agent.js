...
export class ChatAgent extends BaseAgent {
    ...
    async processIterator(iterator) {
        ...
        try {
            ...
            if (!this.stoppedQueryGenerations.has(myGeneration)) {
                ...
                if (isStartupFailure(messageCount, elapsedMs)) {
                    ...
                    await this.deliverUserVisible(chatId, `❌ Agent 启动失败: ${diagnosticMessage}\n\n` +
                        '这是一次配置或环境错误，重试无法解决。\n' +
                ...
                // Notify user about the error
                {
                    ...
                    await this.deliverUserVisible(chatId, `❌ Session error: ${iteratorError.message}`, threadRoot);
                }
        ...
        if (!decision.allowed) {
            ...
            this.logger.error({ chatId, reason: decision.reason, restartCount: decision.restartCount }, 'Restart blocked by circuit breaker');
        ...
        this.logger.warn({ chatId, error: errorMessage, restartCount: decision.restartCount, waitMs: decision.waitMs }, 'Agent loop ended unexpectedly, attempting restart with backoff');
