...
import { BaseAgent, MessageBuilder, MessageChannel, RestartManager, ConversationOrchestrator, EmptyTurnRetryPolicy, getErrorStderr, isStartupFailure, tagErrorCategory, StreamingReplyDriver, REACTIONS, TurnSupersededError, } from "../../../core/dist/index.js";
 ...
 */
function extractSendHttpStatus(err) {
...
function isOversizedFeishuMessageError(err) {
    if (!err || typeof err !== 'object') {
        return false;
    }
    const e = err;
...
function midstreamRetryDelayMs() {
...
export class ChatAgent extends BaseAgent {
    ...
    sessionGeneration = 0;
    stoppedQueryGenerations = new Set();
    ...
    sendCircuitOpen = false;
    activeLifecycleContext;
    pendingLifecycleContexts = [];
    didDeliverUserVisibleThisTurn = false;
    ...
    emptyTurnRetryPolicy = new EmptyTurnRetryPolicy();
    ...
    midstreamAutoRetryAvailable = true;
    ...
    taskCompletionResolve;
    taskCompletionReject;
    ...
    updateCallbacks(callbacks) {
        ...
        this.logger.info({ chatId: this.boundChatId }, 'Agent is busy, deferring callback update until current query completes');
        ...
        void this.taskCompletionPromise
            ...
            if (this.callbacks !== pendingCallbacks) {
                ...
                this.logger.info({ chatId: this.boundChatId }, 'Deferred callback update applied after query completion');
            }
        });
        return false;
    }
     ...
     */
    createTurnCompletion(messageId) {
        ...
        const entry = { promise, settle, settled: false };
     ...
     */
    async runOnce(chatId, text, messageId, senderOpenId) {
        // Issue #644: Verify chatId matches bound chatId
        if (chatId !== this.boundChatId) {
            this.logger.error({ boundChatId: this.boundChatId, receivedChatId: chatId }, 'runOnce called with wrong chatId');
    ...
    async processMessage(params) {
        const { chatId, payload: text, messageId, senderOpenId, attachments, chatHistoryContext, chatType, threadContext, threadRootId, } = params;
        // Issue #644: Verify chatId matches bound chatId
        if (chatId !== this.boundChatId) {
            this.logger.error({ boundChatId: this.boundChatId, receivedChatId: chatId }, 'processMessage called with wrong chatId - this should not happen');
            return;
        }
        ...
        if (!this.isSessionActive) {
            ...
            this.startAgentLoop();
            if (!this.isSessionActive) {
                this.logger.error({ chatId, messageId }, 'Message rejected because the bound project directory is unavailable');
                return;
            }
        }
        ...
        if (!queuedBehindActiveTurn) {
            this.activeLifecycleContext = lifecycleContext;
        }
        if (this.pendingLifecycleContexts.length > 50) {
            this.pendingLifecycleContexts.splice(0, this.pendingLifecycleContexts.length - 50);
        }
        ...
        if (this.channel) {
            ...
            if (queuedBehindActiveTurn) {
                void this.callbacks.sendMessage(chatId, '⏳ 当前回合仍在执行；这条消息已排队，将在当前回合结束后处理。使用 `/stop` 可停止当前回合；`/steer` 会报告后端的即时纠偏能力。', threadRootId).catch((error) => {
                    this.logger.warn({ err: error, chatId, messageId }, 'Failed to send queued-message notice');
                });
            }
        }
        else {
     ...
     */
    tryPushMessage(message, chatId, messageId) {
        if (!this.channel) {
    ...
    startAgentLoop() {
        ...
        this.taskCompletionPromise = new Promise((resolve, reject) => {
        ...
        this.processIterator(iterator).catch(async (err) => {
            ...
            try {
                await this.callbacks.sendMessage(chatId, '❌ 处理消息时发生严重错误，会话已中断。请发送 /reset 重置会话后重试。');
            }
            catch (notifyErr) {
     ...
     */
    async deliverUserVisible(chatId, content, threadRoot, receipt) {
        ...
        const recordDeliveryEvent = (event, fields) => {
        ...
        if (this.sendCircuitOpen) {
            this.logger.debug({ chatId, contentLength: content.length }, 'Send skipped: delivery circuit open for this session (Issue #4626)');
        ...
        try {
            ...
            const isPermanentTargetError = httpStatus !== undefined &&
                httpStatus >= 400 &&
                httpStatus < 500 &&
     ...
     */
    notifyDebugGroupOfDeliveryFailure(chatId, err, httpStatus, isPermanentTargetError) {
        let debugChatId;
        try {
        ...
        const reason = isPermanentTargetError
            ...
            : '连续投递失败';
        const message = `🚫 [投递熔断] Agent 输出无法送达 chat ${chatId}（${reason}），会话保持存活。` +
        ...
        void this.callbacks.sendMessage(debugChatId, message).catch((fwdErr) => {
            this.logger.debug({ err: fwdErr, debugChatId }, 'Failed to forward delivery-circuit notice to debug group (Issue #4626)');
        });
    }
    /**
    ...
    async processIterator(iterator) {
        const chatId = this.boundChatId;
        ...
        const resolveReplyThreadRoot = () => consumeTurnAnchor() ?? this.conversationOrchestrator.getThreadRoot(chatId);
        ...
        const streamDriver = !!streamCapabilities?.supportsStreaming &&
            this.chatType === 'p2p' &&
            !!startStreaming &&
        ...
        try {
            for await (const { parsed } of iterator) {
                ...
                if (this.abortController?.signal.aborted) {
                    this.logger.info({ chatId, messageCount, type: parsed.type }, 'Aborting processIterator: stop/reset signal received');
                    break;
                }
                ...
                if (!firstMessageMs) {
                    ...
                    this.logger.info({ chatId, ttftMs: firstMessageMs - startTime, type: parsed.type }, 'First SDK message received (TTFT)');
                }
                ...
                if (parsed.type === 'tool_use') {
                    ...
                    lastToolCallMs = now;
                    this.logger.info({ chatId, toolCallCount, sinceLastToolMs: sinceLastTool, elapsedMs: now - startTime }, 'Tool call received');
                }
                ...
                if (parsed.content && parsed.terminatedReason !== 'turn_failed') {
                    ...
                    // 投递。
                    const isIntermediateMessage = parsed.type === 'tool_use' ||
                    ...
                    if (isIntermediateMessage) {
                        this.logger.debug({ chatId, messageCount, type: parsed.type, provider: this.sdkProvider.name }, 'Filtered tool trace from user chat');
                    }
                        ...
                        const suppressFakeComplete = visibleContent.startsWith('✅ Complete') && sawMidstreamInterrupt;
                        if (suppressFakeComplete) {
                            toDeliver = '';
                            this.logger.info({ chatId, messageCount }, 'Mid-stream interruption: suppressing fake ✅ Complete summary ' +
                        ...
                        if (toDeliver) {
                            if (streamDriver && isAssistantReplyText) {
                                ...
                                delivered = await this.deliverUserVisible(chatId, toDeliver, threadRoot, isAssistantReplyText ? (id) => { finalDeliveryId = id; } : undefined);
                            }
                ...
                if (parsed.type === 'result') {
                    if (streamDriver && !(await streamDriver.finish(resolveReplyThreadRoot()))) {
                        turnDeliveryFailed = true;
                        finalDeliveryId = undefined;
                        this.logger.error({ chatId, turnMessageId: currentTurnMessageId, ...this.activeLifecycleContext }, 'Streaming terminal delivery failed after fallback');
                    }
                    ...
                    if (parsed.terminatedReason === 'turn_failed') {
                        ...
                        await this.deliverUserVisible(chatId, `❌ 本轮 ${this.sdkProvider.name === 'codex' ? 'Codex' : this.sdkProvider.name} 执行失败，未生成可交付结果。请稍后重试；若持续失败，请检查模型服务、凭据和超时配置。`, resolveReplyThreadRoot());
                    }
                    ...
                    if (parsed.terminatedReason === 'stall') {
                        ...
                        this.logger.warn({ chatId, messageCount }, 'GLM stall: stream terminated by no-content-progress watchdog; recording failure, resolving turn');
                        this.restartManager.recordFailure(chatId, 'stall');
                        this.isProcessingMessage = false;
                        const stallError = new Error(parsed.terminationDetail ?? 'Provider stall watchdog terminated the turn');
                    ...
                    if (parsed.terminatedReason === 'empty-stream') {
                        ...
                        this.logger.warn({ chatId, messageCount }, 'Empty stream: turn terminated by provider after in-request retries exhausted ' +
                    ...
                    if (parsed.terminatedReason === 'evicted') {
                        ...
                        this.logger.info({ chatId, messageCount }, 'Codex session evicted (concurrency cap) — ending stream without auto-restart (Issue #4634)');
                        this.isProcessingMessage = false;
                        this.resolveTurn(currentTurnMessageId);
                        if (this.callbacks.onDone) {
                    ...
                    const willRetryEmptyTurn = isEmptyTurn &&
                        !upstreamApiError &&
                    ...
                    if (isEmptyTurn && !upstreamApiError && !sawMidstreamInterrupt && !willRetryEmptyTurn) {
                        ...
                        await this.deliverUserVisible(chatId, '⚠️ 本轮未产生任何可见输出，会话可能已失效。请重新发送消息触发重试；若持续无响应，请尝试重置会话。', emptyTurnThreadRoot);
                    }
                    ...
                    if (willRetryEmptyTurn && turnMessage) {
                        ...
                        this.logger.warn({ chatId, messageId: turnMessage.messageId, replaySeq }, 'Empty turn on a real-user message (Issue #4391): scheduling one-shot ' +
                        ...
                        setTimeout(() => {
                            ...
                            (async () => {
                                ...
                                const effectiveReplayParams = reInjected
                                    ...
                                    : replayParams;
                                if (reInjected) {
                                    this.logger.info({ chatId, messageId: replayParams.messageId }, 'Re-injected chat history into empty-turn replay context ' +
                                ...
                                void this.processMessage(effectiveReplayParams).catch((replayErr) => {
                                    this.logger.error({ err: replayErr, chatId }, 'Empty-turn replay processMessage failed (Issue #4391)');
                                });
                            })().catch((schedErr) => {
                                this.logger.error({ err: schedErr, chatId }, 'Empty-turn replay scheduling callback failed (Issue #4391)');
                            });
                        }, 0);
                    }
                    ...
                    if (upstreamApiError) {
                        ...
                        try {
                            ...
                            this.logger.warn({ err: notifyErr, chatId }, 'Failed to send upstream-API-error notice (Issue #4322)');
                        }
                    }
                    ...
                    if (midstreamInterrupted) {
                        const willAutoContinue = this.midstreamAutoRetryAvailable &&
                            !this.onceMode &&
                        ...
                        if (willAutoContinue) {
                            ...
                            this.logger.warn({ chatId, messageCount, stopReason: parsed.metadata?.stopReason }, 'Mid-stream interruption: auto-continuing this session once after ' +
                            ...
                            setTimeout(() => {
                                ...
                                if (this.disposed || this.messageSeq !== autoReplaySeq) {
                                    this.logger.info({ chatId, autoReplaySeq, currentSeq: this.messageSeq }, 'Mid-stream auto-continue dropped (agent disposed or a newer message arrived)');
                                    return;
                                }
                                const nudge = {
                                    chatId,
                                ...
                                void this.processMessage(nudge).catch((err) => {
                                    this.logger.error({ err, chatId }, 'Mid-stream auto-continue processMessage failed');
                                });
                            }, autoRetryDelayMs);
                        }
                        else {
                            this.logger.warn({ chatId, messageCount, onceMode: this.onceMode }, 'Mid-stream interruption but cannot auto-continue ' +
                                '(retry budget exhausted / once-mode / session closed): reporting ❌');
                            try {
                                const midstreamThreadRoot = resolveReplyThreadRoot();
                                await this.deliverUserVisible(chatId, '❌ 本轮被上游中断（响应中途停滞），输出不完整，任务可能未完成。请重新发送消息重试。', midstreamThreadRoot);
                            }
                            catch (notifyErr) {
                                this.logger.warn({ err: notifyErr, chatId }, 'Failed to send mid-stream-interruption notice');
                            }
                        }
                    }
                    ...
                    if (!isEmptyTurn && !upstreamApiError && !midstreamInterrupted &&
                        ...
                        finalDeliveryId = undefined;
                        const outcome = await addCompletionReaction(() => addReaction(messageId, REACTIONS.COMPLETE), () => this.sessionGeneration === myGeneration && !this.abortController?.signal.aborted);
            ...
            if (!this.stoppedQueryGenerations.has(myGeneration)) {
                ...
                if (isStartupFailure(messageCount, elapsedMs) && !classification.transient && classification.category !== 'UNKNOWN') {
                    ...
                    await this.deliverUserVisible(chatId, `❌ Agent 启动失败（${classification.category}）。诊断 ID: ${diagnosticId}\n\n` +
                        '请检查配置、权限和运行环境。\n' +
                        '请检查上述错误信息，修复后发送 /reset 重置会话。', threadRoot);
                ...
                // Notify user about the error
                {
                    ...
                    await this.deliverUserVisible(chatId, `❌ 本次请求中断，结果可能不完整。诊断 ID: ${diagnosticId}。请先核对已经执行的操作，再决定是否重新提交。`, threadRoot);
                }
        ...
        if (this.stalledTerminated) {
            ...
            this.isSessionActive = false;
            this.isProcessingMessage = false;
            this.logger.info({ chatId, messageCount }, 'GLM stall: terminated turn ended; suppressing auto-restart, context preserved');
            return;
        }
        ...
        if (this.emptyStreamTerminated) {
            ...
            this.isSessionActive = false;
            this.isProcessingMessage = false;
            this.logger.info({ chatId, messageCount }, 'Empty stream: terminated turn ended; suppressing auto-restart, context preserved (Issue #4442)');
            return;
        }
        ...
        if (this.sessionGeneration !== myGeneration && !wasExplicitClose) {
            this.logger.info({ chatId, messageCount, myGeneration, currentGeneration: this.sessionGeneration }, 'Empty-turn reset+replay: superseded session iterator ended; suppressing unexpected-end path (Issue #4391)');
            return;
        }
        ...
        if (iteratorError) {
            ...
            const terminalRuntimeCategory = category === 'CONFIGURATION' ||
                category === 'VALIDATION' ||
                category === 'PERMISSION';
            if (!transient && terminalRuntimeCategory) {
                this.logger.info({ chatId, errorCategory: category, errorMessage }, 'Non-transient runtime error: terminating session without restart (Issue #4989)');
                return;
            }
        }
        ...
        if (!decision.allowed) {
            ...
            this.logger.error({ chatId, diagnosticId, reason: decision.reason, restartCount: decision.restartCount }, 'Restart blocked by circuit breaker');
        ...
        this.logger.warn({ chatId, diagnosticId, error: errorMessage, restartCount: decision.restartCount, waitMs: decision.waitMs }, 'Agent loop ended unexpectedly, attempting restart with backoff');
        ...
        if (this.sessionGeneration !== myGeneration || this.isSessionActive || this.stoppedQueryGenerations.has(myGeneration)) {
            return;
        }
        const threadRoot = resolveReplyThreadRoot();
    ...
    async steer(prompt) {
        ...
        try {
            const acknowledgement = await handle.steer(prompt);
