 ...
 * try {
 *   await agent.processMessage({ chatId, payload: prompt, messageId });
 ...
 */
import { Config, createLogger, } from "../../../core/dist/index.js";
 ...
 *
 * Issue #1412: Removes duplicate empty implementations from disclaude service.
 ...
 * ```
 */
export function toChatAgentCallbacks(callbacks) {
    return {
...
export class AgentFactory {
     ...
     * ```
     */
    static createChatAgent(name, ...args) {
        if (name === 'pilot') {
            ...
            // (ChatSessionPool.getOrCreateChatAgent) always injects one when wired
            ...
            if (!options.cwdProvider) {
                logger.warn({ chatId }, 'AgentFactory.createChatAgent called without cwdProvider; agent will run in the workspace cwd and ignore any /project binding');
            }
     ...
     * ```
     */
    static createAgent(chatId, callbacks, options = {}) {
        const baseConfig = this.getBaseConfig(options);
        ...
        // omits it (every spawn path injects one via ChatSessionPool); warn so a
        // future caller does not silently trip the workspace fallback.
        if (!options.cwdProvider) {
            logger.warn({ chatId }, 'AgentFactory.createAgent called without cwdProvider; agent will run in the workspace cwd and ignore any /project binding');
        }
        const config = {
            ...baseConfig,
            chatId,
