 ...
 *
 * @module service/testing/browser-use-e2e
 */
 ...
 */
export function parseE2EReport(reply) {
    const out = {};
 ...
 */
export function evaluateE2EReport(reply, workspaceDir) {
    const report = parseE2EReport(reply);
    const results = [];
    const push = (c, passed, evidence) => results.push({ id: c.id, label: c.label, passed, evidence: clip(evidence) });
    for (const check of E2E_CHECKS) {
        ...
        switch (check.id) {
            ...
            case 'screenshot_artifact': {
                ...
                const isPng = size > 0 && readFileSync(abs).subarray(0, 4).equals(Buffer.from([0x89, 0x50, 0x4e, 0x47]));
                push(check, size > 0 && isPng, `${abs} — ${size} bytes${isPng ? ', PNG magic ok' : ', NOT a PNG'}`);
                break;
            }
            case 'cdp_failure_explicit': {
                const looksLikeError = /(refus|unreach|tim(e)?d? ?out|error|fail|closed|reset|econn)/i.test(raw);
                push(check, raw !== 'EMPTY' && raw.length > 0 && looksLikeError, raw === 'EMPTY' || raw.length === 0
    ...
    const lines = [
        ...
        '',
        ...results.map((r) => `- [${r.passed === true ? 'PASS' : r.passed === false ? 'FAIL' : 'N/A '}] ${r.id.padEnd(22)} ${r.evidence}`),
    ];
