...
export class DisclaudeService extends EventEmitter {
    ...
    constructor(config = {}) {
        ...
        for (const key of Object.keys(config)) {
            if (key !== 'instanceId' && key !== 'interactiveContextFile') {
    ...
    async sendInteractive(chatId, params) {
        ...
        if (resolvedPrompts && result.messageId) {
            this.interactiveContextStore.register(result.messageId, chatId, resolvedPrompts, Object.fromEntries(params.options.map(option => [option.value, option.text])));
        }
