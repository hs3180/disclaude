#!/usr/bin/env node
/**
 * CLI entry point for @disclaude/service
 *
 * Usage:
 *   disclaude start [--config PATH]
 *
 * This starts the disclaude service with a REST channel for API access.
...
import { startBrowserRuntime } from './browser-control/runtime.js';
import crypto from 'node:crypto';
import { DisclaudeService } from './service.js';
import { HttpApiServer } from './http-api-server.js';
import { ChatSessionPool } from './chat-session-pool.js';
...
export function resolveStartupApiToken(explicitToken) {
...
Usage:
  disclaude start [options]

Commands:
  start    Start disclaude
...
Examples:
  disclaude start
  disclaude start --config /path/to/disclaude.config.yaml
`);
}
...
export async function main() {
    ...
    const appIds = channelEntries.filter(entry => entry.type === 'feishu')
        .map(entry => String(entry.config.appId ?? '')).sort();
    ...
    // Create AgentPool for disclaude service with Feishu message builder options
    ...
    const agentPool = new ChatSessionPool({
        ...
        onBusyCapExceeded: async (chatId, busyMinutes) => {
            await service.sendMessage(chatId, `⏹️ **响应已超过时长上限被停止**（已运行 ${busyMinutes} 分钟，上限 90 分钟）\n\n` +
                '会话保持活跃，您可以继续发送消息。');
        },
    });
    ...
    const controlHandlerContext = {
        ...
        debugGroups: {
            ...
            setDebugGroup: (chatId, name) => service.getDebugGroupService().setDebugGroup(chatId, name),
            clearDebugGroup: () => service.getDebugGroupService().clearDebugGroup(),
        },
        projectManager,
    ...
    // Must be called before service.start() (which calls initScheduler)
    ...
    const routerCallbacksFactory = (chatId) => {
        ...
        const channel = service.getChannelManager().resolveChannelForChatId(chatId);
        if (!channel) {
    ...
    service.initInputMessageRouter(agentPool, routerCallbacksFactory);
    ...
    const lifecycleManager = new ChannelLifecycleManager(channelManager, {
        ...
        inputMessageRouter: service.getInputMessageRouter(),
    });
    ...
    const shutdown = async () => {
        ...
        try {
            ...
            processLock?.release();
            logger.info('disclaude service stopped');
    ...
    try {
        ...
        logger.info({ channels: channelEntries.map((e) => e.type) }, 'disclaude service started successfully');
        if (restEntry) {
            const restConf = restEntry.config;
            console.log(`disclaude service started on http://${restConf.host}:${restConf.port}`);
        }
        else {
            console.log('disclaude service started (Feishu only mode)');
        }
        ...
        if (options.apiPort !== undefined) {
            ...
            const router = service.getInputMessageRouter();
            if (router) {
            ...
            httpApiServer.setUploadFileHandler((chatId, filePath, threadId) => service.uploadFile(chatId, filePath, threadId));
            // Issue #4279: wire REST /api/send-message to the channel's sendMessage
            // capability (REST parity with the REST API method).
            httpApiServer.setSendMessageHandler((chatId, text, threadId, mentions) => service.sendMessage(chatId, text, threadId, mentions));
            ...
            httpApiServer.setPrivateWorkflowHandler(async (request) => {
                ...
                if (!(channel instanceof FeishuChannel)) {
                    throw new Error('Private workflows require Feishu');
                }
                return channel.requestPrivateWorkflow(request);
            });
            httpApiServer.setSendCardHandler((chatId, card, threadId, description) => service.sendCard(chatId, card, threadId, description));
            ...
            httpApiServer.setSendInteractiveHandler((chatId, params) => service.sendInteractive(chatId, params));
            // Issue #4279: wire REST GET /api/temp-chats to the channel's
            // listTempChats capability (REST parity with the REST API method).
            httpApiServer.setListTempChatsHandler(() => service.listTempChats());
            // Issue #4279: wire REST /api/upload-image to the channel's uploadImage
            // capability (REST parity with the REST API method).
            httpApiServer.setUploadImageHandler((filePath) => service.uploadImage(filePath));
            ...
            // the REST API method).
            httpApiServer.setMarkChatRespondedHandler((chatId, response) => service.markChatResponded(chatId, response));
            ...
            await service.startDeferredScheduler();
        ...
        console.error('Failed to start disclaude service:', error instanceof Error ? error.message : String(error));
        processLock?.release();
        process.exit(1);
    }
}
