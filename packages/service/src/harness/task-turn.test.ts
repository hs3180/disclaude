...
describe('bounded project task turn', () => {
  it('rejects an execution that never publishes a terminal result', async () => {
    const dispose = vi.fn();
    vi.mocked(AgentFactory.createAgent).mockReturnValue({ runOnce: async () => {}, dispose } as never);
    await expect(runTaskTurn(input())).rejects.toThrow('did not finish');
    expect(dispose).toHaveBeenCalledTimes(1);
  });

  it.each([0, -1, 1.5, Number.NaN, 2_147_483_648])('rejects invalid time budget %s before agent creation', async timeoutMs => {
    await expect(runTaskTurn({ ...input(), timeoutMs })).rejects.toThrow('valid time budget');
