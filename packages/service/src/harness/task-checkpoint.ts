...
export function parseTaskCheckpoint(text: string): TaskCheckpoint {
  ...
  if ((typeof r.state !== 'string' || !['continue', 'waiting-user', 'complete'].includes(r.state))) { throw new Error('Invalid task checkpoint state'); }
  const work = list(r.work ?? [], 8).map(value => {
    const w = object(value);
    if ((typeof w.status !== 'string' || !['pending', 'done', 'stopped'].includes(w.status))) { throw new Error('Invalid work status'); }
    return { id: w.id === null || w.id === undefined ? undefined : str(w.id, 100), title: str(w.title, 180), status: w.status as WorkUpdate['status'],
      findings: list(w.findings ?? [], 4).map(value => {
        ...
        if ((typeof f.kind !== 'string' || !['fact', 'inference', 'uncertain'].includes(f.kind))) { throw new Error('Invalid evidence kind'); }
  ...
  const feedback = list(r.feedback ?? [], 24).map(value => {
    const f = object(value);
    if (!Number.isSafeInteger(f.feedbackIndex) || Number(f.feedbackIndex) < 0 || (typeof f.status !== 'string' || !['applied', 'rejected'].includes(f.status))) {
