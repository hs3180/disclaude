...
export interface TaskTurn {
  ...
  owner: string;
  workingDir: string;
  /** Caller supplies persisted context and task instructions; no workflow is implied. */
  prompt: string;
  signal: AbortSignal;
  timeoutMs: number;
}
...
export async function runTaskTurn(request: TaskTurn): Promise<string> {
  ...
  try {
    ...
    if (input.signal.aborted) { throw new Error('Task turn interrupted'); }
    if (!completed?.success || completed.truncated) { throw new Error('Task turn did not finish successfully'); }
