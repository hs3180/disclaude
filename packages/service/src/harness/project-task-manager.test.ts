...
import { ProjectStore, type ProjectTask, type Finding } from './project-task.js';
...
import { ProjectTaskManager, type TaskRunner } from './project-task-manager.js';
...
type ScriptedStep = { type: 'plan' } | { type: 'investigate'; directionId: string } | { type: 'synthesize' };
...
type StepRunner = (p: ProjectTask, step: ScriptedStep, signal: AbortSignal) => Promise<StepResult>;
const managers: ProjectTaskManager[] = [], directories: string[] = [];
...
const result = (step: ScriptedStep): StepResult => step.type === 'plan' ? { directions: ['Compare costs'] } : step.type === 'investigate' ? { findings: [finding] } : { summary: 'A costs less according to the supplied reports.', questions: ['Will costs change?'] };
...
} : result(step)), publish = vi.fn<(project: ProjectTask) => Promise<string>>(() => Promise.resolve('card-1')), directory?: string, readDocument?: DocumentReader, appendDocument?: DocumentAppender) {
  return checkpointFixture(async (p, signal) => {
    const direction = p.directions.find(d => d.status === 'pending');
    const step: ScriptedStep = p.feedback.some(f => f.status === 'pending' || f.status === 'needs-clarification') || !p.directions.length
...
function checkpointFixture(runner: TaskRunner, publish = vi.fn<(project: ProjectTask) => Promise<string>>(() => Promise.resolve('card-1')), directory?: string, readDocument?: DocumentReader, appendDocument?: DocumentAppender) {
  const dir = directory ?? mkdtempSync(join(tmpdir(), 'project-task-state-')); if (!directory) { directories.push(dir); }
  const manager = new ProjectTaskManager(new ProjectStore(dir), runner, publish, readDocument, appendDocument); managers.push(manager);
...
describe('persistent project task lifecycle', () => {
  ...
  it('refuses legacy association while task work is in flight', async () => {
  ...
  it('rejects continuation while cancelling and preserves cancellation after a turn fails', async () => {
    ...
    await expect(manager.create({ ...input, source: 'early-continuation', parent: p.id })).rejects.toThrow('请先结束原任务');
  ...
  it.each(['completed', 'waiting-user', 'failed', 'paused'] as const)(
    'preserves committed %s state when shutdown races final card delivery', async status => {
      ...
      let started = false;
      const publish = vi.fn((project: ProjectTask) => {
