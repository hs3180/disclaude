      ...
      for (const comment of response.data.items) {
        ...
        for (const reply of replies) {
          ...
          const body = reply.content.elements.map(element => {
            const value = element.type === 'text_run' ? element.text_run?.text
              : element.type === 'docs_link' ? element.docs_link?.url
              : element.type === 'person' ? element.person?.user_id : undefined;
