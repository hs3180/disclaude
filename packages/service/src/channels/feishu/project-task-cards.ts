...
// Retain legacy callback/form wire identities so already delivered cards remain usable.
export const taskButton = (label: string, value: Record<string, unknown>, primary = false) => ({
...
export function taskCard(title: string, elements: unknown[]): Record<string, unknown> {
...
export function projectCard(p: ProjectTask): Record<string, unknown> {
  const value = { project: p.id, revision: p.revision };
  const actions: unknown[] = [taskButton('刷新任务', { ...value, action: 'refresh' }), taskButton('返回项目首页', { action: 'index' })];
  if (p.status === 'running') { actions.push(taskButton('暂停任务', { ...value, action: 'pause' })); }
  if (['paused', 'failed', 'interrupted', 'waiting-user'].includes(p.status)) { actions.push(taskButton(p.directions.length ? '恢复任务' : '开始任务', { ...value, action: 'resume' }, true)); }
  if (!['completed', 'cancelled', 'cancelling'].includes(p.status)) { actions.push(taskButton('取消任务', { ...value, action: 'cancel' })); }
  if (!p.workingDir && !['running', 'pausing', 'cancelling'].includes(p.status)) {
    actions.push(taskButton(p.projectLink ? '撤销项目关联' : '关联到当前项目…', { ...value, action: p.projectLink ? 'unlink-project' : 'preview-project-link' }));
  }
  ...
  if (p.status === 'completed' && p.document && p.summary && p.document.export?.status !== 'saved'
    ...
    actions.push(taskButton(['writing', 'unknown'].includes(p.document.export?.status ?? '') ? '核对文档写入' : '将成果追加到关联文档', { ...value, action: 'export' }));
  }
  const elements: unknown[] = [
    ...
    ...(p.parent ? [text('从已有成果继续的任务'), taskButton('查看原任务', { project: p.parent, action: 'open' })] : []),
    ...(p.parentFinding && p.priorResults?.findings[0] ? [text(`本次跟进的发现\n${p.priorResults.findings[0].claim}`), taskButton('查看原发现与来源', { project: p.parent, action: 'evidence', direction: p.parentFinding.directionId, index: p.parentFinding.index })] : []),
    ...
    ...visible.flatMap(d => [
      ...
      ...(d.findings.length ? [taskButton('查看发现与来源', { ...value, action: 'evidence', direction: d.id })] : []),
      ...(d.status === 'pending' && !['completed', 'cancelled'].includes(p.status) ? [taskButton('停止这个方向', { ...value, action: 'stop-direction', direction: d.id })] : []),
    ]),
    ...(p.summary ? [{ tag: 'hr' }, text(`当前成果\n${p.summary}`)] : []),
    ...(p.questions.length ? [text(`尚未解决\n${p.questions.join('\n')}`)] : []),
    taskButton('查看任务记录', { ...value, action: 'history', offset: 0 }),
  ];
  if (!['completed', 'cancelled', 'cancelling'].includes(p.status)) {
    ...
    elements.push(taskButton('基于成果继续任务', { ...value, action: 'continue' }, true));
    elements.push(taskButton(p.archivedAt ? '移回任务列表' : '归档任务', { ...value, action: p.archivedAt ? 'unarchive' : 'archive' }));
  }
  return taskCard(p.title, elements);
}
export function evidenceCard(p: ProjectTask, directionId: string, index: number): Record<string, unknown> {
  const d = p.directions.find(d => d.id === directionId);
  const f = d?.findings[index];
  if (!d || !f) { throw new Error('该发现不存在，请刷新项目。'); }
  return taskCard(d.title, [
    ...
    ...(index + 1 < d.findings.length ? [taskButton('下一项发现', { project: p.id, action: 'evidence', direction: d.id, index: index + 1 })] : []),
    ...(['completed', 'cancelled'].includes(p.status) ? [taskButton('基于这项发现继续任务', { project: p.id, action: 'continue-finding', direction: d.id, index }, true)] : []),
    taskButton('返回任务', { project: p.id, action: 'open' }),
  ]);
}
export function historyCard(p: ProjectTask, offset: number): Record<string, unknown> {
  const entries = [
  ...
  return taskCard('任务记录', [
    ...entries.slice(offset, offset + 6).map(text),
    ...(offset + 6 < entries.length ? [taskButton('更早记录', { project: p.id, action: 'history', offset: offset + 6 })] : []),
    ...p.directions.filter(d => d.findings.length).slice(offset, offset + 6).map(d => taskButton(`证据：${d.title}`, { project: p.id, action: 'evidence', direction: d.id })),
    taskButton('返回任务', { project: p.id, action: 'open' }),
  ]);
}
export function indexCard(projects: ProjectTask[], nonce: string, offset = 0, archived = false, context: { workingDir?: string; error?: string } = {}): Record<string, unknown> {
  return taskCard(archived ? '项目 · 归档任务' : '项目 · 工作空间与任务', [
    ...
    group([
    taskButton(archived ? '返回任务列表' : '查看归档任务', { action: 'index', archived: !archived }),
    ...projects.slice(offset, offset + 6).flatMap(p => [text(`${p.title} · ${labels[p.status]}\n${p.projectLink ? `关联项目：${p.projectLink.directory}（保留独立执行目录）` : p.workingDir ? `工作目录：${p.workingDir}${p.workingDir === context.workingDir ? '（当前项目）' : ''}` : '历史任务 · 未关联项目目录'}`), taskButton('打开任务', { project: p.id, action: 'open' })]),
    ...(projects.length > offset + 6 ? [taskButton('更多任务', { action: 'index', offset: offset + 6, archived })] : []),
    ]),
...
export function projectLinkPreviewCard(p: ProjectTask): Record<string, unknown> {
  if (!p.linkPreview) { throw new Error('项目关联预览已失效。'); }
  return taskCard('确认历史任务的项目关联', [
    ...
    taskButton('确认关联', { project: p.id, revision: p.revision, token: p.linkPreview.token, action: 'confirm-project-link' }, true),
    taskButton('暂不关联，返回任务', { project: p.id, action: 'open' }),
  ]);
}
