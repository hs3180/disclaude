/**
 * Tests for DisclaudeService's REST-only serving behavior (Issue #4280, part 5).
 *
 * Part 5 removed the UnixSocketIpcServer lifecycle from DisclaudeService: start()
 ...
 * tools and push-cli reach DisclaudeService exclusively over the REST API
...
describe('DisclaudeService REST-only serving (Issue #4280 part 5)', () => {
  it('accepts only a diagnostic instance ID, not obsolete node options', () => {
    expect(new DisclaudeService({ instanceId: 'diagnostic' }).getInstanceId()).toBe('diagnostic');
    expect(() => new DisclaudeService({ nodeId: 'old' } as never)).toThrow('Unsupported service option: nodeId');
  });
  beforeEach(() => {
  ...
  it('propagates backend selection failure without switching to Claude', async () => {
    ...
    await expect(new DisclaudeService().start()).rejects.toThrow('Unknown provider type: unknown');
  ...
  it('rejects an unavailable backend before starting the scheduler', async () => {
    ...
    await expect(new DisclaudeService().start()).rejects.toThrow('Codex login required');
  ...
  it('start() does not write the IPC socket-path discovery file', async () => {
    ...
    const statBefore = existsSync(IPC_SOCKET_PATH_FILE)
      ...
      : -1;
    const nodeLate = new DisclaudeService();
  ...
  it('defers cron initialization until the real REST listener is ready, exactly once', async () => {
    const node = new DisclaudeService();
    const init = vi.spyOn(DisclaudeService.prototype as unknown as { initScheduler(): Promise<void> }, 'initScheduler');
  ...
  it('keeps deferred scheduler initialization failure non-fatal', async () => {
    const node = new DisclaudeService();
    vi.spyOn(DisclaudeService.prototype as unknown as { initScheduler(): Promise<void> }, 'initScheduler').mockRejectedValue(new Error('fixture failure'));
    await node.start({ deferScheduler: true });
