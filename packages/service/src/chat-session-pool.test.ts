/**
 * Tests for ChatSessionPool (packages/service/src/chat-session-pool.ts)
 *
 * Issue #1617: Add unit tests for ChatSessionPool.
 *
 * Tests cover:
...
describe('ChatSessionPool', () => {
  ...
  it('applies temporary model/history overrides through the same preset-aware factory', () => {
    const pool = new ChatSessionPool({ agentPresets: {
  ...
  describe('getOrCreateChatAgent()', () => {
    ...
    it('should pass messageBuilderOptions to factory when provided', () => {
      ...
      const pool = new ChatSessionPool({ messageBuilderOptions });
      const callbacks = createMockCallbacks();
    ...
    it('should pass cwdProvider to factory when provided', () => {
      ...
      const pool = new ChatSessionPool({ cwdProvider });
      const callbacks = createMockCallbacks();
    ...
    it('should pass both options when both are provided', () => {
      const messageBuilderOptions = { channel: 'feishu' } as any;
      const cwdProvider: CwdProvider = () => '/project';
      const pool = new ChatSessionPool({ messageBuilderOptions, cwdProvider });
      const callbacks = createMockCallbacks();
    ...
    it('should pass cwdResolver to factory when provided (Issue #4448 direction #1)', () => {
      ...
      const pool = new ChatSessionPool({ cwdProvider, cwdResolver });
      const callbacks = createMockCallbacks();
  ...
  describe('idle eviction (Issue #4169)', () => {
    ...
    it('should be disabled when idleTimeoutMs is 0', () => {
      const pool = new ChatSessionPool({ idleTimeoutMs: 0 });
      const callbacks = createMockCallbacks();
  ...
  describe('busy-turn hard cap (Issue #4577)', () => {
    ...
    it('should NOT stop busy agents within the cap (existing behavior preserved)', () => {
      const pool = new ChatSessionPool({ idleTimeoutMs: 1000, busyTurnHardCapMs: 60_000 });
      const callbacks = createMockCallbacks();
    ...
    it('should still enforce the busy cap when idle eviction is disabled (idleTimeoutMs=0)', () => {
      ...
      const pool = new ChatSessionPool({ idleTimeoutMs: 0, busyTurnHardCapMs: 5000 });
      const callbacks = createMockCallbacks();
    ...
    it('should fire onBusyCapExceeded after a hard-cap stop', async () => {
      ...
      const pool = new ChatSessionPool({ idleTimeoutMs: 1000, busyTurnHardCapMs: 5 * 60 * 1000, onBusyCapExceeded });
      const callbacks = createMockCallbacks();
    ...
    it('should swallow onBusyCapExceeded rejection — a failing channel must not break the sweep', async () => {
      ...
      const pool = new ChatSessionPool({ idleTimeoutMs: 1000, busyTurnHardCapMs: 5000, onBusyCapExceeded });
      const callbacks = createMockCallbacks();
    ...
    it('Issue #4620: back-to-back turns never accumulate — a new turn after 90min of session wall-clock is not insta-killed', () => {
      ...
      const pool = new ChatSessionPool({ idleTimeoutMs: 0, busyTurnHardCapMs: cap });
      const callbacks = createMockCallbacks();
  ...
  describe('pool stats / leak diagnostics (Issue #4256)', () => {
    ...
    it('emits an idle-sweep snapshot even when idle eviction is disabled (idleTimeoutMs=0)', () => {
      ...
      vi.useFakeTimers();
      try {
        const pool = new ChatSessionPool({ idleTimeoutMs: 0, idleSweepIntervalMs: 1000 });
        pool.startIdleSweep();
