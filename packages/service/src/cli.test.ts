/**
 * Tests for disclaude service CLI (packages/service/src/cli.ts)
 *
 ...
 *
 * Issue #1617: Adding meaningful test coverage for the service CLI entry point.
 */
...
vi.mock('./service.js', () => ({
  DisclaudeService: vi.fn(),
}));

vi.mock('./chat-session-pool.js', () => ({
  ChatSessionPool: vi.fn(),
}));
...
describe('parseArgs', () => {
  it.each(['--mode', '--mode=primary', '--node-type', '--node-type=worker', '--node-id', '--node-id=old'])('rejects removed role argument %s', (flag) => {
    expect(() => parseArgs(['start', flag])).toThrow(/removed/i);
  });
