...
export function createResearchRunner(workspace: string): ResearchRunner {
  return async (project, signal) => {
    ...
    const prompt = 'Advance this task within its project using the available tools and evidence. Choose the work that is useful now; there is no required planning/investigation/synthesis sequence. A simple task can finish in one turn. An open task can return progress and continue.\n'
      ...
      + 'work contains only additions/updates, not the entire previous work list. Omit id for new work. Include an existing id only to update pending work; completed/stopped work and its findings must remain unchanged. Use a new work item for follow-up checks. For an existing id, return only new findings (up to the per-work limit); the service preserves prior findings, so do not repeat or rewrite them. Add a new finding with counterevidence or a caveat when an earlier claim needs correction.\n'
      ...
      + 'Return exactly one JSON object beginning with { and ending with }; do not wrap it in an array or append closing brackets. Return one JSON checkpoint in the user language. State fields are execution controls, not research stages: '
