...
export function createResearchRunner(workspace: string): ResearchRunner {
  return async (project, signal) => {
    ...
    const prompt = 'Advance this task within its project using the available tools and evidence. Choose the work that is useful now; there is no required planning/investigation/synthesis sequence. A simple task can finish in one turn. An open task can return progress and continue.\n'
      ...
      + 'When the project already has a pending direction, update that existing direction by id and do not create additional work items unless the user explicitly asked to add a new direction. For a focused follow-up, return exactly the targeted pending direction update; do not turn intermediate checks into extra directions.\n'
      ...
      + 'Processed adjustments are history only: never return a feedback receipt for an already applied or rejected adjustment. If no pending/needs-clarification adjustments are shown, return feedback as an empty array.\n'
