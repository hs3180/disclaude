...
describe('Research runner prompt boundary', () => {
  it('suppresses outbound next-step card guidance for a synthetic Research identity', async () => {
    const root = mkdtempSync(join(tmpdir(), 'research-runner-fix-')); roots.push(root);
    let options: unknown;
