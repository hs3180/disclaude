...
export async function main(): Promise<void> {
  ...
  const controlHandlerContext: ControlHandlerContext = {
    ...
    debugGroups: {
      ...
      setDebugGroup: (chatId: string, name?: string) =>
        service.getDebugGroupService().setDebugGroup(chatId, name),
  ...
  try {
    ...
    logger.info(
      ...
      'disclaude service started successfully'
    );
    if (restEntry) {
      const restConf = restEntry.config as { port: number; host: string };
    ...
    if (options.apiPort !== undefined) {
      ...
      httpApiServer.setUploadFileHandler((chatId, filePath, threadId) =>
        service.uploadFile(chatId, filePath, threadId)
      );
      ...
      httpApiServer.setSendMessageHandler((chatId, text, threadId, mentions) =>
        service.sendMessage(chatId, text, threadId, mentions)
      );
      ...
      httpApiServer.setSendInteractiveHandler((chatId, params) =>
        service.sendInteractive(chatId, params)
      );
      ...
      httpApiServer.setMarkChatRespondedHandler((chatId, response) =>
        service.markChatResponded(chatId, response)
      );
