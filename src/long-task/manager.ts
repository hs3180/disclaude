...
export class LongTaskManager {
  ...
  private async executeTask(
    ...
    // Persist plan
    await this.savePlanToDisk(plan, workspaceDir);
  ...
  private async executeSubtasks(
    ...
    for (let i = 0; i < plan.subtasks.length; i++) {
      ...
      // Persist result after each step
      await this.saveSubtaskResult(result, workspaceDir);
    }
  }
   ...
   * Save plan to disk.
   */
  private async savePlanToDisk(plan: LongTaskPlan, workspaceDir: string): Promise<void> {
    const planFile = path.join(workspaceDir, 'TASK_PLAN.md');
...
${st.outputs.markdownRequirements.map(req => `- **${req.title}** (\`${req.id}\`): ${req.content} ${req.required ? '(Required)' : '(Optional)'}`).join('\n')}
` : ''}
`).join('\n')}
...
`;
    ...
    console.log(`[LongTask] Plan saved to: ${planFile}`);
  }

  /**
   * Save subtask result to disk.
   */
  private async saveSubtaskResult(result: SubtaskResult, workspaceDir: string): Promise<void> {
    const resultFile = path.join(workspaceDir, `subtask-${result.sequence}-result.json`);

    await fs.writeFile(resultFile, JSON.stringify(result, null, 2), 'utf-8');
    console.log(`[LongTask] Result saved to: ${resultFile}`);
  }
  ...
  private async generateFinalSummary(state: LongTaskState, workspaceDir: string): Promise<void> {
    ...
    const summaryPath = path.join(workspaceDir, 'FINAL_SUMMARY.md');
...
`;
    ...
    console.log(`[LongTask] Final summary saved to: ${summaryPath}`);
  }
