 ...
 *
 * @module mcp/tools/study-guide-generator
 */
...
export interface MindMapGeneratorOptions {
  ...
  /** Root node title */
  title?: string;
  /** Output format: 'mermaid' | 'markmap' | 'json' */
  format?: 'mermaid' | 'markmap' | 'json';
...
export interface MindMapGeneratorResult {
  success: boolean;
  /** Raw mind map structure */
  mindMap: MindMapNode | null;
  /** Formatted output based on format */
  output: string;
  /** The format used */
  format: 'mermaid' | 'markmap' | 'json';
  /** Node count */
...
// ============================================================================
// Mind Map Generator (Issue #950 M3)
// ============================================================================

/**
 * Generate a mind map from content.
 * Supports Mermaid mindmap and Markmap formats.
 */
export function generate_mindmap(options: MindMapGeneratorOptions): MindMapGeneratorResult {
  const { content, title = 'Topic', format = 'mermaid', maxDepth = 3 } = options;
  ...
  // Generate mind map prompt based on format
  let formatGuide: string;
  let outputExample: string;

  switch (format) {
    case 'markmap':
      formatGuide = `Generate a Markdown hierarchy that can be rendered as a mind map.
- Use # for root, ## for main branches, ### for sub-branches, etc.
- Use bullet points (- ) for additional nodes at each level
...
- Sub item`;
      break;
    case 'json':
      formatGuide = `Generate a JSON structure representing the mind map.
- Maximum depth: ${maxDepth} levels
- Each node has "text" and optional "children" array`;
      outputExample = `{
  "text": "${title}",
  "children": [
    {
    ...
    case 'mermaid':
    default:
      formatGuide = `Generate a Mermaid mindmap syntax.
- Use proper indentation (2 spaces per level)
- Maximum depth: ${maxDepth} levels
- Use ((text)) for rounded nodes, [text] for square nodes`;
      outputExample = `mindmap
  root((${title}))
    Branch 1
      Sub 1
      Sub 2
  ...
  const mindmapPrompt = `## Mind Map Generation Request
...
**Instructions**: Analyze the content above and generate a mind map that:
1. Extracts key concepts and their hierarchical relationships
2. Organizes information logically with the root "${title}"
3. Uses maximum ${maxDepth} levels of depth
4. Groups related concepts under appropriate branches
5. Uses clear, concise node labels

**Output**: Generate the mind map in ${format} format following the example above.`;

  // Create template mind map structure
  const templateMindMap: MindMapNode = {
    text: title,
    children: [
      {
        ...
        children: [
          ...
          { text: 'Use your LLM capabilities' },
          { text: 'to generate actual mind map' },
        ],
      },
    ],
  };
...
${mindmapPrompt}`;
    ...
    output = JSON.stringify(templateMindMap, null, 2);
...
export interface StudyGuideResult {
  ...
  components: {
    ...
    mindmap?: MindMapGeneratorResult;
  };
  outputPath?: string;
  error?: string;
}
...
export function create_study_guide(options: StudyGuideOptions): StudyGuideResult {
  ...
  // Mind Map (Issue #950 M3)
  if (include.mindmap) {
    components.mindmap = generate_mindmap({ content, title, format: 'mermaid' });
    sections.push('## Mind Map\n');
    sections.push(components.mindmap.output);
    sections.push('\n---\n');
  }
