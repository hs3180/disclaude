...
describe('spawn_subagents', () => {
  ...
  describe('spawn_subagents', () => {
    ...
    it('should handle partial failures', async () => {
      ...
      const result = await spawn_subagents({
        tasks: [
          { type: 'task', name: 'task-1', prompt: 'Prompt 1' },
          { type: 'task', name: 'task-2', prompt: 'Prompt 2' },
        ],
      });
    ...
    it('should respect maxParallel limit', async () => {
      ...
      await spawn_subagents({
        tasks: [
          { type: 'task', name: 'task-1', prompt: 'Prompt 1' },
          { type: 'task', name: 'task-2', prompt: 'Prompt 2' },
          { type: 'task', name: 'task-3', prompt: 'Prompt 3' },
          { type: 'task', name: 'task-4', prompt: 'Prompt 4' },
        ],
        maxParallel: 2,
      });

      // With maxParallel: 2 and 4 tasks, spawn should be called 4 times
      // but in 2 batches of 2
      expect(mockManager.spawn).toHaveBeenCalledTimes(4);
    });
  });

  describe('generateSummary', () => {
    it('should be included in successful results', async () => {
      const mockHandle = {
        id: 'task-1',
        type: 'task',
        name: 'test-task',
      ...
      const result = await spawn_subagents({
        tasks: [{ type: 'task', name: 'test-task', prompt: 'Test' }],
      });

      expect(result.summary).toBeDefined();
      expect(result.summary).toContain('执行结果汇总');
      expect(result.summary).toContain('成功: 1');
    });
  });
});
