/**
 * Orchestration Agent - Task evaluation and user communication agent.
 *
 * NEW Responsibilities (Flow 2):
 * - Receives output from ExecutionAgent (what was done)
 ...
 *
 * NEW Dialogue Flow:
 * - ExecutionAgent works FIRST on user request
 * - ExecutionAgent output → OrchestrationAgent (you evaluate)
 * - OrchestrationAgent output → ExecutionAgent (next instructions)
...
/**
 * Orchestration agent for user-facing conversations.
 *
...
export class OrchestrationAgent {
  ...
  private logger = createLogger('OrchestrationAgent', { model: '' });
  ...
  constructor(config: OrchestrationAgentConfig) {
    ...
    this.logger = createLogger('OrchestrationAgent', { model: this.model });
  }
  ...
  private async loadSkill(): Promise<void> {
    const result = await loadSkill('orchestration-agent');
    if (!result.success || !result.skill) {
      throw new Error(
        `OrchestrationAgent skill is required but failed to load. ` +
        `Error: ${result.error || 'Unknown error'}. ` +
        `Please ensure .claude/skills/orchestration-agent/SKILL.md exists and is valid.`
      );
    }
    ...
    }, 'OrchestrationAgent skill loaded');
  }
  ...
  /**
   * Create SDK options for orchestration agent.
...
${userPrompt}`;
  ...
  async *queryStream(prompt: string, sessionId?: string): AsyncIterable<AgentMessage> {
    ...
    this.logger.debug({ sessionId, promptLength: prompt.length }, 'Starting orchestration query');
    ...
    try {
      ...
      }, 'OrchestrationAgent query config');
      ...
      this.logger.error({ err: error }, 'Orchestration query failed');
      yield {
