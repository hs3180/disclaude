 ...
 * - MessageBuilder: Enhanced content and tools section building
...
import { type PilotCallbacks, type PilotConfig, type MessageData } from './types.js';

// Re-export types for backward compatibility
export { type PilotCallbacks, type PilotConfig, type MessageData } from './types.js';
...
export class Pilot extends BaseAgent implements ChatAgent {
  ...
  constructor(config: PilotConfig) {
    ...
    // Initialize message builder with dependencies
    this.messageBuilder = new MessageBuilder({
      chatId: this.boundChatId,
      getCapabilities: this.callbacks.getCapabilities,
    });
  ...
  processMessage(
    ...
    // Build the user message
    const enhancedContent = this.messageBuilder.buildEnhancedContent({ text, messageId, senderOpenId, attachments, chatHistoryContext });
