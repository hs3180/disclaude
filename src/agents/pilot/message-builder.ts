/**
 * Message builder for Pilot - constructs enhanced content for AI prompts.
 *
...
/**
 * Builds enhanced content with Feishu context for AI prompts.
 *
 * @param msg - Message data containing text, metadata, and attachments
 * @param chatId - The chat ID this message is for
 * @param getCapabilities - Optional function to get channel capabilities
 * @returns Enhanced content string for AI prompt
 */
...
**Message ID:** ${msg.messageId}${buildAttachmentsInfo(msg.attachments)}`;
  ...
  const toolsSection = buildToolsSection(chatId, msg.messageId || '', capabilities, msg.senderOpenId);
 ...
 */
export function buildAttachmentsInfo(attachments?: FileRef[]): string {
