 ...
 *
 * Extracted from pilot.ts for better code organization (Issue #697).
 * Builds enhanced content with Feishu context for user messages.
 */
...
/**
 * Dependencies needed by MessageBuilder.
 */
export interface MessageBuilderDeps {
  /** The chatId this builder is bound to */
  chatId: string;
  /** Function to get channel capabilities */
  getCapabilities?: (chatId: string) => ChannelCapabilities | undefined;
}

/**
 * MessageBuilder - Builds enhanced content with Feishu context.
 *
 * Used by Pilot to construct user messages with appropriate context,
 * tools information, and attachment details.
 */
export class MessageBuilder {
  private readonly chatId: string;
  private readonly getCapabilities?: (chatId: string) => ChannelCapabilities | undefined;

  constructor(deps: MessageBuilderDeps) {
   ...
   */
  buildEnhancedContent(msg: MessageData): string {
    const { chatId } = this;
...
- Note: Thread replies are NOT supported on this channel.`);
   ...
   */
  buildAttachmentsInfo(attachments?: MessageData['attachments']): string {
