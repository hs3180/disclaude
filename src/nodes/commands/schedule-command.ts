/**
 * Schedule Command - Scheduled task management.
 *
 * Issue #469: 定时任务控制指令
...
可用子命令: ${validSubcommands.map(c => `\`${c}\``).join(', ')}`,
  ...
  private async handleList(services: CommandServices): Promise<CommandResult> {
  ...
  private async handleStatus(services: CommandServices, nameOrId: string): Promise<CommandResult> {
...
目标聊天: \`${task.chatId}\``,
  ...
  private async handleEnable(services: CommandServices, nameOrId: string): Promise<CommandResult> {
  ...
  private async handleDisable(services: CommandServices, nameOrId: string): Promise<CommandResult> {
  ...
  private async handleRun(services: CommandServices, nameOrId: string): Promise<CommandResult> {
