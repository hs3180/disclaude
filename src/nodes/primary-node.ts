  ...
  constructor(config: PrimaryNodeConfig) {
    ...
    this.messageRouter = new UnifiedMessageRouter({
      ...
      onTaskDone: async (chatId: string, threadId?: string) => {
        // Issue #884: Use sendMessage instead of agent.processMessage to avoid infinite loop
        // The next-step recommendation is generated by queryOnce (one-shot LLM call)
        // and sent directly to the user via sendMessage, bypassing the agent's processing loop
