...
console.log('[test] full production config: 3 MCP servers');
...
function handleLine(line) {
  try {
    ...
    if (msg.result?.sessionId) {
      console.log(`[← session] ${msg.result.sessionId} @${elapsed()}`);
      send('session/prompt', {
        sessionId: msg.result.sessionId,
        prompt: [{ type: 'text', text: `请使用 send_text 工具发送"测试"到 chatId ${CHAT_ID}` }],
      });
      return;
    }
    if (msg.method === 'session/update') {
      ...
      else if (t === 'tool_call') { toolCalls.push(u.toolName); console.log(`\n[tool_call] ${u.toolName} @${elapsed()}`); }
      else if (t === 'tool_call_update' && u.content?.text) { console.log(`[tool_result] ${u.content.text.slice(0, 200)} @${elapsed()}`); }
      else if (t !== 'agent_thought_chunk') { console.log(`[${t}] @${elapsed()}`); }
      return;
    }
    ...
    if (msg.error) { console.log(`[error] ${JSON.stringify(msg.error).slice(0, 300)} @${elapsed()}`); return; }
  } catch {}
}
