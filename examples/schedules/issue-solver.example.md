...
**前置条件**: 步骤 1 完成（仓库已克隆到临时目录）

克隆仓库后、开始分析 issue 之前，使用 `/load-project-context` skill 加载项目的 `CLAUDE.md`（如存在）：

1. 检查克隆仓库的根目录是否包含 `CLAUDE.md`
2. 如果存在，读取并提取项目的开发规范、测试要求、编码标准等关键信息
3. 在后续步骤（代码分析、实现、测试）中遵循这些规范

**目的**: 确保对项目的实现符合该项目自身的开发规范（如编码风格、测试规则、提交约定等），而非使用通用假设。
