...
test_multi_turn_number_context() {
    log_info "Test: Multi-turn conversation with number context..."
    ...
    response=$(send_chat "$chat_id" "Please remember that my favorite number is 42.")
    response_text=$(extract_response "$response")
    log_debug "Turn 1 response: $response_text"

    if [ -z "$response_text" ]; then
        log_error "Turn 1 failed: No response received"
        return 1
    fi
    ...
    response=$(send_chat "$chat_id" "What is my favorite number?")
    response_text=$(extract_response "$response")
    log_debug "Turn 2 response: $response_text"

    if [ -z "$response_text" ]; then
        log_error "Turn 2 failed: No response received"
        return 1
    fi
    ...
    if echo "$response_text" | grep -q "42"; then
        ...
        # Still pass if we got a reasonable response
    fi

    # Turn 3: Ask for a calculation using the number
    log_turn 3 "Asking for calculation..."
    response=$(send_chat "$chat_id" "What is my favorite number multiplied by 2?")
    response_text=$(extract_response "$response")
    log_debug "Turn 3 response: $response_text"

    if [ -z "$response_text" ]; then
        log_error "Turn 3 failed: No response received"
        return 1
    fi
...
test_multi_turn_name_context() {
    log_info "Test: Multi-turn conversation with name context..."
    ...
    response=$(send_chat "$chat_id" "Hi, my name is Alice and I like programming.")
    response_text=$(extract_response "$response")
    log_debug "Turn 1 response: $response_text"
    ...
    response=$(send_chat "$chat_id" "What is my name?")
    response_text=$(extract_response "$response")
    log_debug "Turn 2 response: $response_text"
    ...
    response=$(send_chat "$chat_id" "What do I like?")
    response_text=$(extract_response "$response")
    log_debug "Turn 3 response: $response_text"
...
test_separate_chat_contexts() {
    log_info "Test: Separate chat contexts..."
    ...
    response=$(send_chat "$chat_id_1" "My favorite color is blue.")
    log_debug "Chat 1 response: $(extract_response "$response")"

    # Set different context in chat 2
    log_debug "Setting context in chat 2..."
    response=$(send_chat "$chat_id_2" "My favorite color is red.")
    log_debug "Chat 2 response: $(extract_response "$response")"

    # Verify chat 1 remembers blue
    response=$(send_chat "$chat_id_1" "What is my favorite color?")
    response_text=$(extract_response "$response")
    ...
    response=$(send_chat "$chat_id_2" "What is my favorite color?")
    response_text=$(extract_response "$response")
