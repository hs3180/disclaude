...
show_test_plan_body() {
    echo ""
    echo "Test Suites:"
    echo "  Feishu IPC Integration Tests (53 tests)"
    echo "    - sendMessage: 9 tests (text, thread, mentions, errors)"
    echo "    - sendCard: 10 tests (card, thread, description, errors)"
    echo "    - sendInteractive: 5 tests (interactive, actionPrompts, context)"
    echo "    - uploadFile: 7 tests (file upload, thread, errors)"
    echo "    - multi-card: 4 tests (LRU eviction, cross-card resolution)"
    echo "    - trigger-mode: 18 tests (mention, always, auto, small group, mode switching, persistence)"
    echo ""
    echo "Prerequisites:"
