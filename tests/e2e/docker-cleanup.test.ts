...
describe('run-owned Docker resources', () => {
  it.skipIf(!process.env.DISCLAUDE_E2E_DOCKER_CLEANUP_IMAGE)('reports an in-use volume, preserves a parallel owner, and reclaims after release', async () => {
    ...
    try {
      ...
      await docker('run', '-d', '--init', '--memory', '256m', '--user', '0:0', '--name', container,
        '--label', label, '--entrypoint', 'node', '-v', `${volume}:/data`, image, '-e', 'setInterval(()=>{},1000)');
      await docker('run', '-d', '--init', '--memory', '256m', '--user', '0:0', '--name', peerContainer,
        '--label', peerLabel, '--entrypoint', 'node', '-v', `${volume}:/shared`, '-v', `${peerVolume}:/data`, image, '-e', 'setInterval(()=>{},1000)');
      ...
      console.info('DOCKER_CLEANUP_ACCEPTANCE', JSON.stringify({ volumeInUseReported: true, parallelOwnerPreserved: true, retryAfterRelease: true }));
    } finally {
