---
name: diagnose-logs
description: "Diagnose disclaude service logs — analyze errors, warnings, WebSocket health, agent behavior, and SDK subprocess issues from launchd log files. Use when user says keywords like 'diagnose logs', 'check logs', 'debug service', 'what went wrong', 'service health', 'log analysis', '查看日志', '诊断日志'."
...
grep '^{' /tmp/disclaude-stdout.log | jq -c 'select(.msg | test("subprocess")) | {time, context, msg}'
...
**ChatAgent**: {count} timeouts, {count} errors.
