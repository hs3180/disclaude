...
See `schedule.template.md` in this directory. Copy it to `schedules/daily-news-inspiration/SCHEDULE.md`, replace `{targetChatId}` with the actual chat ID, replace `{cron}` with the desired cron expression (e.g. `"0 9 * * *"` for daily 9 AM), and enable.
