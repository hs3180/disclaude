...
width = cg.CGDisplayPixelsWide(main_id)
mode = cg.CGDisplayCopyDisplayMode(main_id)
mode_width = cg.CGDisplayModeGetWidth(mode)
print(int(width / mode_width))
"
```

## Helper Script

A helper script is available at `scripts/mac-control.sh` for common operations:

```bash
# All commands work via CGEvent (zero external dependencies)
./scripts/mac-control.sh screenshot
./scripts/mac-control.sh click 500 300
./scripts/mac-control.sh type-cjk "你好世界"
./scripts/mac-control.sh key-combo cmd,c
./scripts/mac-control.sh activate "Safari"
./scripts/mac-control.sh window-bounds "Safari"
```
...
All mouse operations use CGEvent via Python ctypes — **zero external dependencies** (no cliclick needed).
...
# Move to position
cg.CGEventPost(0, cg.CGEventCreateMouseEvent(None, 5, (x, y), 0))
time.sleep(0.05)
# Left click down + up
...
cg.CGEventPost(0, cg.CGEventCreateMouseEvent(None, 3, (x, y), 1))  # right_down, button=1
time.sleep(0.05)
cg.CGEventPost(0, cg.CGEventCreateMouseEvent(None, 4, (x, y), 1))  # right_up, button=1
"
```
...
cg = ctypes.cdll.LoadLibrary(ctypes.util.find_library('CoreGraphics'))
# Cmd key: keycode 55, V key: keycode 9
...
- ⚠️ Add `sleep 0.5` after activating an app or clicking a button to wait for UI updates
...
- **python3** — for CGEvent via ctypes (zero external packages needed)
- **screencapture** — built-in macOS tool
- **osascript** — built-in macOS AppleScript runner
- **pbcopy/pbpaste** — built-in macOS clipboard tools

No external dependencies (no brew install needed).
