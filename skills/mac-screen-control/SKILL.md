...
# Alternative: check via system_profiler
" 2>/dev/null
osascript -e '
...
x, y = ${X}, ${Y}
# Move to position
event = cg.CGEventCreateMouseEvent(None, 5, (x, y), 0)  # 5=mouse_moved
cg.CGEventPost(0, event)
time.sleep(0.05)
# Left click down
event = cg.CGEventCreateMouseEvent(None, 1, (x, y), 0)  # 1=left_down
cg.CGEventPost(0, event)
time.sleep(0.05)
# Left click up
event = cg.CGEventCreateMouseEvent(None, 2, (x, y), 0)  # 2=left_up
cg.CGEventPost(0, event)
"
```
...
cg = ctypes.cdll.LoadLibrary(ctypes.util.find_library('CoreGraphics'))
x, y = ${X}, ${Y}
event = cg.CGEventCreateMouseEvent(None, 3, (x, y), 1)  # 3=right_down, button=1
cg.CGEventPost(0, event)
time.sleep(0.05)
event = cg.CGEventCreateMouseEvent(None, 4, (x, y), 1)  # 4=right_up, button=1
cg.CGEventPost(0, event)
"
```
...
- Always **take a screenshot first** before clicking to verify coordinates
- Add `sleep 0.5` after activating an app or clicking a button to wait for UI updates
- **Preserve clipboard contents** when using the paste method (save > paste > restore)
- CGEvent events are **indistinguishable from real input** — use responsibly
