...
All scripts accept input via **environment variables** (avoids shell quoting issues with JSON) and are located in `scripts/chat/`. All scripts include built-in Chat ID validation (path traversal protection) and Node.js `fs.flock` concurrency safety. Run with `npx tsx scripts/chat/<command>.ts`.
...
| `jq` not available | N/A (replaced by native JSON.parse) |
| `flock` not available | Graceful degradation (warning logged, lock skipped) |
