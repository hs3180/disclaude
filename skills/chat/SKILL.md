...
All scripts accept input via **environment variables** (avoids shell quoting issues with JSON) and are located in `scripts/chat/`. All scripts include built-in Chat ID validation (path traversal protection), `fs.flock` concurrency safety, and native JSON validation (no external dependencies required).
...
- Uniqueness checked under `fs.flock` (TOCTOU-safe)
...
| `jq` not available | N/A (native JSON parsing, no external dependency) |
| `flock` not available | Graceful degradation (logs warning, proceeds without lock) |
