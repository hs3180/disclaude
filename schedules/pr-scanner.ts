 ...
 *
 * Usage:
 *   npx tsx pr-scanner.ts --action <action> [options]
 *
 * Actions:
 *   check-capacity   读取 .temp-chats/ 统计 reviewing 数量
 *   list-candidates  过滤已有状态文件的 PR（需要 gh 输出通过 stdin 传入）
 *   create-state     写入 .temp-chats/pr-{number}.json + 添加 reviewing label
 *   mark             更新状态文件的 state 字段 + 移除 reviewing label（如适用）
 ...
 *   PR_SCANNER_REPO  GitHub 仓库（默认: hs3180/disclaude）
 *
 * Exit codes:
 *   0 — 成功
 *   1 — 致命错误
...
/**
 * 为 PR 添加 reviewing label。
 * Label 操作失败不阻塞主流程（仅输出 WARN）。
 */
export async function addReviewingLabel(prNumber: number): Promise<void> {
  const repo = getRepo();
  try {
    const { stderr } = await execFileAsync('gh', [
      'pr', 'edit', String(prNumber),
      '--repo', repo,
      '--add-label', REVIEWING_LABEL,
    ], { timeout: 30000 });
    if (stderr) {
      console.error(`WARN: gh label add produced stderr: ${stderr.trim()}`);
    }
    console.log(`Label '${REVIEWING_LABEL}' added to PR #${prNumber}`);
  } catch (err) {
    // Label 操作失败不阻塞主流程
    console.error(`WARN: Failed to add label '${REVIEWING_LABEL}' to PR #${prNumber}: ${err instanceof Error ? err.message : err}`);
  }
}

/**
 * 为 PR 移除 reviewing label。
 * Label 操作失败不阻塞主流程（仅输出 WARN）。
 */
export async function removeReviewingLabel(prNumber: number): Promise<void> {
  const repo = getRepo();
  try {
    const { stderr } = await execFileAsync('gh', [
      'pr', 'edit', String(prNumber),
    ...
    if (stderr) {
      console.error(`WARN: gh label remove produced stderr: ${stderr.trim()}`);
    }
    console.log(`Label '${REVIEWING_LABEL}' removed from PR #${prNumber}`);
  } catch (err) {
    // Label 操作失败不阻塞主流程
    console.error(`WARN: Failed to remove label '${REVIEWING_LABEL}' from PR #${prNumber}: ${err instanceof Error ? err.message : err}`);
  }
}
...
/**
 * create-state: 创建新的状态文件并添加 reviewing label
 * 需要 --pr <number> 参数
 */
export async function actionCreateState(prNumber: number): Promise<void> {
  ...
  // 添加 reviewing label（失败不阻塞）
  await addReviewingLabel(prNumber);
}

/**
 * mark: 更新状态文件的 state 字段，离开 reviewing 时移除 label
...
export async function actionMark(prNumber: number, newState: PRState): Promise<void> {
  ...
  // 离开 reviewing 状态时移除 label（失败不阻塞）
...
/**
 * add-label: 独立 action，为 PR 添加 reviewing label
 */
export async function actionAddLabel(prNumber: number): Promise<void> {
  await addReviewingLabel(prNumber);
}

/**
 * remove-label: 独立 action，为 PR 移除 reviewing label
 */
export async function actionRemoveLabel(prNumber: number): Promise<void> {
  await removeReviewingLabel(prNumber);
}
...
async function main(): Promise<void> {
  ...
  switch (action) {
    ...
    case 'add-label':
      if (!pr) {
        throw new Error('--pr <number> is required for add-label');
      }
      await actionAddLabel(pr);
      break;

    case 'remove-label':
      if (!pr) {
        throw new Error('--pr <number> is required for remove-label');
      }
      await actionRemoveLabel(pr);
      break;
...
const isMainModule = process.argv[1]?.includes('pr-scanner.ts');
if (isMainModule) {
