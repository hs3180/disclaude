...
|---|---|---|---|
| **S1** `channel-mcp` — disclaude's own messaging tools (`send_text`/`send_card`/`send_interactive`/`send_file`/`push_to_agent`; the loop tools were removed with the loop system, #4430) | inline / in-process | [`packages/mcp-server/src/channel-mcp.ts`](../packages/mcp-server/src/channel-mcp.ts) | **Open.** These are in-process functions, not stdio servers — per [pi-mcp-landing-research.md](./pi-mcp-landing-research.md) §B-Q4 they are "just in-process functions" already covered by the **inline-tool adapter** (#4387). Whether they additionally surface as CLI Skills is a scope-3 call. |
