...
2. **无内置权限系统（🔴 威胁模型：disclaude 是唯一权限权威）**：pi 运行时本身不带权限门控——它**继承启动者的 OS 权限，既不询问、也不拒绝、更不记录任何权限决策**（与 claude-code 的 `permissionMode` 不同）。因此在 pi 后端下，**disclaude 必须是唯一的权限权威**（[#4383](https://github.com/hs3180/disclaude/issues/4383) §5 / [#4389](https://github.com/hs3180/disclaude/issues/4389)）。任何门控都必须落在「每一条工具调用都必经」的 disclaude 自有代码上：因 2026-08-07 决策 pi 不做 MCP（见上条），pi 路径上**唯一的工具注入点 = inline-tool adapter**（[#4387](https://github.com/hs3180/disclaude/issues/4387)，`packages/core/src/sdk/providers/pi/inline-tool-adapter.ts`），它须把每个工具的 `execute` 经门控放行，不得绕过。**当前状态：该门控尚未实现**——[#4389](https://github.com/hs3180/disclaude/issues/4389) 仍 open（等 [#4432](https://github.com/hs3180/disclaude/issues/4432) 的 C1/C2/C3 范式选型拍板 + pi loop [#4386](https://github.com/hs3180/disclaude/issues/4386) live），故落地前请勿在 pi 后端上依赖细粒度权限控制。完整的威胁模型 + 候选范式对比矩阵（含 Claude 现状审计 + pi `pi-agent-core@0.83.0` 复核）见 [`pi-permission-gating-research.md`](./pi-permission-gating-research.md)（#4432）。
