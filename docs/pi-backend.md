...
2. **无内置权限系统（🔴 威胁模型：disclaude 是唯一权限权威）**：pi 运行时本身不带权限门控——它**继承启动者的 OS 权限，既不询问、也不拒绝、更不记录任何权限决策**（与 claude-code 的 `permissionMode` 不同）。因此在 pi 后端下，**disclaude 必须是唯一的权限权威**（[#4383](https://github.com/hs3180/disclaude/issues/4383) §5 / [#4389](https://github.com/hs3180/disclaude/issues/4389)）。任何门控都必须落在「每一条工具调用都必经」的 disclaude 自有代码上：因 2026-08-07 决策 pi 不做 MCP（见上条），pi 路径上**唯一的工具注入点 = inline-tool adapter**（[#4387](https://github.com/hs3180/disclaude/issues/4387)，`packages/core/src/sdk/providers/pi/inline-tool-adapter.ts`），它须把每个工具的 `execute` 经门控放行，不得绕过。**当前状态（#4389 part 1 + part 2 均已落地，双层防御）**：
    - **执行层门（part 2，#4538）**——`PiPermissionGate`（`packages/core/src/sdk/providers/pi/permission-gate.ts`）经 `adaptInlineTool` 在**每个** inline 工具的 `execute` 开头咨询（deny → 抛错 → pi 转为 isError 工具结果，**handler 不执行**，deny 路径有单测）；`PiAgentProvider.queryStream` 按查询安装 denylist 门（`options.disallowedTools` → 执行期拒绝）。⚠️ provider 是进程级缓存单例，gate 为实例字段，**单活跃 query 假设**（多 chat 并发会覆写彼此 denylist）。
    - **beforeToolCall 钩子层（part 1，#4542）**——`queryStream` 在 `disallowedTools` 非空时给 pi Agent 构造器装 **`beforeToolCall` deny hook**（`packages/core/src/sdk/providers/pi/tool-permission-gate.ts`；pi 原生钩子，loop 在参数校验后、执行前调用，`{block:true}` 即拦截并向模型回错误 tool result）。**per-query 作用域**：钩子装在每个查询自己的 Agent 实例上，天然多 query 隔离——恰好补上执行层门的单例覆写隐患。决策层暂为 disclaude 既有 deny-list（`buildDisallowedTools()` #4181：`EnterPlanMode`/`AskUserQuestion`/内置 cron 系），**deny 路径有测试**（`tool-permission-gate.test.ts`）。
    ⚠️ 2026-08-20 审查修正：queryStream 目前**只消费** `adaptedOptions.systemPrompt`，options-adapter 算出的 `activeToolNames` **尚未接到 pi Agent**（无 `setActiveTools` 调用，属 #4386 工具注入的预置缺口）——「枚举过滤 + 执行门」的配合要等工具注入接线后才真正成立。**尚未落地**：更细的策略范式（allowlist / arg 级检查 / `ExecutionEnv` 沙箱）——等 [#4432](https://github.com/hs3180/disclaude/issues/4432) 的 C1/C2/C3 选型拍板后在此同一接缝上替换即可；当前仅工具名 denylist 级别，请勿在 pi 后端上依赖细粒度权限控制。完整的威胁模型 + 候选范式对比矩阵（含 Claude 现状审计 + pi `pi-agent-core@0.83.0` 复核）见 [`pi-permission-gating-research.md`](./pi-permission-gating-research.md)（#4432）。

> 第 1 项（MCP）已于 2026-08-07 定为**不支持**（[#4417](https://github.com/hs3180/disclaude/issues/4417) 已关闭、决策见 [#4461](https://github.com/hs3180/disclaude/pull/4461)）；第 2 项（权限）已落地双层机制——执行层门（part 2 #4538）+ beforeToolCall 钩子（part 1 #4542），策略范式选型（C1/C2/C3）仍跟踪于该 issue。本页描述以各自当前状态为准。
