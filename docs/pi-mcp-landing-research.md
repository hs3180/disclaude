...
**Factual finding retained from the part-2 upstream survey (PR #4455, closed — its "B1 stands" conclusion was stale, but its facts hold):** MCP in `earendil-works/pi` lives in the **app/extension layer** (`packages/coding-agent` + extension runtime — 121 issue/PR hits, a `/mcp` command, MCP HTTP servers, `structuredContent`), **not** in the embeddable **`agent-core`** library (0 `mcp` symbols in source + `dist`). This is exactly why disclaude — which embeds `agent-core`, not the full pi app — cannot reach MCP, and why the "wait for upstream" option (B1.5) was never viable. The distinction **supports** the no-MCP decision: pi's app-layer MCP is irrelevant to disclaude's embedded-library integration, so there is nothing to bridge and nothing to wait for.
