...
- [x] Scope-5 reuse decision — recorded above (this PR)
- [x] Scope-2/3/4 contract + policy docs — this page (this PR)
- [ ] Scope-6 smoke acceptance — a scripted round-trip (browser-use Skill
  attaching via `BU_CDP_URL`, executing a script, returning a result) against
  a live deployment. Needs a headless host run; tracked as the remaining part
  of #4496, not covered by this docs-only change.
- [ ] Optional: Playwright attaching to the same endpoint as a second driver
  (cross-driver reuse check — the contract above is written to make this a
