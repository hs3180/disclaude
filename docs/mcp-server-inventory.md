...
- **Runtime wiring**: retired in #4652. ChatAgent no longer constructs or injects `channel-mcp`; capability-aware
  messaging guidance points agents to the one-shot [`bin/disclaude.js channel`](../skills/channel/README.md) instead.
...
| **3. Migrate non-Playwright MCP tools → Skill** | **S1** (channel-mcp, 5 tools) | ✅ done — ChatAgent no longer creates/injects `channel-mcp` (#4652); all five operations use `bin/disclaude.js channel`; the first-party implementations + REST backing and standalone MCP export stay |
...
1. **Skill transport for S1's send-side tools**: `bin/disclaude.js channel` is one-shot and reaches PrimaryNode over REST.
