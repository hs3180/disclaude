...
`install` enables login-session autostart after readiness succeeds; `install --no-autostart` selects disabled login autostart. The saved `CHROMIUM_CDP_AUTOSTART=1|0` preference applies on install/start/restart; older configurations without it preserve existing enablement on start/restart. Setup offers the same choice interactively and through `--autostart` / `--no-autostart`. `restart`, `stop`, `status`, `logs`, `generate` and `uninstall` are also supported. `generate` only writes configuration and a unit; it does not assert browser readiness. Uninstall removes/disables the managed unit and retains browser configuration and profile data. Autostart means the user manager's default target, not boot without login; this command does not enable lingering.
...
The native lifecycle/setup CI runs headless and headed separately. The headed job uses an isolated Xvfb X11 display, passes DISPLAY/XAUTHORITY through the real systemd unit and verifies the unit has no headless flag. It exercises the same real browser start/restart/recovery/input/screenshot paths. This is a virtual-display test, not evidence of a physical desktop login or human intervention. Both jobs also test an explicitly unavailable user bus and a headed request with no display: diagnostics must precede configuration/unit creation. The unavailable-manager case isolates both DBUS_SESSION_BUS_ADDRESS and XDG_RUNTIME_DIR because systemctl can use the user manager’s private socket. On Ubuntu x64 with systemd255 and Chrome152.0.7977.82, source cb2f0cda passed both full suites in run35014473088: headed/Xvfb :99 in123.09seconds and headless in110.94seconds. Both verify unavailable-manager and missing-display diagnostics before configuration/unit creation.
