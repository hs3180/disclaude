...
The `service` service (the compute/agent service that runs Python, Pilot, and
...
>
> **Scope**: the GPU config applies to `service`, the only compute service. The
> `worker` / `test-service` services and `Dockerfile.worker` were removed in
> [#2964](https://github.com/hs3180/disclaude/pull/2964), so the current compose
> has only `service`, `chromium`, and `filebeat`. (If a worker service is ever
...
|---|---|---|---|
| `DOCKER_RUNTIME` | `runc` | `nvidia` | Docker runtime for the `service` container. `runc` is Docker's standard CPU-only runtime (a no-op); `nvidia` mounts host GPUs. |
...
| `WORKER_MEMORY` | `16G` | `40G` | Memory limit for `service`. GPU ML workloads need far more than the 16G CPU default. |
...
The `service` service in `docker-compose.yml` reads these via compose
...
`Dockerfile.service` is kept unchanged, and the GPU is exposed purely through
...
- **Dockerfile base-image decision** (#4285 refined requirement #3, the flagged
  blocker): whether a future `Dockerfile.service.gpu` based on
