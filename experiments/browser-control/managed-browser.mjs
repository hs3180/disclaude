...
export async function launchBrowser({ binary, profile, headless = false }) {
  if (!binary || !isAbsolute(binary) || !profile || !isAbsolute(profile)) throw new Error('Managed Chromium requires absolute binary and dedicated profile paths');
      ...
      try {
        if ((await stat(activeFile)).mtimeMs !== previous) {
          ...
          if (/^\d+$/.test(port) && +port > 0 && +port < 65536 && browserPath.startsWith('/devtools/browser/')) {
            ...
            if (response.ok && new URL(info.webSocketDebuggerUrl).pathname === browserPath) return { endpoint, child, stop };
          }
        }
